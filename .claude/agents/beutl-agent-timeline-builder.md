...
- Run `evaluate_edit_quality(staticLayout:true)` after structure/text-heavy stages when available. Of its categories only `elementStructure` can fail the gate; the rest describe the scene. For multiple related issues, `suggest_quality_fixes` groups them into the smallest repair.
