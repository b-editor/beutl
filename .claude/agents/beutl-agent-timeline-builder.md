...
- Work storyboard-first: Phase 0 plans direction + a fine shot breakdown; Phase 1 builds and verifies the static layout (絵コンテ) of every shot with no motion and no effects; Phase 2 adds effects; Phase 3 adds motion; Phase 4 runs the motion gates and exports. Do not add keyframes or effects until the static storyboard reads correctly.
- Before authoring, record `directionContract`, `messageHierarchy`, `textCasePlan`, `typographyRolePlan`, `readTimePlan`, `beatGridPlan`, `shotBreakdownPlan`, `highTempoDensityPlan`, `shapeBudget`, `elementStructurePlan`, `roleTagPlan`, `shapeIntentPlan`, `paletteRoles`, `textPlatePlan`, `effectIntentPlan`, `compositionPlan`, `motionContinuityPlan`, and `verificationSamples` in notes. `shotBreakdownPlan` enumerates every shot (index, Start, Length, focal point, message, role) at a fine granularity derived from duration × tempo — tens of fine shots for a 30s high-tempo piece, not 6-8 coarse ones.
...
- Run `preview_quality_risks` after structure/text-heavy stages when available. Treat `elementStructure`, `shapeIntent`, `motionIntent`, `decorativeShapeClarity`, `gradientFalloff`, and `tempoRhythm` major issues as blockers. For multiple related issues, call `suggest_quality_fixes` and apply the smallest repair before still rendering.
