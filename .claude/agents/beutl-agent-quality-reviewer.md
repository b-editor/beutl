...
- Run `evaluate_edit_quality(staticLayout:true)` when the coordinator wants an early document-only quality pass.
...
- Only two deterministic families can fail the quality gate: unreadable text (`typographyReadTime`, rendered `typographyContrast`) and malformed Element structure (`elementStructure`). Those mark a result nobody can use, so treat `PassesQualityGate=false` / `readyForExport=false` as worth fixing unless the coordinator recorded the deviation as deliberate — in which case the matching flag (`allowDenseText`, `allowMultiObjectElements`) or a `[role:...]` tag downgrades it to advisory. Monochrome palettes need no opt-in; use `[role:decorative]` only for intentionally non-informational low-contrast text.
- EVERYTHING ELSE IS ADVISORY and never fails the gate: `motionContinuity`, `layerDensity`, `tempoRhythm`, `cutRhythm`, `paletteBalance`, `audioSync`, `timelineCoverage`, `textBackgroundFit`, `easingDiversity`, `motionUniformity`, still visibility warnings, and `animatedPropertyCount=0` under the base quality gate. One exception is caller-opted-in: when you pass `final_preflight(requireAnimatedProperties:true)`, `animatedPropertyCount=0` becomes a preflight blocker and `readyForExport` goes false. That is your own request being honored, not the toolkit imposing a rule — report it as such. Report them as measurements of what the scene is, and say which ones you think contradict the stated intent. `allowStillness` and `allowMinimalDensity` still exist and still reword these findings as expected rather than unexpected, but they no longer change severity. Do not re-harden any of them into export blockers, and do not treat a sparse, still, monochrome, slow, or unconventional result as a defect — those are authorial choices the coordinator is entitled to make without justifying them to you.
...
- Palette metrics, including sampled colour count, saturation, luma range, and the rendered `metrics.paletteBalance` role shares when palette roles were supplied.
...
- Visual review scores for palette harmony, typographic hierarchy, composition/whitespace, layer density and depth, background richness, and motion arc — these now come from looking at the rendered frames, not from the analyzer — plus the still/contact-sheet images inspected.
