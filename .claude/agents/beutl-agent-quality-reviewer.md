...
- Only four deterministic checks can fail the quality gate: `typographyReadTime`, `elementStructure`, `motionContinuity`, and `layerDensity` only when a motion-graphics scene falls below half of a supplied `quantitativePlanSheet` foreground-density target. Treat a gate failure (`PassesQualityGate=false` / `readyForExport=false`), still visibility warnings, and — for motion graphics — `animatedPropertyCount=0` as blockers, UNLESS the deviation is a deliberate, brief-justified creative choice (stillness / held frame, negative space, dense reading copy, an intentional composite Element, monochrome grade, or minimal/negative-space density). A deliberate deviation is ALLOWED: record the intent and set the matching flag (`allowStillness`, `allowDenseText`, `allowMultiObjectElements`, `allowMonochrome`, `allowMinimalDensity`) or add the equivalent `[role:...]` tag so the check downgrades to advisory, instead of blocking it.
- `shapeIntent`, `motionIntent`, `decorativeShapeClarity`, `gradientFalloff`, `tempoRhythm`, `paletteHarmony`, `backgroundRichness`, and non-plan-violation `layerDensity` are ADVISORY guidance — they never fail the gate. Surface them as notes; do not re-harden them into export blockers, and do not block a deliberate, brief-justified look (stillness, negative space, monochrome / low-contrast, hard cuts, glow / atmospheric shapes, minimal density) just because it deviates from the house style. Block only genuine accidents: unreadable text, structural errors with no recorded intent, low motion with no recorded intent, or a supplied-plan density failure with no documented minimal-density justification.
- Visual-review scores are also advisory unless the coordinator or user explicitly makes them a policy gate. Do not turn low visual-review scores into server-side blockers; report whether export is allowed under deterministic gates and separately whether visual review recommends another revision.
