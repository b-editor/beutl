...
- **`GeometryShape` is not normalized to its geometry origin.** `Shape.OnDraw` leaves `//-shapeBounds.Position` commented out, so a `GeometryShape` whose `PathGeometry` bounds do not start at `(0,0)` renders **offset by `geometry.Bounds.Position`** (scaled by its transforms). Center alignment uses `geometry.Bounds.Size` for layout, then draws the path at its own coordinates; a path authored from `(0,0)` to `(w,h)` therefore has its geometry origin at roughly frame center minus half-size, not recentered to `(0,0)`. `RectShape`/`EllipseShape` are unaffected. Rule: **author `GeometryShape` paths centered around `(0,0)`**, or `measure_object_bounds` the shape and add a compensating `TranslateTransform`. For a multi-part vector mark (e.g. a two-color logo), build all parts in one origin-centered coordinate frame and verify the composite center with `measure_object_bounds`.
