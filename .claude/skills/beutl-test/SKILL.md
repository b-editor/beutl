...
- Test framework: **NUnit + Moq**. Tests live under `tests/` in per-area projects (e.g. `tests/Beutl.UnitTests/`, `tests/Beutl.PublicApiContractTests/`, `tests/Beutl.Graphics3DTests/`, `tests/SourceGeneratorTest/`, `tests/Beutl.FFmpegIpc.Tests/`). `tests/Beutl.PublicApiContractTests/` is the non-friend compile gate for public authoring APIs. Note `tests/Beutl.Graphics3DTests/` is a Vulkan-gated NUnit suite (GPU rendering checks; self-skips via `Assert.Ignore` when no Vulkan device is available); GPU-free Graphics3D logic tests live under `tests/Beutl.UnitTests/Engine/Graphics3D/`.
