...
14. Run `preview_quality_risks` after structure/text-heavy stages when available. Treat major issues as early blockers, including `elementStructure`, `shapeIntent`, `motionIntent`, `decorativeShapeClarity`, `gradientFalloff`, and `tempoRhythm`; call `suggest_quality_fixes` if there are multiple related failures and apply the smallest repair before still rendering. For high-tempo promos, use a `styleProfile` such as `high-tempo-promo`, `kinetic-type`, or a BPM-bearing label such as `high-tempo-promo 130bpm`.
