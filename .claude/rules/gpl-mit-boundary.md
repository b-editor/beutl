...
1. **No `ProjectReference` from a MIT project to `Beutl.FFmpegWorker`.** The CI/local hook `.claude/hooks/check-gpl-mit-boundary.sh` denies edits that try to add one — do not work around it.
