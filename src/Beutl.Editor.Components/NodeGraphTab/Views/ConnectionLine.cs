...
public sealed class ConnectionLine : Line
{
    ...
    public static readonly StyledProperty<ConnectionViewModel?> ConnectionViewModelProperty =
        AvaloniaProperty.Register<ConnectionLine, ConnectionViewModel?>(
            nameof(ConnectionViewModel)
        );
        ...
        _strokeBinding = this.GetResourceObservable("TextControlForeground")
            .CombineLatest(
                ...
                this.GetObservable(ConnectionViewModelProperty)
                    .Select(o =>
                        o?.Status as IObservable<ConnectionStatus>
                        ?? Observable.ReturnThenNever(ConnectionStatus.Disconnected)
                    )
                    .Switch()
            )
    ...
    public INodePort? GetTarget(INodePort? port)
    {
        return InputPort?.Model == port ? OutputPort?.Model
            : OutputPort?.Model == port ? InputPort?.Model
            : null;
    }
    ...
    protected override Geometry CreateDefiningGeometry()
    {
        ...
        context.CubicBezierTo(
            StartPoint.WithX(StartPoint.X - delta),
            EndPoint.WithX(EndPoint.X + delta),
            EndPoint
        );
