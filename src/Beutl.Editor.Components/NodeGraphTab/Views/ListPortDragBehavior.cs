...
public sealed class ListPortDragBehavior : Behavior<NodePortPoint>
{
    ...
    private void OnPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        if (point.Properties.IsLeftButtonPressed)
        {
            ...
            _nodeView = AssociatedObject.FindAncestorOfType<GraphNodeView>();
            if (_stackPanel != null)
            {
                _enableDrag = true;
                _dragStarted = false;
    ...
    private void ReleaseVertical()
    {
        ...
        {
            if (AssociatedObject?.DataContext is NodePortViewModel portViewModel)
            {
                portViewModel.MoveConnectionSlot(_draggedIndex, _targetIndex);
            }
        }
    }
    ...
    private void MoveHorizontal(PointerEventArgs e)
    {
        ...
        if (_tempLine == null)
        {
            ...
            Point? portPosition = AssociatedObject.TranslatePoint(new(5, 5), _canvas);
            if (portPosition.HasValue)
            {
                if (AssociatedObject.DataContext is InputPortViewModel)
                {
                    _tempLine.EndPoint = portPosition.Value;
                    _tempLine.StartPoint = e.GetPosition(_canvas);
                }
                else
                {
                    _tempLine.StartPoint = portPosition.Value;
    ...
    private void AddTransforms()
    {
        ...
        for (int i = 0; i < connectedCount; i++)
        {
            if (_stackPanel.Children[i] is NodePortPoint sp)
            {
    ...
    private void UpdateConnectionPositionsDuringDrag()
    {
        ...
        for (int i = 0; i < connectedCount; i++)
        {
            if (_stackPanel.Children[i] is NodePortPoint sp && sp.Tag is ConnectionViewModel connVM)
            {
