    ...
    private void PasteKeyFrame(string json, TimeSpan pointerPosition)
    {
        ...
        try
        {
            ...
            if (discriminator.GenericTypeArguments[0] != animation.ValueType)
            {
                ...
                NotificationService.ShowWarning(Strings.GraphEditor, Message.KeyframePropertyTypeMismatch_EasingApplied);
                return;
            }
            ...
            if (animation.KeyFrames.FirstOrDefault(k => k.KeyTime == keyTime) is { } existingKeyFrame)
            {
                ...
                NotificationService.ShowWarning(Strings.GraphEditor, Message.KeyframeExistsAtPastePosition);
            }
            else
            {
