...
public sealed class GraphEditorTabViewModel : IToolContext
{
    ...
    public void Refresh()
    {
        ...
        foreach (
            IProperty prop in searcher
                .SearchAll()
                .OfType<EngineObject>()
                .SelectMany(o => o.Properties)
        )
        {
            if (prop.Animation is not KeyFrameAnimation anm)
                continue;

            string name = Property.GetLocalizedName(prop);
            var item = new GraphEditorItemViewModel(name, anm);
            tmp.Add(item);
        }
        ...
        foreach (INodeMember member in searcher.SearchAll().OfType<INodeMember>())
        {
            if (
                member.Property
                is not IAnimatablePropertyAdapter
                {
                    Animation: KeyFrameAnimation anm,
        ...
        foreach (var item in Items)
        {
            ...
            _animationDisposables.Add(
                Disposable.Create(
                    item.Object,
                    obj => obj.DetachedFromHierarchy -= OnAnimationDetached
                )
            );
        }
    }
    ...
    public void ReadFromJson(JsonObject json)
    {
        try
        {
            ...
            if (
                json.TryGetPropertyValueAsJsonValue("elementId", out Guid elmId)
    ...
    {
        if (
            SelectedAnimation.Value is
            { Element.Id: { } elmId, Animation: ICoreObject { Id: var anmId } }
        )
        {
