...
public partial class InlineAnimationLayerHeader : UserControl
{
    public InlineAnimationLayerHeader()
    {
        ...
        Interaction
            .GetBehaviors(this)
            .Add(new _DragBehavior() { DragControl = border, Orientation = Orientation.Vertical });
    }
    ...
    private void OpenTab_Click(object? sender, RoutedEventArgs e)
    {
        if (
            ...
            && viewModel.Property
                is IAnimatablePropertyAdapter { Animation: KeyFrameAnimation kfAnimation }
        )
        {
    ...
    private sealed class _DragBehavior : GenericDragBehavior
    {
        ...
        {
            if (
                AssociatedObject?.DataContext is InlineAnimationLayerViewModel viewModel
