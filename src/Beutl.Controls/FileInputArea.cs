...
public class FileInputArea : ContentControl
{
    ...
    public static readonly StyledProperty<string> TextProperty =
        TextBlock.TextProperty.AddOwner<FileInputArea>(
            new StyledPropertyMetadata<string>(DefaultTextValue)
        );
    ...
    private void OnDrop(object sender, DragEventArgs e)
    {
        try
        {
            if (e.DragEffects != DragDropEffects.None && _matchResult != null)
            {
    ...
    {
        if (_patternContexts != null && e.DataTransfer.TryGetFiles() is { } files)
        {
    ...
    private async void OnButtonClick(object sender, RoutedEventArgs e)
    {
        if (VisualRoot is TopLevel toplevel)
        {
            ...
            {
                IsEnabled = false;
                IReadOnlyList<IStorageFile> items =
                    await toplevel.StorageProvider.OpenFilePickerAsync(
                        OpenOptions ?? s_defaultOptions
                    );
                if (items.Count > 0)
                {
    ...
    private static IStorageFile Match(
        List<IPatternContext> patternContexts,
