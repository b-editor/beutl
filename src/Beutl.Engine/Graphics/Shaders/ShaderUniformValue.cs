...
internal readonly record struct ShaderUniformValue(
    float[]? Floats,
    int[]? Integers,
    bool IsInteger)
{
    public static ShaderUniformValue Create<T>(T value)
        where T : unmanaged
    {
        object boxed = value;
        return boxed switch
    ...
    public void ThrowIfIncompatible(SkslUniformDeclaration declaration)
    {
        ...
        int actual = IsInteger ? Integers!.Length : Floats!.Length;
    ...
    private static ShaderUniformValue Float(float[] values) => new(values, null, false);

    private static ShaderUniformValue Integer(int[] values) => new(null, values, true);
}
