...
public readonly struct PaintedRenderSession
{
    ...
    /// value-equal holder keeps reuse, but the identity then tracks only what that holder's equality compares,
    /// so a pixel-affecting change covered by neither it nor a declared resource's version is served from a
    /// stale cached output — a node that keeps one holder and mutates it in place draws its first frame once and
    /// then serves those pixels for every later frame; and a token left over from
