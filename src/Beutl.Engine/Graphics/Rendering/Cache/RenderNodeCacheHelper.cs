...
public static class RenderNodeCacheHelper
{
        ...
        PipelineDiagnostics? diagnostics = null, RenderTargetPool? pool = null)
    {
        ...
        if (CanCacheRecursive(node))
        {
            ...
            {
                CreateDefaultCache(node, cacheOptions, outputScale, maxWorkingScale, diagnostics, pool);
            }
        }
        else if (node is ContainerRenderNode containerNode)
        {
            ...
            foreach (RenderNode item in containerNode.Children)
            {
                MakeCache(item, cacheOptions, outputScale, maxWorkingScale, diagnostics, pool);
            }
        }
    }
        ...
        PipelineDiagnostics? diagnostics = null, RenderTargetPool? pool = null)
    {
        ...
        var processor = new RenderNodeProcessor(node, false, outputScale, maxWorkingScale, diagnostics, pool);
