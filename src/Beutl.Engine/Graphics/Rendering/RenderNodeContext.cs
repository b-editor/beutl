...
public sealed class RenderNodeContext
{
    ...
    /// produced value's output-cache runtime identity. It must be a lightweight immutable CPU value.
    /// </param>
    /// <param name="draw">
    /// A non-capturing callback. Declare it <see langword="static"/>: a capture would let a per-frame value
    /// shape the output without reaching <paramref name="state"/>, and is rejected.
    ...
    /// </param>
    /// <param name="resources">
    /// Any further resources the callback reaches through
    /// <see cref="PaintedRenderSession.UseDeclaredResource"/>, in the order it indexes them. Index 0 is the
    /// first of these: <paramref name="primary"/> is declared ahead of them but is the recorder's declaration
    /// rather than the author's, so it is outside the positional space, exactly as the lowered paint's slots
    /// are.
    /// </param>
    /// <returns>A new transaction-scoped value fragment. The result is not published automatically.</returns>
    /// <remarks>
    /// The form to reach for whenever a painted source draws one resource, which is every in-tree one: it
    /// replaces an index, a type argument, a <c>UseDeclaredResource</c> call and a nested closure with the
    /// <paramref name="primary"/> argument, and the resource arrives type-checked at compile time instead of at
    /// execution. A callback that needs a second resource keeps addressing that one positionally.
    /// </remarks>
