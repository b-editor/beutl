...
/// <see cref="FilterEffect.Resource.RenderNodeFactory"/> (an internal sealed subclass) supplies that, along with the
/// per-node plan and prefix caches. A plugin that overrides <see cref="FilterEffect.Resource.RenderNodeFactory"/> to
/// build a custom subclass must implement <see cref="Process"/> itself and does not inherit that caching.
/// </summary>
