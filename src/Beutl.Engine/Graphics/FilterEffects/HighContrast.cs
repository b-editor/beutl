...
public sealed partial class HighContrast : FilterEffect
{
    ...
    [Display(
        Name = nameof(GraphicsStrings.HighContrast_Grayscale),
        ResourceType = typeof(GraphicsStrings)
    )]
    public IProperty<bool> Grayscale { get; } = Property.CreateAnimatable(false);

    [Display(
        Name = nameof(GraphicsStrings.HighContrast_InvertStyle),
        ResourceType = typeof(GraphicsStrings)
    )]
    public IProperty<HighContrastInvertStyle> InvertStyle { get; } =
        Property.CreateAnimatable(HighContrastInvertStyle.NoInvert);

    [Display(
        Name = nameof(GraphicsStrings.HighContrast_Contrast),
        ResourceType = typeof(GraphicsStrings)
    )]
