...
internal sealed class AudioVisualizerRenderNode(AudioVisualizerDrawable.Resource resource)
    : RenderNode
{
    public (AudioVisualizerDrawable.Resource Resource, int Version)? Visualizer
    {
        get;
        private set;
    } = resource.Capture();
    ...
    public override RenderNodeOperation[] Process(RenderNodeContext context)
    {
        ...
        [
            RenderNodeOperation.CreateLambda(
                bounds,
                canvas => resource.RenderToCanvas(canvas, bounds)
            ),
        ];
    }
