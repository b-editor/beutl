...
/// </summary>
[Display(Name = nameof(Strings.GraphicEqualizer), ResourceType = typeof(Strings))]
public sealed partial class GraphicEqualizerEffect : AudioEffect
{
    ...
    private BandCountPreset _lastBandCount;

    public GraphicEqualizerEffect()
    {
        ScanProperties<GraphicEqualizerEffect>();
        InitializeBands(BandCountPreset.Bands10);
        _lastBandCount = BandCountPreset.Bands10;
    }
    ...
    {
        // Reinitialize if band count has changed
        var currentPreset = BandCountOption?.CurrentValue ?? BandCountPreset.Bands10;
        if (_lastBandCount != currentPreset)
        {
            InitializeBands(currentPreset);
            _lastBandCount = currentPreset;
        }
