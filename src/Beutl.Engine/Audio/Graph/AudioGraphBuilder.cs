...
public sealed class AudioGraphBuilder
{
    ...
    public T AddNode<T>(T node) where T : AudioNode
    {
        ...
        if (_built) throw new Exception("Cannot add nodes after the graph has been built.");

        if (_nodes.Contains(node)) throw new Exception("Node has already been added to this builder.");
