...
public readonly struct BitmapColorSpaceXyz : IEquatable<BitmapColorSpaceXyz>
{
    ...
    public static BitmapColorSpaceXyz Bt470M { get; } =
        FromPrimaries(0.67, 0.33, 0.21, 0.71, 0.14, 0.08, 0.310, 0.316);

    /// <summary>BT.470BG (PAL/SECAM, D65)</summary>
    public static BitmapColorSpaceXyz Bt470BG { get; } =
        FromPrimaries(0.64, 0.33, 0.29, 0.60, 0.15, 0.06, 0.3127, 0.3290);

    /// <summary>SMPTE 170M (NTSC, D65)</summary>
    public static BitmapColorSpaceXyz Smpte170M { get; } =
        FromPrimaries(0.630, 0.340, 0.310, 0.595, 0.155, 0.070, 0.3127, 0.3290);
    ...
    public static BitmapColorSpaceXyz Film { get; } =
        FromPrimaries(0.681, 0.319, 0.243, 0.692, 0.145, 0.049, 0.310, 0.316);

    /// <summary>SMPTE ST 431-2 / DCI-P3 (DCI white)</summary>
    public static BitmapColorSpaceXyz Smpte431 { get; } =
        FromPrimaries(0.680, 0.320, 0.265, 0.690, 0.150, 0.060, 0.314, 0.351);

    /// <summary>EBU Tech. 3213-E / JEDEC P22 (D65)</summary>
    public static BitmapColorSpaceXyz Ebu3213 { get; } =
        FromPrimaries(0.630, 0.340, 0.295, 0.605, 0.155, 0.077, 0.3127, 0.3290);

    /// <summary>ITU-R BT.709 / ITU-R BT1361 / IEC 61966-2-4 / SMPTE RP 177 Annex B</summary>
    public static BitmapColorSpaceXyz Bt709 { get; } =
        FromPrimaries(0.640, 0.330, 0.300, 0.600, 0.150, 0.060, 0.3127, 0.3290);
    ...
    {
        var v = _values;
        return new SKColorSpaceXyz(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8]);
    }
    ...
    public override string ToString()
    {
        if (this == Srgb)
            return "sRGB";
        else if (this == AdobeRgb)
            return "Adobe RGB";
        else if (this == Dcip3)
            return "DCI-P3";
        else if (this == Rec2020)
            return "Rec.2020";
        else if (this == Xyz)
            return "XYZ";
        else if (this == Bt470M)
            return "BT.470M";
        else if (this == Bt470BG)
            return "BT.470BG";
        else if (this == Smpte170M)
            return "SMPTE 170M";
        else if (this == Film)
            return "Film";
        else if (this == Smpte431)
            return "SMPTE ST 431";
        else if (this == Ebu3213)
            return "EBU 3213";
        else if (this == Bt709)
            return "BT.709";
        else
            return $"Custom({string.Join(", ", _values)})";
    }
    ...
    )
    {
        ...
        double det = Xr * (Yg * Zb - Yb * Zg) - Xg * (Yr * Zb - Yb * Zr) + Xb * (Yr * Zg - Yg * Zr);
        double invDet = 1.0 / det;

        double Sr =
            ((Yg * Zb - Yb * Zg) * Xw + (Xb * Zg - Xg * Zb) * Yw + (Xg * Yb - Xb * Yg) * Zw)
            * invDet;
        double Sg =
            ((Yb * Zr - Yr * Zb) * Xw + (Xr * Zb - Xb * Zr) * Yw + (Xb * Yr - Xr * Yb) * Zw)
            * invDet;
        double Sb =
            ((Yr * Zg - Yg * Zr) * Xw + (Xg * Zr - Xr * Zg) * Yw + (Xr * Yg - Xg * Yr) * Zw)
            * invDet;
        ...
        // Bradford行列
        const double b00 = 0.8951,
            b01 = 0.2664,
            b02 = -0.1614;
        const double b10 = -0.7502,
            b11 = 1.7135,
            b12 = 0.0367;
        const double b20 = 0.0389,
            b21 = -0.0685,
            b22 = 1.0296;

        // Bradford逆行列
        const double bi00 = 0.9869929,
            bi01 = -0.1470543,
            bi02 = 0.1599627;
        const double bi10 = 0.4323053,
            bi11 = 0.5183603,
            bi12 = 0.0492912;
        const double bi20 = -0.0085287,
            bi21 = 0.0400428,
            bi22 = 0.9684867;
