        ...
        string? videoType = null)
    {
        return Execute(() =>
        {
            ...
            [
                ...
                "Flow operators (DrawableGroup, DrawableDecorator, SoundGroup, Scene3D) need a PortalObject immediately before them in Element.Objects. PortalObject.Count=N pulls the timeline Elements at ZIndex rig+1..rig+N into the operator so grouped content keeps per-Element timing and stays visible as timeline layers; Count=0 groups only the operator's nested Children. See get_examples insert-camera-rig-portal and insert-camera-rig-push-in.",
