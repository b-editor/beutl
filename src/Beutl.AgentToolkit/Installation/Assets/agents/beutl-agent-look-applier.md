...
- Run `evaluate_edit_quality(staticLayout:true)` for the document-only checks it still performs: short-lived typography/read time, text-backing relationships, Element/Object structure, and high-tempo rhythm.
- When the look change touches foreground `RectShape` objects, abstract decorative light shapes, ambient/glow gradients, or other large/animated shapes, inspect the before/after `render_still` output with the `beutl-agent-visual-review` rubric; the static analyzer no longer evaluates those look and shape judgments.
- Run `evaluate_edit_quality` after the look change and resolve critical/major issues before export.
- `PassesQualityGate=false` comes only from unreadable text (`typographyReadTime`, rendered `typographyContrast`) or malformed Element structure (`elementStructure`) — the cases where the result is unusable rather than unusual. Everything else the analyzer reports (`motionContinuity`, `layerDensity`, `tempoRhythm`, `cutRhythm`, `paletteBalance`, `audioSync`, `textBackgroundFit`) is an advisory measurement. Look and shape judgments are no longer analyzed at all — read a `render_still` for those. A deliberately still, monochrome, sparse, or atmospheric look is a valid result; monochrome palettes need no opt-in. The intent flags (`allowStillness`, `allowDenseText`, `allowMultiObjectElements`, `allowMinimalDensity`) and `[role:...]` tags make their targeted findings read as expected rather than unexpected. Use `[role:decorative]` only for intentionally non-informational low-contrast text.
