...
namespace BEditor.Drawing
{
    public static unsafe partial class Image
    {
        ...
        public static Image<BGRA32> Blur_(this Image<BGRA32> image, int size)
        {
            var dst = new Image<BGRA32>(image.Width, image.Height);
            Parallel.For(0, image.Height, yy =>
            {
                Parallel.For(0, image.Width, xx =>
                {
                    var minY = Math.Max(0, yy - size);
                    var maxY = Math.Min(yy, yy + size);
                    var ver = maxY - minY;
                    ...
                    float bAvgF = 0;
                    float aAvgF = 0;
                    var minX = Math.Max(0, xx - size);
                    var maxX = Math.Min(image.Width, xx + size);
                    ...
                    dst[xx, yy] = new(rAvg, gAvg, bAvg, aAvg);
                });
            });

            return dst;
        }
        public static void Blur(this Image<BGRA32> image, DrawingContext context, int size)
        {
            using var memory = context.Context.CreateMappingMemory(image.DataSize);

            image.PixelOperate<BlurOperation, AbstractMemory, int>(context, memory, size);

            memory.Read(context.CommandQueue, true, image.Data, 0, image.DataSize);
        }
    }
}