...
public static class LibraryRegistrar
{
    ...
    {
        LibraryService.Current.AddMultiple(
            GraphicsStrings.EllipseShape,
            m => m.BindDrawable<Graphics.Shapes.EllipseShape>().BindGeometry<EllipseGeometry>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.RectShape,
            m => m.BindDrawable<Graphics.Shapes.RectShape>().BindGeometry<RectGeometry>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.RoundedRectShape,
            m =>
                m.BindDrawable<Graphics.Shapes.RoundedRectShape>()
                    .BindGeometry<RoundedRectGeometry>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.GeometryShape,
            m => m.BindDrawable<Graphics.Shapes.GeometryShape>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.TextBlock,
            m => m.BindDrawable<Graphics.Shapes.TextBlock>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.SourceVideo,
            m => m.BindDrawable<SourceVideo>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.SourceImage,
            m => m.BindDrawable<SourceImage>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.SourceBackdrop,
            m => m.BindDrawable<SourceBackdrop>()
        );

        LibraryService.Current.AddMultiple(
            AudioStrings.SourceSound,
            m => m.BindSound<SourceSound>()
        );

        LibraryService.Current.RegisterGroup(
            $"3D ({Strings.Experimental})",
            g =>
                g.AddMultiple(GraphicsStrings.Scene3D, m => m.BindDrawable<Scene3D>())
                    .AddMultiple(GraphicsStrings.Cube3D, m => m.BindEngineObject<Cube3D>())
                    .AddMultiple(GraphicsStrings.Sphere3D, m => m.BindEngineObject<Sphere3D>())
                    .AddMultiple(GraphicsStrings.Plane3D, m => m.BindEngineObject<Plane3D>())
                    .AddMultiple(GraphicsStrings.Model3D, m => m.BindEngineObject<Model3D>())
                    // Lights
                    .AddMultiple(
                        GraphicsStrings.DirectionalLight3D,
                        m => m.BindEngineObject<DirectionalLight3D>()
                    )
                    .AddMultiple(
                        GraphicsStrings.PointLight3D,
                        m => m.BindEngineObject<PointLight3D>()
                    )
                    .AddMultiple(
                        GraphicsStrings.SpotLight3D,
                        m => m.BindEngineObject<SpotLight3D>()
                    )
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.ParticleEmitter,
            m => m.BindDrawable<ParticleEmitter>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.NodeGraphDrawable,
            m => m.BindDrawable<NodeGraphDrawable>()
        );

        LibraryService.Current.AddMultiple(Strings.Portal, m => m.BindEngineObject<PortalObject>());

        LibraryService.Current.AddMultiple(
            GraphicsStrings.SceneDrawable,
            m => m.BindDrawable<SceneDrawable>()
        );

        LibraryService.Current.AddMultiple(AudioStrings.SceneSound, m => m.BindSound<SceneSound>());

        LibraryService.Current.AddMultiple(
            GraphicsStrings.DrawableDecorator,
            m => m.BindDrawable<DrawableDecorator>()
        );

        LibraryService.Current.AddMultiple(
            GraphicsStrings.Group,
            m => m.BindDrawable<DrawableGroup>()
        );

        LibraryService.Current.AddMultiple(AudioStrings.SoundGroup, m => m.BindSound<SoundGroup>());

        LibraryService.Current.AddMultiple(
            GraphicsStrings.DrawableTimeController,
            m => m.BindDrawable<DrawableTimeController>()
        );

        LibraryService.Current.RegisterGroup(
            GraphicsStrings.AudioVisualizer,
            g =>
                g.AddMultiple(
                        GraphicsStrings.AudioWaveform,
                        m => m.BindDrawable<AudioWaveformDrawable>()
                    )
                    .AddMultiple(
                        GraphicsStrings.AudioSpectrum,
                        m => m.BindDrawable<AudioSpectrumDrawable>()
                    )
                    .AddMultiple(
                        GraphicsStrings.AudioSpectrogram,
                        m => m.BindDrawable<AudioSpectrogramDrawable>()
                    )
        );

        LibraryService.Current.RegisterGroup(
            GraphicsStrings.Transform,
            g =>
                g.AddTransform<TranslateTransform>(GraphicsStrings.TranslateTransform)
        ...
        LibraryService.Current.RegisterGroup(
            GraphicsStrings.FilterEffect,
            g =>
                g.AddFilterEffect<Blur>(GraphicsStrings.Blur)
                    ...
                    .AddGroup(
                        GraphicsStrings.Script,
                        gg =>
                            gg.AddFilterEffect<CSharpScriptEffect>(
                                    GraphicsStrings.CSharpScriptEffect
                                )
        ...
        LibraryService.Current.RegisterGroup(
            AudioStrings.AudioEffect,
            g =>
                g.AddAudioEffect<DelayEffect>(AudioStrings.DelayEffect)
