...
public sealed class CommandPaletteService
{
        ...
        if (_commandManager != null)
        {
            ...
            foreach (ContextCommandEntry entry in _commandManager.GetDefinitions())
            {
                ...
                if (
                    ...
                    && (
                        activeEditor is null
                        || activeEditor.DockHost.FindToolContext(entry.ExtensionType) is null
                    )
                )
                {
                    continue;
                ...
                KeyGesture? gesture = entry
                    .KeyGestures.FirstOrDefault(i => i.Platform == s_currentPlatform)
                    ?.KeyGesture;
                ...
                IContextCommandHandler? snapshotHandler = _handlerProvider.Resolve(
                    entry.ExtensionType
                );
                ...
                IObservable<System.Reactive.Unit>? stateChanged = (
                    handler as IContextCommandStateNotifier
                )?.CanExecuteChanged;
                ...
                result.Add(
                    new PaletteCommand(
                        ...
                        CanExecute: () =>
                            handler.CanExecute(
                                new ContextCommandExecution(capturedEntry.Definition.Name)
                            ),
                        Execute: () =>
                        {
                            // スロットル窓や状態変化で表示と実行可否がずれる可能性があるため、
                            // 実行直前にもう一度 CanExecute を確認してから Execute する。
                            var execution = new ContextCommandExecution(
                                capturedEntry.Definition.Name
                            );
    ...
    private string ResolveCategoryName(Type extensionType)
    {
        ...
        Extension? matched = ExtensionProvider.Current.AllExtensions.FirstOrDefault(i =>
            i.GetType() == extensionType
        );
    ...
    private void AppendMenuCommands(List<PaletteCommand> commands)
    {
        ...
        foreach (
            MenuBarViewModel.PaletteMenuCommand menuCommand in menuBar.EnumeratePaletteCommands()
        )
        {
