...
public partial class AudioEffectEditor : UserControl
{
    ...
    private void Drop(object? sender, DragEventArgs e)
    {
        if (e.Data.Contains(KnownLibraryItemFormats.AudioEffect)
            ...
            && DataContext is IAudioEffectViewModel viewModel)
        {
            ...
            {
                viewModel.ChangeEffectType(type);
            }
    ...
    {
        if (DataContext is not IAudioEffectViewModel viewModel) return;
    ...
    {
        if (DataContext is IAudioEffectViewModel viewModel)
        {
            viewModel.SetNull();
        }
    }
