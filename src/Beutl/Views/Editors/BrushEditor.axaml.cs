...
public sealed partial class BrushEditor : UserControl
{
    ...
    {
        if (DataContext is not BrushEditorViewModel { IsDisposed: false } viewModel)
            return;
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        ...
        if (change.Property == OriginalBrushProperty)
        {
            ...
            _flyout.DrawableName = GetDrawableName(
                (OriginalBrush as Media.DrawableBrush)?.Drawable.CurrentValue
            );
            _flyout.CanEditDrawable =
                (OriginalBrush as Media.DrawableBrush)?.Drawable.CurrentValue is not null;
    ...
    private void OnDrawableBrushPropertyChanged(
        object? sender,
        PropertyValueChangedEventArgs<Drawable?> e
    )
    {
        if (_flyout == null)
            return;
    ...
    {
        if (DataContext is not BrushEditorViewModel)
            return;
    ...
    private void OnEditDrawableClicked(object? sender, EventArgs e)
    {
        ...
        if (DataContext is not BrushEditorViewModel { IsDisposed: false } viewModel)
            return;
        if (viewModel.Value.Value is not DrawableBrush { Drawable.CurrentValue: { } drawable })
            return;
    ...
    private void OnBrushTypeChanged(object? sender, BrushType e)
    {
        ...
        if (e == BrushType.SolidColorBrush)
        {
            viewModel.SetValue(
                ...
                new SolidColorBrush { Color = { CurrentValue = Colors.White } }
            );
        }
        else if (e == BrushType.Null)
        {
            ...
            e
            is BrushType.ConicGradientBrush
        ...
        )
        {
            ...
            {
                gradStops.AddRange(
                    oldBrush.GradientStops.Select(v => new GradientStop(
                        v.Color.CurrentValue,
                        v.Offset.CurrentValue
                    ))
                );
            }
            else
    ...
    {
        if (DataContext is not BrushEditorViewModel viewModel)
            return;
        if (viewModel.Value.Value is not SolidColorBrush)
            return;
    ...
    private void OnColorChanged(object? sender, (Color2 OldValue, Color2 NewValue) e)
    {
        if (DataContext is not BrushEditorViewModel viewModel)
            return;
        if (viewModel.Value.Value is not SolidColorBrush solid)
            return;
        if (viewModel.IsDisposed)
            return;
    ...
    private void OnGradientStopAdded(
        object? sender,
        (int Index, Avalonia.Media.GradientStop Object) e
    )
    {
        if (DataContext is not BrushEditorViewModel { IsDisposed: false } viewModel)
            return;
    ...
    private void OnGradientStopDeleted(
        object? sender,
        (int Index, Avalonia.Media.GradientStop Object) e
    )
    {
    ...
    private void OnGradientStopConfirmed(
        ...
        (
            int OldIndex,
            int NewIndex,
            Avalonia.Media.GradientStop Object,
            Avalonia.Media.Immutable.ImmutableGradientStop OldObject
        ) e
    )
    {
        if (DataContext is not BrushEditorViewModel viewModel)
            return;
        if (viewModel.Value.Value is not GradientBrush { GradientStops: { } list })
            return;
        if (viewModel.IsDisposed)
            return;
    ...
    {
        if (DataContext is not BrushEditorViewModel { IsDisposed: false } vm)
            return;
        if (_flyoutOpen)
            return;
