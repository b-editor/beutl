...
public partial class HistoryView : UserControl
{
    ...
    private void TryAttach()
    {
        ...
        _viewModelSubscriptions = new CompositeDisposable
        {
            viewModel.CurrentIndex.Subscribe(
                _ => SyncSelection(),
                ex =>
                    _logger.LogError(ex, "CurrentIndex stream errored; selection will stop syncing")
            ),
        };

        ((INotifyCollectionChanged)viewModel.Entries).CollectionChanged += OnEntriesChanged;
        _viewModelSubscriptions.Add(
            Disposable.Create(
                viewModel,
    ...
    {
        if (_currentViewModel is null || HistoryList is null)
            return;
    ...
    private void UpdateItemAppearance()
    {
        ...
        {
            if (HistoryList.ContainerFromIndex(i) is not ListBoxItem item)
                continue;
    ...
    {
        if (_currentViewModel is null || e.Container is not ListBoxItem item)
            return;
