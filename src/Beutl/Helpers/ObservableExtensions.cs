...
{
    public static IObservable<bool> EqualTo<T>(this IObservable<T> left, IObservable<T> right)
    {
        return left.CombineLatest(right)
            .Select(t => EqualityComparer<T>.Default.Equals(t.First, t.Second));
    }
    ...
    public static IObservable<bool> AreTrue(this IObservable<bool> first, IObservable<bool> second)
    {
        return first.CombineLatest(second).Select(x => x.First && x.Second);
    }
    ...
    {
        return first.CombineLatest(second, third).Select(x => x.First && x.Second && x.Third);
    }
    ...
    {
        return first
            .CombineLatest(second, third, fourth, fifth, sixth, seventh, eighth)
            .Select(x =>
                x.First
                && x.Second
                && x.Third
    ...
    public static IObservable<bool> AnyTrue(this IObservable<bool> first, IObservable<bool> second)
    {
        return first.CombineLatest(second).Select(x => x.First || x.Second);
    }
    ...
    public static IObservable<(TSource? OldValue, TSource? NewValue)> CombineWithPrevious<TSource>(
