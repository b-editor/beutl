...
)
{
    ...
    )
    {
        string localNupkgPath = Path.Combine(Helper.LocalSourcePath, $"{package}.nupkg");
        s_logger.LogDebug(
            "Reading local source for package {PackageId} at path {Path}",
            package.Id,
            localNupkgPath
        );
        if (File.Exists(localNupkgPath))
        {
            try
            {
                ...
                s_logger.LogInformation(
                    "Successfully read local package {PackageId} from path {Path}",
                    package.Id,
                    localNupkgPath
                );
                return new PackageChangeModel(
                    package.Id,
                    package.Version,
                    nuspec.GetTitle() ?? package.Id,
                    false,
                    action
                )
                {
            ...
            catch (Exception ex)
            {
                s_logger.LogError(
                    ex,
                    "An exception occurred while reading nupkg file for package {PackageId} at path {Path}",
                    package.Id,
                    localNupkgPath
                );
            }
        ...
        {
            s_logger.LogWarning(
                "Local package {PackageId} not found at path {Path}",
                package.Id,
                localNupkgPath
            );
        }
    ...
    )
    {
        ...
        if (splited.Length == 2)
        {
            ...
            try
            {
                ...
                item = new PackageChangeModel(
                    pkg.Id,
                    pkg.Version,
                    package.DisplayName.Value ?? pkg.Id,
                    true,
                    action
                )
                {
            ...
            catch (Exception ex)
            {
                s_logger.LogError(
                    ex,
                    "An exception occurred while discovering package {PackageId}",
                    pkg.Id
                );
        ...
        else if (splited.Length == 1)
        {
            try
            {
                ...
                if (releases.Length > 0)
                {
                    ...
                    var pkg = new PackageIdentity(
                        package.Name,
                        new NuGetVersion(release.Version.Value)
                    );
                    bool alreadyInstalled = repos.ExistsPackage(pkg);
                    s_logger.LogInformation(
                        "Successfully discovered package {PackageId} with release version {Version}",
                        package.Name,
