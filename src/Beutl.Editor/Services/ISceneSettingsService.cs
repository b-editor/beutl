...
/// (frame size, start, duration, preview source). Key behavior: change detection
...
public interface ISceneSettingsService
{
    ...
    /// <paramref name="duration"/>, and <paramref name="previewSourceMode"/> to <paramref name="scene"/>. Returns true and
    /// commits one <c>ChangeSceneSettings</c> when at least one field differs;
    /// otherwise false and no commit.</summary>
    bool Apply(Scene scene, PixelSize frameSize, TimeSpan start, TimeSpan duration, PreviewSourceMode previewSourceMode);
}
