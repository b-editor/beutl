...
public class QuerySchemaBuilder
{
    ...
    public QuerySchemaBuilder Select(string fieldName)
    {
        _fields.Add(new QueryField(fieldName));
        return this;
    }

    public QuerySchemaBuilder Select(string fieldName, Action<QueryFieldBuilder> configure)
    {
    ...
    public QuerySchemaBuilder Select<T>(Expression<Func<T, object>> expression)
    {
    ...
    public QuerySchemaBuilder Select<T, TProperty>(Expression<Func<T, TProperty>> expression, Action<QueryFieldBuilder> configure)
    {
        string fieldName = GetPropertyName(expression);
        var builder = new QueryFieldBuilder();
        configure(builder);
    ...
    {
        return new QuerySchema(_fields.ToArray());
    }
