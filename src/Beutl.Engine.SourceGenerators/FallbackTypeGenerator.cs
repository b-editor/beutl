...
public sealed class FallbackTypeGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var classDeclarations = context
            .SyntaxProvider.CreateSyntaxProvider(
                ...
                static (syntaxContext, cancellationToken) =>
                    FallbackClassAnalyzer.TryExtract(syntaxContext, cancellationToken)
            )
        ...
        context.RegisterSourceOutput(
            ...
            static (productionContext, pair) => Execute(productionContext, pair.Right)
        );
    }

    private static void Execute(
        SourceProductionContext context,
        ImmutableArray<FallbackClassInfo> classes
    )
    {
