...
public static class DiagnosticDescriptors
{
    ...
    public static readonly DiagnosticDescriptor CapturingMetadataCallback = new(
        ...
        description:
            ...
            + "was written. A captured non-constant local or parameter has none of that - nothing "
            + "re-records when it is assigned, and the plan compiled for its first answer is replayed for "
            + "the second - and "
    ...
    public static readonly DiagnosticDescriptor UnmarkedRenderNodeMutation = new(
        ...
        description:
            ...
            + "where the rule can read it: a member the node's own type or a source base type in the same "
            + "compilation declares that code outside it can write - an auto-property whose setter is "
            + "neither private nor init-only, a field-like event "
            ...
            + "so that only the node's own code, which the first shape does read, can write it. "
            + "Declarations available only as metadata stay invisible because their source shape cannot be "
            + "distinguished here. The Process both shapes read is the "
