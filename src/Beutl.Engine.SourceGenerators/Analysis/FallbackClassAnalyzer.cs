...
{
    public static FallbackClassInfo? TryExtract(
    ...
    )
    {
        ...
        INamedTypeSymbol? iFallbackSymbol = context.SemanticModel.Compilation.GetTypeByMetadataName(
            "Beutl.Serialization.IFallback"
        );
        ...
        bool implementsIFallback = symbol.AllInterfaces.Any(iface =>
            SymbolEqualityComparer.Default.Equals(iface, iFallbackSymbol)
        );
    ...
    private static ImmutableArray<AbstractMethodInfo> CollectUnimplementedAbstractMethods(
        INamedTypeSymbol symbol
    )
    {
        ...
        {
            foreach (
                IMethodSymbol method in current
                    .GetMembers()
                    .OfType<IMethodSymbol>()
                    .Where(m =>
                        m is { IsAbstract: true, MethodKind: MethodKind.Ordinary, IsStatic: false }
                    )
            )
            {
    ...
    private static ImmutableArray<AbstractMethodInfo> CollectResourceAbstractMethods(
        INamedTypeSymbol symbol
    )
    {
        var result = ImmutableArray.CreateBuilder<AbstractMethodInfo>();
        ...
        )
        {
            ...
            {
                foreach (
                    IMethodSymbol method in nestedType
                        .GetMembers()
                        .OfType<IMethodSymbol>()
                        .Where(m =>
                            m
