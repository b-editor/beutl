...
public static class ToResourceMethodEmitter
{
    ...
    )
    {
        ...
        if (info.Symbol.IsAbstract)
        {
            sb.Append(indent)
                .AppendLine(
                    $"public abstract override {currentTypeDisplay}.Resource ToResource({renderContextType} context);"
                );
        }
        else
        {
            sb.Append(indent)
                .AppendLine(
                    $"public override {currentTypeDisplay}.Resource ToResource({renderContextType} context)"
                );
            sb.Append(indent).AppendLine("{");
            sb.Append(indent)
                .AppendLine($"    var resource = new {currentTypeDisplay}.Resource();");
