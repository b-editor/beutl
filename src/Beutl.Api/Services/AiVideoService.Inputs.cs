        ...
        try
        {
            ...
            return await ExecuteAsync("AiVideoService.FromSource",
                (authorization, token) => Application.Ai.CreateSourceVideo(authorization, key, mode,
                    AiMultipartFormData.File(video, request.SourceVideo.FileName, request.SourceVideo.MediaType, "sourceVideo"),
                    character is null ? null : AiMultipartFormData.File(character, request.CharacterImage!.FileName, request.CharacterImage.MediaType, "characterImage"),
