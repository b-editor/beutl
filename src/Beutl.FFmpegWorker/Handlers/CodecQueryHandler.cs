...
internal sealed class CodecQueryHandler
{
    ...
    {
        var request =
            msg.GetPayload<QueryCodecsRequest>()
    ...
    {
        var request =
            msg.GetPayload<QueryPixelFormatsRequest>()
        ...
        {
            WorkerLog.Warning(
                $"QueryPixelFormats: codec-specific query failed, falling back to all formats: {ex.Message}",
                ex
            );
            // フォールバック: 全対応フォーマット
            var allFmts = Enum.GetValues<AVPixelFormat>()
                .Where(f =>
                    f != AVPixelFormat.AV_PIX_FMT_NONE
                    && (int)f >= 0
    ...
    {
        var request =
            msg.GetPayload<QuerySampleRatesRequest>()
    ...
    {
        var request =
            msg.GetPayload<QueryAudioFormatsRequest>()
    ...
    {
        var request =
            msg.GetPayload<QueryDefaultCodecRequest>()
        ...
        try
        {
            ...
            string? videoCodec =
                outFormat.VideoCodec != AVCodecID.AV_CODEC_ID_NONE
            ...
            return IpcMessage.Create(
                msg.Id,
                MessageType.QueryDefaultCodecResult,
