...
internal sealed class IpcFrameProvider : IFrameProvider
{
    ...
    public IpcFrameProvider(
        IpcConnection connection,
        SharedMemoryBuffer[] videoBuffers,
        long frameCount,
        Rational frameRate
    )
    {
    ...
    public async ValueTask<Bitmap> RenderFrame(long frame)
    {
        ...
        var frameInfo =
            response.GetPayload<ProvideFrameMessage>()
        ...
        if (nextFrame < FrameCount)
        {
            ...
            var nextRequest = IpcMessage.Create(
                _connection.NextId(),
                MessageType.RequestFrame,
                new RequestFrameMessage
                {
                    FrameIndex = nextFrame,
                    BufferIndex = _prefetchBufferIndex,
                }
            );
        ...
        unsafe
        {
            _videoBuffers[readBufferIndex]
                .Read(new Span<byte>((void*)bmp.Data, frameInfo.DataLength));
        }
