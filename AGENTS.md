...
This rule never authorizes scope expansion. Before addressing PR feedback, freeze the PR's intended behavior, affected modules, and acceptance tests. Classify every finding as an original-scope defect, a regression introduced by any remediation in the current review cycle, a pre-existing or adjacent issue, an optional improvement, or an acceptance gap. Only the first two may be fixed without reopening scope. A valid but out-of-scope finding stays out of the branch unless the user explicitly widens the PR; an unanswered prompt never widens the branch and follows only the configured conservative default. Public-API changes, new project dependencies, and newly touched subsystems are always scope expansion unless the frozen scope already names them.
