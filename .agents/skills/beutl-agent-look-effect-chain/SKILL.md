...
- Preserve Element/Object structure. `elementStructure`, `shapeIntent`, `motionIntent`, `decorativeShapeClarity`, `gradientFalloff`, and `tempoRhythm` issues from `preview_quality_risks` or `evaluate_edit_quality` are blockers introduced by the look change.
