...
description: |
  ...
  Codex-review, and humans), decide which genuinely need a code change, address the clearly
...
`copilot-pull-request-reviewer[bot]`, the Codex reviewer, and the repo's `Codex-review` output —
...
- **Clear bot false positives:** when a **known bot's** comment is demonstrably wrong (you can point
  ...
  the pattern to `.Codex/loop-memory/bot-false-positive-patterns.md`** (D-8): one line per pattern —
  ...
  `mkdir -p .Codex/loop-memory` first if it does not exist.
