...
`Codex-review.yml`) and **hand-added tasks** — including features. This skill turns any one
...
- A **test** plan (see Step 5) — this repo requires new logic to ship with an NUnit test
  (`.Codex/rules/csharp.md`, AGENTS.md rule #3).
...
own formatting. Respect subtree AGENTS.md rules (e.g. `Beutl.Engine` forbids UI references and
...
- **Self-review (six checks).** (1) new/changed XAML declares `x:CompileBindings="True"` + `x:DataType`;
  ...
  cause**, not a symptom; (5) the GPL/MIT boundary is intact; (6) subtree `AGENTS.md` rules honored.
  (The `.Codex/hooks/check-gpl-mit-boundary.sh` hook also enforces #5 mechanically; this check is the
