...
description: |
  ...
  reviews (CodeRabbit / Copilot / Codex / Codex), classifies the PR's risk, and then auto-merges
...
allowed-tools: Task, Read, Grep, Glob, Write, Edit, Bash(gh:*), Bash(git:*), Bash(dotnet:*), Bash(python3:*), Bash(jq:*), Bash(mktemp:*), Bash(mkdir:*), Bash(rm:*), Bash(date:*), Bash(sleep:*), Bash(timeout:*), Bash(find:*), Bash(head:*), Bash(tail:*), Bash(bash .Codex/scripts/*:*)
...
Codex session running on **opus** with auto-accept (`acceptEdits`) enabled. There is **no
headless `Codex -p` launcher** (that path moved billing to metered API usage and was removed). The
...
- **TRIGGER** manual, in-session (`/beutl-loop` in an interactive opus / `acceptEdits` session). No
  cron, no headless `Codex -p` launcher.
...
3. **Initialize the journal** at `.Codex/logs/beutl-loop-state.json` (gitignored, ephemeral). Run
   `mkdir -p .Codex/logs` first — on a fresh checkout the gitignored directory is absent, and the
   journal write, the coverage probe's `mktemp -d .Codex/logs/...`, and the run summary all need it.
...
3. **Load loop memory (D-7 — cross-run, gitignored at `.Codex/loop-memory/`).** `mkdir -p
   .Codex/loop-memory` first. If present, read these advisory files — they never override the board
   ...
   - `bot-false-positive-patterns.md` — patterns where bots (CodeRabbit / Copilot / Codex / Codex)
...
- `false_positive: true` → a **candidate** false positive (the runner did **not** touch the board).
  **Spot-check it first:** independently verify the runner's cited refutation against the current code.
  - **Confirmed** → move the board item to `False positive` (`e6ff360e`) now — the item **leaves the
    ...
    `.Codex/loop-memory/false-positive-signatures.json`** (D-7). Continue.
...
- `blocked: true` → record `{reason, kind}`; **never stop the whole drain on a single item** (skip it
  ...
  `{item_id, kind, reason}` to `.Codex/loop-memory/blocked-reasons.json`** (D-7), then by
  `blocked_kind`:
...
- **No result / invalid result (runner crashed, timed out, or returned malformed/non-JSON):** the
  ...
  `.Codex/loop-memory/blocked-reasons.json` (D-7), increment `consecutive_no_progress`, and set
...
- GPL/MIT boundary: run `bash .Codex/scripts/check-gpl-mit-boundary-diff.sh origin/main "origin/$DRAFT_BRANCH"`
...
if [ "$TEST_RC" -ne 0 ]; then
  ...
  if [ -z "$COV" ]; then
    ...
    python3 .Codex/scripts/changed-line-coverage.py origin/main "origin/$DRAFT_BRANCH" "$COV" --threshold 70
    PROBE_RC=$?
    rm -rf "$COV_DIR"
...
**Also emit a JSON run summary** (H-15) at `.Codex/logs/beutl-loop-run-<run_id>.json` for cross-run
