---
description: |
  Run the same checks locally that `Codex-review.yml` (CI) and the `beutl-reviewer`
...
allowed-tools: Bash(dotnet:*) Bash(git:*) Bash(./build.sh:*) Bash(bash .Codex/scripts/*:*)
...
The PreToolUse hook `.Codex/hooks/check-gpl-mit-boundary.sh` only inspects fragments from `Edit` / `Write` tool calls (`new_string`, `content`, `edits[].new_string`) — it cannot scan the working tree retroactively. So this step runs the shared diff-side scan script, which targets only the two forbidden linkage forms (`ProjectReference` and `Compile Include`); a bare mention of the `Beutl.FFmpegWorker` namespace in a comment, doc string, or already-linked source file is fine.
...
bash .Codex/scripts/check-gpl-mit-boundary-diff.sh --files $CHANGED
```
