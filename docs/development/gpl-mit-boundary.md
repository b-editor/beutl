...
3. **Share code via `<Compile Include="..." Link="..." />`.**
   - The `Beutl.FFmpegWorker` `.csproj` link-includes specific shared sources from `Beutl.Extensions.FFmpeg.Core` and `Beutl.Extensions.FFmpeg`.
