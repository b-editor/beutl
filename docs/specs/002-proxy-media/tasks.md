...
- [ ] T004 [P] Skim `src/Beutl.Engine/Media/Source/MediaSource.cs`, `VideoSource.cs`, and `src/Beutl.Engine/Media/Decoding/DecoderRegistry.cs` / `MediaOptions.cs` end-to-end and note any uses of `MediaOptions` that will need the new `PreferProxy` flag (planning input for T013). **Also (post-003)** skim `src/Beutl.Engine/Graphics/SourceImage.cs`, `SourceVideo.cs`, and `src/Beutl.Engine/Graphics/Rendering/{ImageSourceRenderNode,VideoSourceRenderNode}.cs` — note how logical size is derived from decoded `FrameSize` (`r.Source.FrameSize.ToSize(1)`) and how `EffectiveScale.At(1f)` is hard-coded; these are the seam sites for the logical-size decoupling (T062–T065). Confirm `src/Beutl.Engine/Graphics/Rendering/EffectiveScale.cs` exists (003) and read its `At`/`Unbounded` API.
...
- [ ] T024 [P] [US1] Create `tests/Beutl.UnitTests/ProjectSystem/SceneComposerExportSafetyTests.cs` asserting that every `MediaOptions` constructed on the **export** path has `PreferProxy == false` (use a test seam — see T030). **Post-003**: 003 routes export through `OutputViewModel` (builds `SceneRenderer(Model, supersampleScale, disableResourceShare: true)`; downscales in `FrameProviderImpl.RenderCore`). Audit/grep **all** export-path `MediaOptions` construction sites — the pre-003 `SceneComposer` helper **and** any 003 export call site — and assert each sets `PreferProxy = false`. The T030 test seam must cover whichever sites actually build `MediaOptions` after 003.
...
- [ ] T030 [US1] On the **export** path, ensure every `MediaOptions` constructed sets `PreferProxy = false` **explicitly** (even though that's the default — defense-in-depth for FR-002/FR-004). **Post-003**: do this at every export `MediaOptions` site (the `SceneComposer` helper and/or the 003 `OutputViewModel` / `FrameProviderImpl` path — grep to find them all). Add a small `internal` test seam (e.g., factory method) so T024 and T025 can assert this without reflection. Ensure that an open call against a missing source surfaces the existing decoder-failure path *unchanged* (T025 (a)) — do not introduce a silent fallback
- [ ] T031 [US1] Modify `src/Beutl.ProjectSystem/ProjectSystem/SceneRenderer.cs` so `MediaOptions.PreferProxy` is set from `Scene.PreviewSourceMode` (which gets its real wiring in US3; for now, default to `PreferProxy = true` since the spec default is `PreferProxy`). **Post-003**: `SceneRenderer`'s ctor is `(Scene scene, float renderScale = 1f, float maxWorkingScale = +∞)` (003 breaking change) and the renderer is rebuilt by-replacement off `(FrameSize, OutputScale)` (003 FR-031). Do NOT add a renderer rebuild for the proxy toggle — toggling `PreviewSourceMode` changes only per-source supply density (FR-023), so invalidate the affected sources' render-cache entries and re-queue a render; read `PreferProxy` fresh inside the render work-item (as 003 reads the renderer fresh). Add NUnit coverage in `tests/Beutl.UnitTests/ProjectSystem/SceneRendererPreviewRoutingTests.cs`
...
- [ ] T040 [US2] Implement `src/Beutl.Engine/Media/Proxy/ProxyPresetDefinitions.cs` — table `ProxyPreset → ProxyEncodeParameters` with starting values from research R-5 (`Half`: 1/2 scale, CRF 25; `Quarter`: 1/4 scale, CRF 26 (default); `Eighth`: 1/8 scale, CRF 28). Long-edge clamps included. `tune=fastdecode preset=fast` baked in. **Post-003**: the preset's scale factor **is** the proxy's supply density (`ProxyResolution.SupplyDensity`, FR-022) and is intentionally aligned with the 003 preview render-scale vocabulary (`Half`/`Quarter`). Document the interaction in a code comment: under 003's `w = max(s_out, densest supply)`, a proxy denser than the active preview scale still saves decode cost but the effect pass runs at the proxy density (not the cheaper preview scale); a proxy at/below the preview scale lowers `w` fully. (See spec FR-017.)
...
- Within US1: `IProxyResolver` impl precedes `DecoderRegistry` wire-up (T029, which hands the resolved `ProxyResolution` to the source layer) which precedes the 003 logical-size seam (T063 source → T064 render node → T065 cache invalidation) which precedes `SceneRenderer` / export-path wiring (T030/T031). The seam (T062–T065) is part of US1 — US1's guarantee is incorrect without it.
...
- The export path (`SceneComposer` and/or the 003 `OutputViewModel` / `FrameProviderImpl` path — audit both) is the spec's headline safety floor — every change touching it must keep T024 + T025 + T030 green.
