...
- The **preview** `SceneCompositor` seeds the render context's `PreferProxy` from the editor-wide `EditorConfig.PreviewSourceMode` (originally specified on `Scene`; see the R-8 supersession note); `VideoSource.Resource.Update` reads it when constructing `new(MediaMode.Video)` — the sole video `MediaOptions` site (`src/Beutl.Engine/Media/Source/VideoSource.cs`). (`SceneRenderer` / `SceneComposer` do **not** construct `MediaOptions`; the compositor's `CompositionContext` is the seam, as a sibling to the existing mutable `DisableResourceShare` property — `src/Beutl.Engine/Composition/CompositionContext.cs`, `src/Beutl.ProjectSystem/SceneCompositor.cs`.)
