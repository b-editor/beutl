...
- Proxy identity is keyed on **source content fingerprint** (path + size + mtime; content hashing is out of scope for MVP — see FR-010), not on the project-level clip object. Two timeline clips referencing the same file share one proxy on disk.
