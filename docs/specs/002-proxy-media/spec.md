...
**Why this priority**: This is the headline value of the feature. Without it, editing 4K/8K footage on commodity hardware is painful, and proxies are useless unless the export path is guaranteed to use the originals.
...
- **FR-002**: The system MUST, during any export / final render, decode from the original source media of every clip — never from a proxy — regardless of whether a proxy exists.
...
- **FR-017**: The system MUST ship with **2–3 fixed proxy quality presets** at MVP, all H.264-based, differing primarily by scale (e.g., 1/2 source resolution and 1/4 source resolution; a third intermediate preset MAY be included). The user MUST be able to choose which preset is used when generating a proxy, and the chosen preset MUST be recorded with each generated proxy so a later regeneration can match (or deliberately replace) it.
...
- **SC-003**: Switching the project-level preview source between "Proxy" and "Original" takes effect on the next preview frame (no project reload required), within **2 seconds** on a project of ≤500 clips.
