...
- **Stale proxy detection**: source file is edited / replaced externally after the proxy was generated. Beutl must detect this (via mtime + size, or content hash) and treat the proxy as stale rather than silently serving outdated frames.
...
- **Source media types that do not benefit from proxies**: very small images, short audio-only clips, generative / procedural sources. The feature should not waste disk and CPU on these.
...
- **FR-002**: The system MUST, during any export / final render, decode from the original source media of every clip — never from a proxy — regardless of whether a proxy exists. **Rationale (post-003)**: at export `s_out = 1.0`, 003's supply-driven floor rule `w = max(s_out, densest supply)` would lift a sub-output proxy (e.g. `At(0.5)`) back up to `w = 1.0`, i.e. the proxy would be **upsampled** (soft) into the effect pass. Routing export through the original (`At(1.0)`) is therefore not only the FR-004 safety floor but also the only way to keep export full-fidelity under 003's model.
...
- **FR-021**: The system MUST preserve each proxied clip's **logical footprint** when decoding from a proxy. As shipped by 003, a source's logical size is derived from its decoded `FrameSize` (`SourceImage`/`SourceVideo` return `r.Source.FrameSize.ToSize(1)`, and `ImageSourceRenderNode`/`VideoSourceRenderNode` hard-code `EffectiveScale.At(1)`). Naively pointing the decoder at a smaller proxy file would therefore shrink the clip's decoded `FrameSize`, shrink its on-canvas bounds, and move its content. Proxy decode MUST instead carry the **original** source `FrameSize` as a stable logical size — independent of the proxy's decoded `FrameSize` — so the clip's bounds, layout, and hit region are identical whether the backing decode is original or proxy (the 003 seam: US3 acceptance #1 / SC-007 / 003 FR-023). This is the "stable intrinsic-logical-size channel" 003 explicitly deferred to the proxy feature.
- **FR-022**: Each proxied source operation MUST report its supply density as the ratio `proxyDecodedPixels / originalLogicalPixels` (e.g. `0.5` for a `Half` proxy) via the 003 `EffectiveScale.At(scale)` annotation, replacing the current hard-coded `EffectiveScale.At(1)`. The decoded proxy bitmap MUST be drawn into the original-footprint logical destination rect (the 003 FR-024 dest-rect seam), so 003's supply-driven rule reconciles the lower-density proxy exactly once at the composite boundary (`w = max(s_out, densest supply)`; Mitchell resample). A proxy never moves content and never changes logical bounds; it only lowers supply density.
...
- **FR-020**: The system is NOT required to proxy audio-only clips, procedural/generative sources, or static images below a size threshold; these MAY be skipped silently.

### Key Entities

- **Source media reference**: the project's existing handle to an on-disk source file (video, image). Identity key for proxies is the triple `(absolute path, file size, mtime)`. Content hashing is not used at MVP.
...
- **SC-005**: A user new to the feature can generate a proxy for a clip and confirm it is being used for preview within **2 minutes** of opening the project, with no help beyond the in-app UI.
...
- The feature targets **video** as the primary beneficiary; static images and other media types may be proxied or skipped per FR-020. Audio is out of scope for proxy generation in the MVP.
