...
- clarification phase session 2026-05-20 resolved five additional ambiguities (see `## Clarifications` in `spec.md`): proxy preset shape (2–3 fixed H.264 presets), staleness fingerprint (`path + size + mtime`), stale handling (manual regenerate at MVP), concurrency (serial, 1 job), and cache cap (global LRU with configurable default).
- One spec-level assumption is structurally load-bearing: **preview path and export path are distinct in `Beutl.Engine`**. If planning phase finds them merged in a single media-resolution layer, that is a planning-phase finding, not a spec defect — call it out and adjust the plan accordingly. *(Post-003: 003 confirmed export routes through `OutputViewModel`; the remaining risks are the `PreferProxy` render-context seam and the logical-size seam below.)*
...
- **Readiness remediation (2026-06-27)**: implementation-start blockers were resolved before switching `spec.md` to `Ready for Implementation`: Engine now owns only an `IProxyGenerator` abstraction while the concrete FFmpeg generator lives in `Beutl.Extensions.FFmpeg`; `IProxyJobQueue` uses async `EnqueueAsync` so bounded-channel back-pressure is coherent; `ProxyEntry` / `proxy-index.schema.json` now persist `originalLogicalFrameSize` and `proxyDecodedFrameSize`; `meta.json` sidecar shape is pinned under `$defs.proxySourceMetadata`; proxy generation E2E tests are routed to `tests/Beutl.UnitTests/Extensions/FFmpeg/`, which already references the FFmpeg extension.
- **Contract cleanup (2026-06-27)**: implementation-start ambiguity was removed from job/store lifecycle wording. `Skipped` is a job/UI result (`ProxyJobStatus.Skipped` + `ProxyJobChangeKind.Skipped` + `StatusMessage`), not a persisted `ProxyState`; cancel is a `Canceled` job terminal that deletes `*.tmp` and restores the previous store entry (or `None`) without recording `Failed`; `IProxyResolver.Pin` is explicitly acquired before opening the proxy reader and released after reader disposal.
- Items marked incomplete require spec updates before the clarification phase or planning phase.
