...
Each fragment/value record computes pure conservative aggregate bounds, contribution/query metadata, effective scale, value cardinality, and CPU hit-test metadata immediately from already-recorded input metadata. Standard and custom scale contracts use complete output bounds at this point; a custom resolver cannot observe the later ROI, and the 16,384-axis clamp is applied against those complete bounds. Target commands expose no value supply/cardinality, while public target capture resolves an explicit working density and remains non-contributing until `ContributeValues` is recorded. Runtime shrink/discard and later ROI cropping remain within the declaration and never change density; graph-wide analysis later validates/finalizes metadata and performs reverse ROI, but never executes work to make a parent handle readable.
...
- **Let public value-producing Layer accept symbolic Full**: its immediately readable Bounds/density and value-input eligibility would depend on an enclosing scope that is not yet known.
...
**Rationale**: The donor's top-down requested-bounds propagation occurs while children are pulled, before later effect bounds are known. That can under-request coordinate-changing operations. A separate region map also avoids mutating public handles as cache substitution changes execution producers.
...
- `CurrentPixel`: a mechanically validated `half4 apply(half4 color)` snippet with identity bounds and fusible semantics;
...
CurrentPixel has no output-position coordinate and permits only value-coordinate resources proven independent of destination position. WholeSource receives local output device pixels (`0.5,0.5` at the first pixel center); the execution context exposes the logical origin, complete output bounds, required logical region, device bounds, working density, and input supply density. The implicit `src` child maps that local coordinate back through input logical origin/density. Extra resource bindings explicitly declare value, output-logical, or output-device coordinates and create any local matrix during scoped execution binding.

CurrentPixel preserves its input supply, including an unbounded vector fragment, until the whole eligible run reaches one materialization; the standard supply-driven density is then resolved once for the run. WholeSource is a materialization boundary and publishes the standard concrete density from mapped complete bounds. Direct/custom unmanaged uniform values use a validated canonical scalar/vector/matrix allowlist and reject pointers/native handles/padding-dependent blobs.
...
Existing color filters, Skia filters, and transforms lower to typed operations only when their equivalence and bounds are proven. Unsupported engine-controlled items lower to the appropriate capability-guarded opaque value or target-scope topology. Existing `CustomEffect` keeps its raw `CustomFilterEffectContext`/materialized `EffectTarget` execution behavior and therefore lowers to a distinct `LegacyCustomEffect` opaque-external island. Every other retained public/protected raw-`ImmediateCanvas` hook—including custom `IBackdrop.Draw`, audio-visualizer foreground/shape callbacks, `RawTargetScope`, and `RawTargetCommand`—lowers to `LegacyRawCanvas` opaque-external work. Their outer fragment/order/resource ownership is planned, but internal passes/flushes are unmeasured, nothing fuses through them, and diagnostics mark the limitation. The built-in SnapshotBackdrop/DrawBackdrop pair instead uses typed capture/binding and never calls the raw interface in the same request. A repository-wide migration census must classify every old `CreateLambda`, decorator, target/surface wrapper, and raw-canvas hook; none remains as an executable-operation escape.
...
Before behavior changes, capture a new target-baseline category with starting SHA, generator script/patch, environment, file hashes, immutable `AssertExisting` behavior, and non-vacuity comparisons. Use linear-light SSIM >= 0.99, linear RGB MAE <= 0.02, and alpha MAE <= 0.02 for scale-1 parity. Multiple scales/regions and fallback use freshly recorded baseline tolerances. Every workload must change by more than its parity threshold plus a recorded margin when the operation under test is disabled.
...
Seven test overrides and all direct `RenderNodeProcessor`, executable-operation factory/subclass, operation-backed `EffectTarget`, `OperationWrapperRenderNode.SetOperations`, cache replay, hit-test/bounds, NodeGraph measure/preview, ProjectSystem `SceneDrawable`, AgentToolkit query, thumbnail, brush, texture-source, and player consumers migrate in the same breaking change. A repository-wide census test or source scan must keep this list from silently shrinking around an unmigrated executable path.
