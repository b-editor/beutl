...
- target baseline code at the actual pre-feature parent `88ce0e132`, the parent of this branch's first commit; the branch later merged main at `9352a5cf3`, and the migration census below was counted at `83e63689d`;
...
**Withdrawn.** This section adjudicates workloads and artifacts belonging to the
evidence tree retired at R14; no `docs/specs/004-gpu-pass-fusion/evidence/`
directory exists, so `refresh-intentional-visual-baselines.sh` and the
`scene3d-with-2d-tail` / `nested-drawable-brush-delay` artifacts named below have
no referent, and the delivered benchmark set (`RenderPipelineBenchmarkScenes`)
contains neither workload. The adjudications are kept as the record of what was
decided; the refresh instruction and its runnable command have been removed.
...
*Amended.* The recorded-bounds-hint mechanism was superseded. The materializer now
crops its snapshot to the compiled output bounds and reports them as
`MaterializedDrawableBrush.ContentBounds`, which `BrushConstructor.CreateTileShader`
feeds to `TileBrushCalculator` as the content size; the executor separately
recompiles the brush request against the compiled `Measurement.QueryBounds` when
that intrinsic domain differs from the destination domain. The hint shape reported
the brush's destination box as the source size, so a 40 px drawable filling a
160 px shape covered only 42 px.
...
resolver caps snapping at 2.5e-5 steps while retaining the 8e-6 tick-truncation
floor. That precision fix shipped on main in #2160 rather than as part of feature
004; this branch inherits it. Ten-minute 30000/1001 and 60000/1001 sweeps are
exact. If the authoritative MCP pixel probe still diverges after this precision
fix, its remaining mechanism is downstream of `ParticleSimulator` state and
requires a frame-level capture of the evaluated `ParticleEmitter.Resource` plus
its render-node inputs; the current evidence does not justify changing
deterministic simulation or RNG semantics.
...
**Decision**: Add a renderer-neutral shader primitive in `Beutl.Graphics.Effects`, accepted by both `FilterEffectContext.Shader` and `RenderNodeContext.Shader`. The public authoring pair is the immutable `ShaderDefinition<TState>` and the per-recording `ShaderCall<TState>` it binds state and resources into; `ShaderDescription` is the engine-internal lowered form both contexts consume. [contracts/public-api.md](contracts/public-api.md) carries the delivered authoring shape.
...
- `WholeSource`: a complete shader with mandatory bounds contract that may lead a fused run containing only downstream CurrentPixel or opacity stages.
...
*Amended.* Skia runtime effects are no longer the only execution backend. An engine-authored `ShaderDescription` may carry a `SpirvShaderLowering` beside its SkSL source, and the executor's internal `ShaderBackendPreference` — `Auto` by default — runs an eligible single-stage description through SPIR-V. Supplying that internal lowering is the engine's automatic-selection contract; extensions cannot assert it. `Auto` requires Beutl's Vulkan or Vulkan-backed composite recording context, RGBA16F input and output at equal density, and matching complete footprints; an unknown third-party context or native program-creation failure falls back to the mandatory SkSL lowering. The built-in LumaColor description is the first production lowering. Because it is already a normal `Shader` fragment, native selection changes neither semantic-opacity planning nor where the graph requires materialization; directly replayable shader roots retain the cheaper SkSL path, while an already-materializing single-stage luma run may select SPIR-V. Multi-stage runs retain SkSL fusion. The native path has its own program cache and `SpirvShaderRunExecutions` counter, caches one immutable program identity per lowering instead of rebuilding boxed binding keys per frame, submits recorded native work before releasing its input, and is covered by executor-level parity, fallback, scale, submission, and cache-reuse tests. This work does not solve the separately tracked Skia/Vulkan image-layout synchronization limitation in b-editor/beutl#2263; tests that exercise that shared-image hand-off retain the existing `KnownVulkanSkiaLayoutInterop` classification.

*Amended.* The two-layer authoring pair this decision chose was collapsed before delivery. `ShaderDefinition<TState>` and `ShaderCall<TState>` no longer exist; `ShaderDescription` is public and is the recording surface both contexts take. Nothing about the shader model above changed with them — the two forms, the validation, the structural/execution split, and the fusion rules are as decided. What was abandoned is the premise that a reusable schema object had to be declared separately from the recording: a plan is keyed by the callback's method and the declared contracts and never by the values a recording carries, so a description rebuilt where it is recorded compiles the plan a held definition would have. The uniform surface moved with it: `ShaderBindingBuilder` (public, non-generic) writes a value directly rather than mapping it out of a `TState`, and the `bindings` action it is handed runs inside the factory and is never retained, so it may close over the recording's values. The execution-time binders that action registers are still held to the non-capturing rule this decision states. [contracts/public-api.md](contracts/public-api.md) carries the delivered authoring shape.
...
- **Folding work upstream of a WholeSource shader into the same run**: WholeSource sampling is too broad to prove
  that rewrite equivalent. A WholeSource shader may instead lead a run and feed only downstream CurrentPixel or
  opacity stages, which preserves its authored sampling before applying per-pixel transforms.
...
*Amended.* The definition/call pair was collapsed the same way R8's was. `GeometryDefinition<TState>` and `GeometryCall<TState>` do not exist; `GeometryDescription` is public and `GeometryDescription.Create<TState>` takes the state, the `render` callback, the contracts, and the declared slots and bindings in one expression. The structural-identity argument this decision rests on is unchanged and is in fact what made the collapse safe: the shape is the `render` method handle plus the declared contracts, so it is identical whether the author holds one object or rebuilds it per recording. `CreateRequestLocal` remains engine-internal, so there is still no public capturing escape hatch.
...
**Decision**: `FilterEffect.ApplyTo(FilterEffectContext, Resource)` remains the only abstract effect entry point. Existing `FilterEffectContext` methods and ordering remain available. `Shader(ShaderCall<TState>)` and `Geometry(GeometryCall<TState>)` append to the same transactional ordered item list. During render-node recording, the effect resource invokes `ApplyTo` to produce descriptions only; activation, target allocation, GPU access, and custom callback execution are deferred.
...
*Amended.* The two parameter types named above were renamed by the collapse recorded under R8 and R9. The delivered methods are `FilterEffectContext.Shader(ShaderDescription)` and `FilterEffectContext.Geometry(GeometryDescription)`; everything else this decision states is unchanged.
...
The donor's `004-parity-strong` eight references may be imported as supplemental regressions because they are independently reproducible from a historical legacy activator — not imported; the same-process fusion A/B in `GpuPassFusionSameProcessParityHarness` covers this ground. Donor absolute timing and effect-local workload observations are historical only. Timings are remeasured on the target baseline with persistent production-equivalent renderer state, while the feature plan and component-local cache/pool statistics are validated in their owning tests.
