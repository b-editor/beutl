...
*Amended.* The definition/call pair was collapsed the same way R8's was. `GeometryDefinition<TState>` and `GeometryCall<TState>` do not exist; `GeometryDescription` is public and `GeometryDescription.Create<TState>` takes the state, the `render` callback, the contracts, and the ordered resource bindings in one expression. The structural-identity argument this decision rests on is unchanged and is in fact what made the collapse safe: the shape is the `render` method handle plus the declared contracts, so it is identical whether the author holds one object or rebuilds it per recording. `CreateRequestLocal` remains engine-internal, so there is still no public capturing escape hatch.
