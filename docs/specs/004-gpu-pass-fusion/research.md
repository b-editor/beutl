...
- **Expose target-relative/symbolic public capture density**: breaks the recording-time concrete `EffectiveScale` contract and would require symbolic scale propagation through every downstream Shader, Geometry, opaque, custom-resolver, and cache identity. That is a separate scale-model redesign; `Unbounded` cannot stand in because it means vector/lossless supply rather than an unresolved raster target.
