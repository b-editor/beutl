...
- **Execution**: `FilterEffectActivator.Apply` walks the items. Consecutive Skia items accumulate into `SKImageFilterBuilder` (zero allocations). The first custom item triggers `Flush()`: the accumulated chain is baked into a **freshly allocated** RGBA16F `RenderTarget` (`RenderTarget.Create` → `SKSurface`, Vulkan-texture-backed on the render thread), then the callback runs, typically allocating *another* target (`CustomFilterEffectContext.CreateTarget`) and snapshotting the current one. GPU sync (`_surface.Flush(true,true)` + `ITexture2D.PrepareForSampling/PrepareForRender`) happens inside `RenderTarget.PrepareForSampling()/BeginDraw()` — i.e. per materialization, uncoordinated.
...
- *Pool at the SkiaSharp `SKSurface` level only*: leaves Vulkan depth/ping-pong textures unpooled (PixelSort allocates them per pass today). Rejected — the spec names depth textures explicitly.
...
**Decision**: Six node primitives (the spec's five, with the spec's "shader" split into two flavors that share fusion semantics):
...
| `ComputeNode` | GLSL sources, pass count, ping-pong/depth declaration, push-constant layout | Never fused; scheduled pass(es) | Vulkan (Skia-composited fallback: none — requires `Supports3DRendering`, else the node's declared CPU/identity fallback applies, as today) |
...
- **→ `ColorFilterNode`**: `Saturate`, `HueRotate`, `Brightness`, `HighContrast`, `Lighting`, `LumaColor`, `BlendEffect`(color), all `ColorMatrix` primitives.
...
- **→ `SkiaFilterNode`**: `Blur`, `DropShadow(Only)`, `Dilate`, `Erode`, `MatrixConvolution`, `Transform` (matrix path), `InnerShadow` (recomposed from blur+composite primitives).
- **→ `ComputeNode`**: `PixelSortEffect` (3-shader multi-pass), `GLSLScriptEffect`.
- **→ `GeometryNode`**: `FlatShadow`, `StrokeEffect`, `Clipping`, `LayerEffect`, `DelayAnimationEffect`, `ShakeEffect`, `PathFollowEffect`, `DisplacementMapEffect` (mask composition part), `TransformEffect` (custom path), `CSharpScriptEffect` (compat session; fusion-ineligible, documented).
- **→ `SplitNode`/`CompositeNode`**: `SplitEffect`, `PartsSplitEffect` (split side), `InnerShadow`/`DropShadowOnly`-style composites where applicable.
- **Meta**: `FilterEffectGroup` concatenates children's descriptions into one graph (as today, one context); `NodeGraphFilterEffect` describes its evaluated chain the same way; `FallbackFilterEffect` describes an identity graph.
...
3. **Graph model + compiler + executor** land alongside the old path; `ShaderNode`/`ColorFilterNode` primitives; migrate the ~16 color effects; `FilterEffectRenderNode` routes graphs through the new executor; remaining effects still route via a bridge that wraps legacy items as opaque passes.
