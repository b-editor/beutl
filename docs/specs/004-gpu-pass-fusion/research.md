...
- **Meta**: `FilterEffectGroup` concatenates children's descriptions into one graph (as today, one context); `NodeGraphFilterEffect` describes its evaluated chain the same way; `FallbackFilterEffect` describes an identity graph; `FilterEffectPresenter` delegates — it describes whatever its target's current effect describes.
