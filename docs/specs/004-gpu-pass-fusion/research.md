...
`OpacityMask` accepts the existing `Brush.Resource` semantics rather than pretending the brush is a finite alpha bitmap. The recorder snapshots and lowers known brush kinds during `Process`, records DrawableBrush content as nested fragments, and uses the mask bounds as brush mapping geometry over the late-bound scope domain. It never starts `BrushConstructor`/a renderer during execution. The resolved-brush execution path supplies that recorded tile content to `BrushConstructor`; a legacy/raw direct call that supplies an unlowered `DrawableBrush.Resource` fails explicitly instead of launching a nested renderer or silently producing transparency. Guarded callback canvases reject every `SaveLayer`-backed opacity/blend/mask/paint API and hidden target allocation; retained raw hooks are the marked raw forms above.
...
Render-output cache identity includes subtree revision, the request's `FusionMode`, built-in scalar parameters, canonical Shader uniform values, complete deeply immutable state snapshots from reusable deferred-execution and custom metadata factories, declared resource key/versions, logical bounds, covered region, effective density, format, render intent/purpose where relevant, and device/context identity. Request-local execution-callback factories synthesize a unique identity for every recording and therefore disable cross-request pixel-cache reuse without risking stale pixels; metadata factories instead reject mutable/capturing state because their phases must share one snapshot. No author-supplied runtime key can omit either kind of state. A Shader subtree with a reusable custom binder additionally includes request `OutputScale` and `MaxWorkingScale`, because that binder may read both while the surrounding resolved density remains unchanged; unrelated cache candidates retain their existing cross-scale reuse. Nested requests inherit the enclosing `FusionMode`, so neither plan nor pixel payload reuse can cross a fusion-mode boundary. Structural plan and program identities deliberately exclude runtime-only values and resource contents.
...
**Decision**: `RenderRequestExecutor` owns all request-scoped planner-acquired intermediates, program leases, owned materialized inputs, deferred sessions, owned declared resources, cache-capture outputs, and synchronization transitions. It owns only the token/lease for an explicitly borrowed external resource and never disposes or permits pixel-affecting mutation of that raw value. Every deferred author resource uses `RenderResource<T>`: `Own<T>` requires a disposable reference type, while `Borrow<T>` accepts any reference type because no disposal transfers. A request-family reference table rejects duplicate `Own` and Own/Borrow conflict. Repeated Borrow with the same explicit key/version coalesces, an explicit mismatch is rejected, and a null key creates a distinct request-local identity that disables cross-request output-cache reuse. Keys are lightweight deeply immutable CPU identities. Public resource identity retains the authored key, while registration comparison and every Shader/output-cache consumer use a separately normalized complete-field key so incomplete author equality cannot coalesce distinct content. The token carries that internal resource cache identity, while reusable opaque, Geometry, target-scope, target-command, and painted descriptions derive scalar cache identity from their complete immutable callback state; request-local forms receive a fresh recording identity. The persistent `RenderNodeRenderer` owner retains structural/program caches and pooled targets across requests. The executor schedules exact-size RGBA16F leases by liveness, transparently initializes callback outputs, tracks lease generation, and discharges every acquire by pool return/disposal or successful cache-ownership transfer while preserving cleanup failures and the first primary failure.

`EngineObject.Resource` uses a separate two-phase terminal-disposal state machine and a runtime lifetime-claim graph. Every Resource owns an immutable internal node with no backreference; installing a child reserves one acyclic node edge, rejects any existing/same-owner second claim or borrowed-only node, waits for preexisting child work outside registry/resource locks, then commits or rolls back. Owner-authorized child Update/Dispose/generated mutation is the only accepted engine authoring while claimed; public `Update` is non-virtual and dispatches to protected `UpdateCore` after that validation. Another custom public lifecycle uses `ExecuteResourceOperation` and a protected virtual core, while `ExecuteOwnedResourceCallback` verifies one exact claimed edge and runs its synchronous callback without a registry/resource gate held. Public `ResourceReconciler` methods run only inside the derived Resource's protected reconciliation scope, which supplies the exact authorization and version coalescing; the old null-owner raw fallback is removed. The wait is restricted to preexisting work on the incoming child. Any operation that encounters an owner mutation already in progress fails immediately and commits nothing, so a child callback cannot wait for a parent that is itself waiting for that child. Detach releases the edge, failed cleanup retains it for retry, and finalizer abandonment orphans edges without virtual dispatch or hidden parent retention. The first containing `Dispose()` call marks the owner terminal, then invokes the most-derived explicit-only author `DisposeAuthorResources()` entry once and the generator-reserved child cleanup hook independently. This parameterless virtual replaces `Dispose(bool)` because finalization deliberately invokes neither hook; an author that formerly relied on `Dispose(false)` moves unmanaged ownership to `SafeHandle` or its own finalizer. Generated implementations capture earlier parameterless `PreDispose`/`PostDispose` failures in the base-owned author collector and continue through their base attempt; a conforming hand-written override calls the same reserved capture helper and provides finally-equivalent base-call discipline because the orchestrator cannot resume a skipped virtual base phase. Arbitrary author-owned `IDisposable` state is attempted once through the author collector, whereas explicit `EngineObject.Resource` fields/lists use the generated retry hook; the author helper rejects crossing that boundary. Weak references and types declaring `[BorrowedResourceReferences]` are exempt because they only read the resources they reference and never dispose them; `RenderNode` carries that declaration for its borrowed Resource/version snapshots. Generated custom cleanup is implemented through the generated private partial `DisposeGeneratedResourcesCore`; manual non-generated subclasses override `DisposeGeneratedResources`. Both capture custom failures through the reserved generated-cleanup helper and continue to slots/base. Generated object/list slots clear/release only after successful child disposal; failed slots remain claimed in deterministic order. Generated and hand-written owning list properties expose a read-only view and route add, dispose-remove, clear, detach-at, replace-at, move, and whole replacement through the same claim/owner gates. The flow-backed `DrawableGroup`, `DrawableDecorator`, `SoundGroup`, and `Scene3D` lists keep their reconciled borrowed prefix separate from the owned tail; `Brushes.Resource` singletons are borrowed-only. DelayAnimationEffect moves its render-time child cache into a private retryable aggregate with an unclaimed internal Resource owner, so a deferred callback never mutates the parent-claimed Delay Resource; the outer generated cleanup retains a failed aggregate for retry. GraphSnapshot gives every GraphNode Resource a private lifetime owner, uses exact-child callbacks for Bind/Initialize/Update/Uninitialize, then enters owner disposal for retryable child cleanup; an outer node-graph Resource retains a failed snapshot in its generated retry phase. Later `Dispose()` calls retry only the generated hook. This prevents a child failure from re-entering reached author cleanup while preserving retry for the only work the generator can prove idempotent. Cleanup aggregates are recursively flattened without changing author-first/generated-second or source declaration order.

Generated resource construction has a matching failure split. An attached factory finishes construction first, then aborts a failed initial `Update` through ordinary explicit disposal so every partially acquired author/generated owner is attempted and the initialization exception remains first. A detached constructor stages each lexical inheritance level inside its own `try`; its failure calls a non-virtual current-level cleanup method and then a base-qualified chain, never a most-derived hook whose fields have not run. `BESG007` moves mechanically visible disposable or nested-resource instance-initializer ownership into private `InitializeResourceConstructionStateCore`/`DisposeResourceConstructionStateCore` partials. Ordinary successful-lifetime cleanup still uses the one-shot author or retryable generated path according to ownership kind; a provably empty exact BCL list/array is the only owned-element-container initializer exception. The automatic detached-default path first constructs a short-lived owner that deliberately skips ordinary constructor bodies, so it has no cleanup phase. `BESG008` therefore rejects disposable, Resource, subscription, custom property implementation, populated/custom collection, or other mechanically visible ownership in that owner's instance initializers while allowing recognized engine property storage and provably empty exact BCL list/array containers. `[ResourceDefaultValuesConstructor]` marks one dedicated instance constructor that takes and directly forwards `ResourceDefaultValuesConstruction`; its restricted body initializes only stable property storage and never invokes an ordinary application constructor. Primary-constructor owners refactor to ordinary construction or manual Resource generation because C# cannot bypass their primary path. Opaque property-factory arguments remain an author no-throw/no-owned-state responsibility.

Direct detached authoring invalidates by construction: every generated value/`IsEnabled` setter call writes and increments `Resource.Version`, while object/list ownership helpers increment only on a mechanically proven mutation and still record partial success before throwing. The revision setter is private; derived code uses parameterless gated `MarkResourceChanged()` for one explicit authoring invalidation. The non-virtual public Update validates terminal/claim authorization and dispatches to protected `UpdateCore`. The generated attached-update path bypasses direct setters and threads one `ref updateOnly` token through private Pre/base/generated/Post phases; manual partials call the ref overload, and nested child changes propagate in `finally`, so the parent increments once even when a later child exception escapes. `GraphNode.Resource` keeps binding membership protected/internal rather than exposing mutable `IItemValue` elements through a nominally read-only collection; graph snapshots replace the internal arrays/maps, so external mutation cannot desynchronize generated fields from indexed execution state. A graph snapshot builds the next resource/item-value set outside the active arrays, commits it before `Initialize` so initialization-time queries see the next graph, and rolls back partial creation, binding, and initialization through attempt-all cleanup. `Uninitialize` runs only for resources whose initialization started, and only failed Resource cleanup remains pending for an explicit later retry.

Working scale for each ordinary materialization remains:

`min(max(OutputScale, densest concrete input supply), MaxWorkingScale)`

followed by the existing per-buffer 16,384-pixel dimension clamp against the complete conservative operation output bounds. The pure helpers move from `RenderNodeContext` to the independent public `RenderScaleUtilities` type because 3D, brushes, export policy, and planning use them outside a recording transaction; all callers migrate together with no forwarding shim. Concrete inputs can resolve supply during recording. Symbolic dependencies keep only an internal hint and make the public handle's metadata query fail for the remainder of that recording transaction; after those handles invalidate, graph-wide analysis establishes internal concrete metadata. `EffectiveScale.Unbounded` continues to mean vector/lossless supply, including a late-bound scope-density-preserving capture. `RenderScaleContract.MapInputSupply<TState>(state, Func<TState, EffectiveScale, EffectiveScale>, structuralKey)` is the declarative one-input resolved-supply transform used by Transform and DrawableGroup. It is restricted to element-wise one-input maps, may return `Unbounded`, and is reevaluated with the resolved input supply and the exact same immutable state after symbolic dependency resolution. A coverage-resolved unbounded input may remain unbounded through a CurrentPixel run until its eventual materialization. Public target capture has no value input supply. Its standard/custom target-capture policies resolve an output-derived concrete density and may intentionally downsample a denser owning target; `TargetCaptureScaleContract.PreserveTargetSupply` instead remains `Unbounded` through planning and materializes at the active root, finite Layer, or TargetLayerScope density. The built-in backdrop uses the same public policy. Later requested-region analysis crops the materialized logical region but does not increase or recompute density. Root target density remains the active destination density. Execution binds final cropped device bounds and other runtime values; recording does not bake them into structural Shader or Geometry descriptions.
