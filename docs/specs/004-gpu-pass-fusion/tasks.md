...
- [X] T125 Verified against HEAD. `docs/specs/004-gpu-pass-fusion/contracts/breaking-changes.md` now covers the final `Beutl.Engine` surface and the migrated `Beutl.Editor`, `Beutl.NodeGraph`, `Beutl.ProjectSystem`, `Beutl.AgentToolkit`, application, and downstream author call sites; every one of the forty-four `!` commits on the branch — not only `35e7f28b0` and `699332cc5` — retains its breaking Conventional Commit subject and a literal `BREAKING CHANGE:` footer. The SHAs quoted inside `contracts/breaking-changes.md` were corrected after this note was first written: all forty-four now resolve to commits that are ancestors of HEAD, and the list there is the current one.
