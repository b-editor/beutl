...
- [X] T073 [US1] Resolve request-wide root provenance, ordered publications, separate output/query metadata, and scope-token dependencies before planning in `src/Beutl.Engine/Graphics/Rendering/Planning/RenderRequestRecorder.cs` and `src/Beutl.Engine/Graphics/Rendering/Planning/RecordedRenderGraph.cs`
...
- [X] T087 [US4] Resolve render-cache candidates only after graph/region discovery, preserve provenance and token edges, select parent/child boundaries, and stage successful miss captures in `src/Beutl.Engine/Graphics/Rendering/Planning/RenderCacheResolver.cs`
