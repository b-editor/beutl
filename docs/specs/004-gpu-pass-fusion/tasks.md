...
- [X] T021 [P] [US3] Add a source census over compiled `src/**/*.cs` and `tests/**/*.cs` only that fixes the baseline at 29 production and 7 test `Process` overrides and rejects returning overrides, `RenderNodeOperation`, operation factories, `Pull`/`PullToRoot`, list rasterization, `SetOperations`, operation-backed `EffectTarget`, isolated nested processors, independent cache pulls, unclassified raw callbacks, and references to the four removed `RenderNodeContext` scale helpers while explicitly excluding the historical evidence patch in `docs/specs/004-gpu-pass-fusion/evidence/target-baseline-generator.patch`, in `tests/Beutl.UnitTests/Engine/Graphics/Rendering/Recording/RenderPipelineMigrationCensusTests.cs`
