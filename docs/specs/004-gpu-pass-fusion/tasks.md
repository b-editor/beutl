...
- [x] T035 [US2] Implement ProgramCache (stable source-hash buckets with exact ordered source/kind comparison, merged-source cache, LRU above cap) in src/Beutl.Engine/Graphics/Rendering/EffectGraph/ProgramCache.cs
...
**Independent Test**: a non-friend assembly compiles one representative authoring path per node kind using public API only; internal black-box execution tests independently verify correct pass shape/ROI and invariant-shader fusion between built-ins; the full 42-effect census (research §0) passes parity
...
- [x] T038 [P] [US4] Implement ComputeNodeDescriptor (GLSL pass set, structural pass count, ping-pong color declaration, push-constant writer, declared no-Vulkan fallback) in src/Beutl.Engine/Graphics/FilterEffects/Nodes/ComputeNodeDescriptor.cs
- [x] T039 [P] [US4] Implement SplitNodeDescriptor/CompositeNodeDescriptor and compiler/executor branch scheduling (fusion never crosses; per-branch resource intervals; exact static branch counts structural per C3.6; dynamic-outputs pass support per C3.5 with execution-time counts excluded from the key) in src/Beutl.Engine/Graphics/FilterEffects/Nodes/SplitCompositeDescriptors.cs and src/Beutl.Engine/Graphics/Rendering/EffectGraph/EffectGraphCompiler.cs
- [x] T040 [US4] Extend PlanExecutor with ComputePass (Vulkan pipeline via GLSLFilterPipeline, pooled ping-pong color textures, color-only fullscreen render passes, C4 sync at backend transitions) and GeometryPass session lifecycle in src/Beutl.Engine/Graphics/Rendering/EffectGraph/PlanExecutor.cs
...
- [x] T049 [P] [US4] Add plugin-style authoring coverage for the seven rendering descriptor kinds plus nested-graph/custom-render-node boundaries: a non-friend tests/Beutl.PublicApiContractTests project compiles the public-only surface in CI, while tests/Beutl.UnitTests/Engine/Graphics/Rendering/EffectPipeline/EffectAuthoringTests.cs uses internals to assert compiled pass shape, invariant-snippet fusion between two built-ins (SC-006), and the convolution ROI contract (A3)
- [x] T050 [P] [US4] Add FR-007 peak-intermediates test (10-effect chain peak-live ≤ same-shape 3-effect chain; dynamic-outputs passes counted and leak-checked but exempt from the static bound), SplitEffect threshold tests (static count changes recompile; sub-pixel/post-fan-out/>4096 stay dynamic; crossing static↔dynamic recompiles once), allocation-failure normalization tests (forced pool failure on fused/geometry/compute paths ⇒ preview drop, delivery throw — C7), and no-Vulkan fallback tests (Skia-only context executes fused plans; ComputeNode declared fallback applies — FR-014) in tests/Beutl.UnitTests/Engine/Graphics/Rendering/EffectPipeline/EffectPipelineCounterTests.cs and PrimitivePassTests.cs
