...
- assert the complete public `CanBeUsedAsValueInput` table: value maps remain eligible, Opacity preserves eligible pure children, Blend/public target scopes/commands—including `TargetLayerScope`—are ineligible, and finite Layer restores eligibility by local materialization;
...
- treat public `TargetCapture` as a concrete declared-density resampling boundary with two public policies: `TargetCaptureScaleContract.MaterializeAtWorkingScale` (the default) starts from output-derived density, and `TargetCaptureScaleContract.PreserveTargetSupply` keeps the enclosing scope's supply density so downstream Shader/replay work inside a denser `Layer` or `TargetLayerScope` does not lose density before it is used. The custom resolver receives empty `InputSupplies` and may use only `OutputBounds`, `OutputScale`, and `MaxWorkingScale`. Neither output-derived form inherits a denser enclosing target resolved later; the built-in backdrop path MUST use the public `PreserveTargetSupply` mode rather than a reserved internal path.
