...
- **C# script effects**: `CSharpScriptEffect` exposes an imperative surface to user scripts; it needs a compatibility node that materializes its inputs, with documented (unfused) cost. Legacy scripts targeting the removed surface fail at script compile time with a migration diagnostic (breaking).
