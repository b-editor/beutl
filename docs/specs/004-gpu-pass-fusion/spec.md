...
3. **Given** the existing in-app script effects, **When** a project using them is opened after the redesign, **Then** the SKSL and GLSL scripts render with output parity via native graph nodes, while a C# script written against the removed imperative surface fails at script compile time with a clear diagnostic that points to the migration guide (breaking, maintainer-approved); its `GeometrySession`-based rewrite renders via the compatibility geometry node.
