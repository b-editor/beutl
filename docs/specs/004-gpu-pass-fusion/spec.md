...
- A **render-target pool** for effect intermediates (including ping-pong color targets), replacing per-effect fresh allocation.
...
3. **Given** the existing in-app script effects, **When** a project using them is opened after the redesign, **Then** the SKSL and GLSL scripts render with output parity via native graph nodes, while a C# script written against the removed imperative surface fails at script compile time with a clear diagnostic that points to the migration guide (breaking, maintainer-approved); its `Builder`-based rewrite renders via the declarative graph, expressing any custom canvas drawing through `Builder.Geometry(session => ...)`.
...
- **Full-frame bounds**: some effects cannot safely narrow their input from an output ROI. The graph must represent them with valid forward/backward maps while conservatively disabling ROI cropping.
- **Fan-out/fan-in**: `SplitEffect` / `PartsSplitEffect` produce multiple targets that later composite. Fusion must not merge across split boundaries, and the resource plan must account for parallel branches. `SplitEffect` uses an exact static count only for a first-level grid whose logical tiles are at least 1×1 and whose total is ≤ 4096; those division changes are structural. Sub-pixel grids, a split after an existing fan-out, and grids above 4096 use the dynamic-outputs contract, so their runtime division counts are parameters rather than plan-key parts. `PartsSplitEffect` is always dynamic because contour tracing discovers its output count at execution time (FR-007).
...
- **Allocation failure**: the pool applies the normalized FR-015 semantics (preview drops the pass output and continues, delivery/export raises an explicit error) uniformly across pass kinds; the legacy per-path divergence is intentionally not reproduced.
...
- **C# script effects**: `CSharpScriptEffect` scripts author the declarative surface directly (`Builder` + `Progress`/`Duration`/`Time`; custom canvas drawing via `Builder.Geometry(session => ...)`); script-emitted invariant color nodes participate in fusion. A describe-time script exception rolls back the script's own appends and renders identity with a logged error. Legacy scripts targeting the removed surface fail at script compile time with a migration diagnostic (breaking).
...
- **FR-002**: The system MUST provide node primitives covering at least: pixel **shader** pass, **compute** pass, **geometry** (bounds/coordinate transformation, including Skia image-filter application), **split** (one input → multiple outputs), and **composite** (multiple inputs → one output). Plugin authors MUST be able to compose the public sealed descriptors, callbacks, nested graphs, and custom-render-node boundary without modifying the engine. Adding a new compiler discriminator is not part of this requirement; the descriptor union remains closed so every node has defined compiler, resource-plan, and executor semantics.
...
- **FR-006**: Intermediate render targets MUST be acquired from a pool and returned for reuse (including ping-pong color targets); per-frame steady-state rendering of a structurally constant scene MUST NOT allocate new targets after warm-up. Fullscreen GLSL passes MUST omit depth attachments when depth testing and writes are disabled.
- **FR-007**: The number of simultaneously live intermediate targets during a frame MUST be determined by the compiled resource plan (peak concurrency of the pass schedule), not by the number of effects in the chain. A pass whose output *count* is only resolvable at execution time (declared **dynamic outputs**, e.g. contour-based part splitting) allocates through the pool under executor ownership; it is exempt from the static bound but fully counted and leak-checked.
...
- **FR-009**: Graph structure and animated parameters MUST be separated: the compiled plan (pass schedule, shader programs, and the resource plan's *shape* — formats, lifetime intervals, peak concurrency) is keyed on structure, and per-frame parameter changes update only uniform/parameter blocks. Buffer sizes, ROIs, and the resolved working scale are re-resolved every frame from current bounds (a parameter-driven bounds change — e.g. an animated blur radius — changes sizes without recompiling the plan). Shader programs MUST be cached and reused across frames and across identical fusion groups.
...
- **FR-011**: Every node MUST declare its bounds contract: output bounds as a function of input bounds, and required input bounds as a function of the requested output region (ROI). The executor MUST render upstream passes only for the union of required regions, falling back to full bounds for nodes that declare full-frame bounds. Production render pulls carry the logical parent request through `RenderNodeProcessor` / `RenderNodeContext.RequestedBounds`; `PlanFilterEffectRenderNode` seeds resource resolution from it, and `Rect.Invalid` remains the explicit full-output request for callers with no narrower region.
...
- **FR-013**: All built-in `FilterEffect` subclasses (including the delegating `FilterEffectPresenter`), the in-app script effects (SKSL, GLSL, C# script), and `NodeGraphFilterEffect` MUST be migrated in this feature (`NodeGraphFilterEffect` migrates as a render-node boundary via the `RenderNodeFactory` seam — its inner effects flow through the declarative model individually). The C# script-facing surface is `Builder` (an `EffectGraphBuilder`) plus `Progress`/`Duration`/`Time`; `GeometrySession` appears only as the callback parameter inside `Builder.Geometry(session => ...)` for custom canvas drawing. A legacy script written against the removed imperative surface MUST fail at script compile time with a clear diagnostic naming `Builder` and referencing the migration guide — it never silently renders wrong output.
...
- **FR-015**: Pool exhaustion / allocation failure MUST behave uniformly: preview rendering degrades by dropping the affected pass output and continuing; delivery (export) rendering fails with an explicit error. This **normalizes** legacy behavior that was path-dependent (`FilterEffectActivator.Flush` dropped in preview / threw on delivery, but `CustomFilterEffectContext.CreateTarget` returned an *empty* target whose `Open` then threw unconditionally) — an accepted behavioral change documented in the breaking-changes contract, with tests covering each legacy path's replacement.
...
- **Execution Plan**: the compiled artifact — an ordered pass schedule, fused shader programs, and the resource plan's structural shape. Cached by structural key; concrete buffer sizes/ROIs are re-resolved each frame from current bounds.
...
- **Resource Plan / Target Pool**: the compiler's declaration of intermediate targets (size, format, lifetime) and the runtime pool that satisfies it with recycled RGBA16F color targets.
...
- **SC-006**: A plugin-style effect authored purely against the new public surface can express each node primitive and (when coordinate-invariant) participates in fusion. A non-friend test assembly that references only public API is the compile/accessibility gate; engine-internal execution-shape tests independently prove that the authored invariant snippet fuses between built-ins.
