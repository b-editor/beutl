...
- **FR-008**: `RenderNodeContext` MUST expose its ordered inputs as borrowed, read-only `RenderFragmentHandle` instances. A handle MUST expose non-executing bounds, effective scale, value cardinality, contribution state, value-input eligibility, and hit-test information but MUST NOT expose rendering, disposal, a backing target, or mutable ownership. The existing executable `RenderNodeOperation` public type MUST be removed in the same breaking change rather than retained as a second lifecycle or compatibility name.
...
- **FR-029**: Planned execution MUST preserve authored fragment/value order, painter order across top-level drawables, scope-local target-token/read dependencies, blend and premultiplied-alpha behavior, working color semantics, hit testing, output bounds, clipping, and source-to-output coordinate mapping. Forward metadata MUST retain separate `RootOutputExtent` and `QueryBounds`: the former unions contributing value bounds with potentially pixel-writing root target-effect regions after scope transforms/clips, while the latter unions contributing-value and target-command/scope query provenance for measurement and hit testing while excluding non-contributing read-only capture anchors or order-only access without query metadata.
- **FR-030**: Planned execution MUST preserve feature 003's `OutputScale`, per-fragment `EffectiveScale`, supply-driven working scale, `MaxWorkingScale`, scale-1 rounding behavior, and per-buffer dimension clamp for the same inputs. A custom scale resolver MUST return a finite value greater than zero; throws and invalid results MUST fail and roll back recording rather than being sanitized to a fallback. `Render(ImmediateCanvas)` MUST use the active destination density/transform/clip/state and prior pixels without clearing or flushing the borrowed target. A null `RequestedRegion` MUST select the resolved `RootOutputExtent` for backward root requirements, final commit, and rasterization domain; a non-null region MUST remain the explicit final root output requirement/commit crop. Neither case shrinks the available external-root or offscreen target domain, so target-read ROI may expand to that domain when required. `RenderNodeMeasurement` MUST report `OutputBounds` and `QueryBounds` separately, and hit testing MUST use query metadata rather than target-write extent.
...
- **FR-044**: Automated coverage MUST include public non-friend filter-effect and render-node authoring tests, a migration census for every production `Process` override and every old `CreateLambda`/raw-canvas hook, root/Layer/TargetLayerScope command order, non-empty and Empty TargetLayerScope execution, `Transform(+10) -> PushLayer(default) -> Full clear`, RootOutputExtent-versus-QueryBounds, empty/shifted rasterization results, SnapshotBackdrop sequences, recording-side-effect tests, cross-render-node fusion and barrier tests, parity at multiple scales and requested regions, cache and animation tests, fallback tests, and injected lifetime/failure tests, while retaining all applicable existing feature-003 and engine regression tests.
...
- **Render Fragment Handle**: A context-owned, non-executable `RenderFragmentHandle` reference to an ordered fragment stream with read-only bounds, effective scale, value cardinality/contribution, value-input eligibility, and hit-test behavior. It can be used only by its active recording/request owner and never grants rendering or disposal access to a node author.
...
- **Target Capture**: A scope-local token-to-value edge that snapshots the preceding target once and remains non-contributing until explicitly wrapped by `ContributeValues`.
