...
7. Update `Beutl.Engine.SourceGenerators` as a required part of the breaking contract: generate detached-resource default constructors and owning nested-resource replacement/disposal, emit `BESG003`–`BESG006`, migrate every affected owner, and add snapshot plus positive/negative diagnostic coverage in `tests/SourceGeneratorTest/EngineObjectResourceGeneratorTests.cs`. Generated owning lists expose a cached `IReadOnlyList` plus ownership-gated add, dispose-remove, clear, detach-at, replace-at, move, and whole-replacement methods instead of a mutable backing-list alias. Migrate the same public surface on the hand-written `DrawableGroup`, `DrawableDecorator`, `SoundGroup`, and `Scene3D` resource lists while preserving their borrowed flow prefix/owned tail split, and remove the accidental public mutable `DelayAnimationEffect.Resource.DelayedResources` cache surface. A source/public-API census rejects every remaining public mutable `List<EngineObject.Resource>` member. Resource disposal transitions terminal before callbacks, invokes the most-derived author virtual entry once, requires conforming hand-written overrides to preserve failures while attempting their base chain, and isolates retryable generated object/list cleanup in `DisposeGeneratedResources`; only failed slots remain for later retries. Characterize object/list, retained-view mutation, move/transfer, mixed borrowed/owned lists, inheritance, author-plus-child multi-failure order, reentrant/concurrent disposal, and post-attempt mutation rejection. Run the source-generator impact reviewer before the Phase B checkpoint; this work is not conditional on later implementation discovery.
...
- Phase B explicitly changes `Beutl.Engine.SourceGenerators`: implement detached-default construction, owning nested-resource setters/disposal, and diagnostics `BESG003`–`BESG006`; migrate affected owners, add generator snapshot/diagnostic NUnit coverage in `tests/SourceGeneratorTest`, run `beutl-source-generator-impact`, and require source-generator review before the public breaking commit.
