...
- Phase B explicitly changes `Beutl.Engine.SourceGenerators`: implement owning nested-resource setters/disposal; migrate affected owners, add generator snapshot NUnit coverage in `tests/SourceGeneratorTest`, run `beutl-source-generator-impact`, and require source-generator review before the public breaking commit.
