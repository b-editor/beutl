...
3. Compute each RGBA16F allocation as checked `width * height * 8` bytes, reject non-positive/overflowed footprints, and simulate lifetime intervals for the complete schedule. Fail planning when peak simultaneous target count or bytes exceed the request-family budget. Externally owned destination/presentation targets are diagnosed separately and do not consume planner-owned budget.
4. Recheck the same shared ledger transactionally at every actual acquire because dynamic opaque expansion can choose runtime sizes. Release/transfer decrements the live ledger exactly once. Preview may return the existing explicit drop outcome only at materializations already classified preview-drop-eligible; Delivery and all non-eligible acquisitions throw without publishing partial output.
