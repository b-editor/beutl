...
7. Update `Beutl.Engine.SourceGenerators` as a required part of the breaking contract: generate owning nested-resource assignment/disposal, migrate every affected owner, and add snapshot coverage in `tests/SourceGeneratorTest/EngineObjectResourceGeneratorTests.cs`. Resource ownership serializes on the per-resource gate through the base `SetOwnedResource`/`ExchangeOwnedResource`/`ReplaceOwnedResource` seam; generated resource classes expose plain owning properties and `Dispose(bool)` overrides, not a separate detach/replace helper surface. Resource lists remain `List<TResource>`, reconciled by `CompareAndUpdateList`; no read-only view, gated mutation methods, or `DelayAnimationEffect.Resource.DelayedResources` removal is made. Resource disposal invokes the most-derived author virtual entry once under the gate and publishes the terminal state after the override returns; a disposal failure leaves the old child owned with the replacement uninstalled. Characterize object/list replacement, inheritance, author-plus-child failure order, reentrant/concurrent disposal, and post-attempt mutation rejection. Run the source-generator impact reviewer before the Phase B checkpoint; this work is not conditional on later implementation discovery.
