...
│   ├── refresh-intentional-visual-baselines.sh # selective approved-divergence refresh
│   ├── run-paired-benchmarks.sh        # paired starting-SHA/feature benchmark runner
...
`DrawableGroup` records its ordered children inside `TargetLayerScope(..., Full)`. The group's filter and one invariant-opacity application wrap the isolated result, so opacity is composite-then-fade and nested group opacity multiplies. Child blends therefore see only preceding group content. A blend whose transparent source changes the destination (`Clear`, `Src`, `SrcIn`, `DstIn`, `SrcOut`, `DstOut`, `DstATop`, or `Modulate`) lowers as an ordered target command with Full access to the group scope; its source value retains child-bounds ROI. Other blends remain destination-dependent barriers but keep child-bounds target coverage. The scope is allocated on the resolved device-aligned owning-target domain, including at 100% opacity, so isolation does not depend on the legacy opacity SaveLayer shortcut or alter antialiased coverage through a clip-sized layer.
...
7. Update `Beutl.Engine.SourceGenerators` as a required part of the breaking contract: generate detached-resource default constructors and owning nested-resource replacement/disposal, emit `BESG003`–`BESG008`, migrate every affected owner, and add snapshot plus positive/negative diagnostic coverage in `tests/SourceGeneratorTest/EngineObjectResourceGeneratorTests.cs`. `BESG007` rejects disposable/`Resource` ownership in a partial Resource initializer through an immediate-evaluation semantic-operation walk and routes it through staged construction, construction-abort cleanup, and the correct normal one-shot-or-retry lifecycle. `BESG008` applies the same proof to the short-lived owner instance that supplies detached defaults; that path may create recognized declaration storage and provably empty exact BCL list/array containers but never an owned disposable, Resource, subscription, custom property, or fallible custom collection. Generated owning lists expose a cached `IReadOnlyList` plus ownership-gated add, dispose-remove, clear, detach-at, replace-at, move, and whole-replacement methods instead of a mutable backing-list alias. Migrate the same public surface on the hand-written `DrawableGroup`, `DrawableDecorator`, `SoundGroup`, and `Scene3D` resource lists while preserving their borrowed flow prefix/owned tail split, and remove the accidental public mutable `DelayAnimationEffect.Resource.DelayedResources` cache surface. A source/public-API census rejects every remaining public mutable `List<EngineObject.Resource>` member. Give every owned child one acyclic internal lifetime claim with two-phase pending reservation/quiescence/commit, no Resource backreference from the node graph, finalizer orphaning, owner-authorized child work, and borrowed-only static Brushes. Make public `Update` non-virtual and migrate overrides to protected `UpdateCore`; custom public lifecycles use `ExecuteResourceOperation` plus a protected virtual core, and exact claimed-child callbacks use `ExecuteOwnedResourceCallback`. Registry code never waits on a resource gate or invokes virtual code while locked. Resource disposal transitions terminal before callbacks, invokes the most-derived explicit-only author virtual entry once, and isolates retryable generated object/list cleanup in `DisposeGeneratedResources`; generated partial authors use `DisposeGeneratedResourcesCore`, while manual subclasses use the protected virtual hook and both route failures to the base-owned collector. Only failed claimed slots remain for later retries. Attached Update failures run ordinary cleanup with the primary first; detached constructor failures use a non-virtual current-to-base abort chain. Generated direct value setters and ownership operations update `Resource.Version` without author-managed increments, nested partial Update changes propagate before rethrow, and GraphNode binding membership becomes protected/internal. Characterize cross/same-owner alias and cycle rejection, two-phase rollback/quiescence, object/list retained-view mutation, move/transfer, mixed borrowed/owned lists, construction failure at base/derived levels, owner-default ownership rejection, version/cache invalidation including partial failure, one-shot author disposables, inheritance, recursively flat author-plus-child multi-failure order, reentrant/concurrent disposal, terminal/borrowed incoming rejection, and finalizer orphaning. Make graph-snapshot creation/binding/initialization transactional under a private Resource owner with exact-child authorization, attempt-all rollback, one-shot Uninitialize, and failed-Resource-only retry. Run the source-generator impact reviewer before the Phase B checkpoint; this work is not conditional on later implementation discovery.
...
### Phase C.5 - Bound request-wide allocation before execution

1. Add `RenderAllocationBudget` to `RenderNodeRenderRequest`, with a runtime `Default` of 1 GiB of simultaneously live planner-owned RGBA16F storage and 256 simultaneously live planner-owned targets. Its public constructor requires both limits explicitly; no optional numeric arguments or public compile-time default constants freeze policy into consumer binaries. Accept only 1 through 1 TiB live bytes and 1 through 65,536 live targets. Nested requests share the request-family ledger instead of receiving a fresh budget. A caller may select any value in those ranges but cannot disable checked arithmetic or dimension validation.
2. Resolve the physical dimension ceiling as the minimum of `RenderScaleUtilities.MaxBufferDimension` and the active backend/factory-reported maximum texture dimension. Validate every planned target against that ceiling before execution; unsupported or unknown backends use the portable 16,384-axis ceiling.
3. Compute each RGBA16F allocation as checked `width * height * 8` bytes, reject non-positive/overflowed footprints, and simulate lifetime intervals for the complete schedule. For Preview, tag only compiler-approved effect-class materializations optional and deterministically plan whole-fragment drops whenever an optional interval would exceed the limit or would leave a later mandatory interval unable to start; if mandatory intervals alone exceed the limit, fail planning. Delivery treats every interval as mandatory. Externally owned destination/presentation targets are diagnosed separately and do not consume planner-owned budget.
4. Pass the planned Preview drop set into execution so optional work is skipped before it acquires or consumes resources. Recheck the same shared ledger transactionally at every actual acquire because dynamic opaque expansion can choose runtime sizes. Release/transfer decrements the live ledger exactly once. Preview may return the existing explicit drop outcome only at materializations already classified preview-drop-eligible; Delivery and all non-eligible acquisitions throw without publishing partial output.
5. Add deterministic tests for constructor zero/negative/above-maximum rejection, runtime defaults, fan-out peak bytes/count, nested-request sharing, arithmetic overflow, backend dimensions smaller than 16,384, dynamic expansion, Preview degradation, Delivery failure, and ledger cleanup after primary and cleanup exceptions.
...
4. Schedule pooled RGBA16F intermediates by lifetime, prove the Phase C.5 peak byte/count budget and backend dimension ceiling before execution, recheck the shared request-family ledger at runtime, execute one request owner, release all resources on every path, and publish caches only after complete success.
...
1. Run public contract, raw-callback migration census, immutable metadata-state/capturing-callback rejection, forward/backward snapshot consistency, generated-resource one-shot/retry disposal, fragment/scope order, target capture/backdrop, transaction, ROI, scale, cache, animation, fallback, nested, 3D-boundary, failure, program-cache, pool, and diagnostics reconciliation suites.
...
- Phase B explicitly changes `Beutl.Engine.SourceGenerators`: implement transactional detached/default construction, owning nested-resource setters/disposal, and diagnostics `BESG003`–`BESG008`; migrate affected owners, add generator snapshot/diagnostic NUnit coverage in `tests/SourceGeneratorTest`, run `beutl-source-generator-impact`, and require source-generator review before the public breaking commit.
