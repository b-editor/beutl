...
1. Add `RenderAllocationBudget` to `RenderNodeRenderRequest`, with a production default of 1 GiB of simultaneously live planner-owned RGBA16F storage and 256 simultaneously live planner-owned targets. Nested requests share the request-family ledger instead of receiving a fresh budget. A caller may lower either limit but cannot disable checked arithmetic or dimension validation.
...
5. Add deterministic tests for fan-out peak bytes/count, nested-request sharing, arithmetic overflow, backend dimensions smaller than 16,384, dynamic expansion, Preview degradation, Delivery failure, and ledger cleanup after primary and cleanup exceptions.
