...
Before scheduling behavior changes, create a new target-specific golden/provenance category. It must contain:

- baseline code SHA and repository state;
- exact generator test or patch/script;
- scene name, dimensions, scale, requested region, seed, and parameter values;
- OS/architecture/backend/device/runtime versions;
- raw linear premultiplied RGBA16F blob hashes;
...
Tests use immutable `AssertExisting` behavior. Missing baseline files fail and are never generated by the implementation under test in CI.
...
The source/output format is lossless raw linear premultiplied RGBA16F. Metrics must not compare gamma-encoded screenshots.
...
```text
deterministic semitransparent source
...
- Public `TargetCapture -> Shader -> ContributeValues`: one target read/materialization, optional pure fan-out without a second capture, and one explicit contributing draw.
...
- strong CurrentPixel color chain over semitransparent content;
...
A migration census covers all 29 production and 7 test `Process` overrides from the baseline, every executable operation subclass/factory, every raw `ImmediateCanvas` author hook, and every `RenderNodeProcessor` pull/rasterize consumer. The gate fails if a returning override, executable `RenderNodeOperation`, `Pull`/`PullToRoot`, list rasterizer, `OperationWrapperRenderNode.SetOperations`, `EffectTarget.NodeOperation`, `EffectTarget(RenderNodeOperation)`, isolated nested processor, independent cache-generation pull, or unclassified `CreateLambda`/raw callback remains.
...
The hardware-required command runs on a capable GPU/Vulkan or configured software device. Ordinary fallback/public-contract tests must pass independently of that gate.
