...
`RenderScaleContract.PreserveInputSupply` copies the resolved supply for an element-wise one-input map or per-fragment replay scope. `RenderScaleContract.MapInputSupply(Func<EffectiveScale, EffectiveScale>, structuralKey)` instead applies a pure transform to the resolved supply of exactly one corresponding input; it may return `EffectiveScale.Unbounded`, and the mapping shape/key participates in structural identity. Transform and DrawableGroup use this contract. A symbolic input may produce an internal recording hint, but the map is evaluated again from the resolved supply during forward analysis.
