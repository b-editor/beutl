...
`RenderFragmentHandle.TryGetMetadata(out RenderFragmentMetadata)` and `TryHitTest(Point, out bool)` expose only concrete recording-time metadata. An `OwningTargetDomain` dependency remains symbolic through every ordinary descendant, including handles remapped through `RecordNode`/`RecordSubtree`, so those queries return false with default out values even when the internal record contains a finite hint. `RenderNodeContext.TryCalculateInputBounds(out Rect)` follows the same rule across all inputs and succeeds for an empty list with `default(Rect)`. Value cardinality, contribution, and value-input eligibility remain readable regardless of metadata availability.
...
If a `FilterEffectRenderNode` receives symbolic value-input metadata, the legacy `FilterEffectContext` no longer exposes `Bounds` publicly (engine-internal recording tracker only); bounds-dependent parameters must be resolved from the execution-time target bounds. Recording may use an internal finite input-bounds hint to retain the opaque legacy segment, but that hint is never exposed through the public handle. Bounds-transforming legacy items are retained in authored order and reevaluated from the resolved input bounds during graph-wide forward resolution. A value-input-ineligible stream is first isolated: owner-independent finite contribution/write metadata produces an ordinary finite Layer, while an unresolved Full/raw write uses the public `OwningTargetLayer` primitive and stays symbolic until scope lowering. Plugin nodes use the same primitive when they need this value-eligible symbolic isolation. This preserves filter/backdrop painter order without freezing the root target domain during child recording.

### Recording side-effect guard

Tests combine an exhaustive source census of every production `Process` override with runtime recording probes. The census rejects direct GPU-context, render-target, snapshot, media read/decode, nested-renderer, flush/synchronization, and readback work outside deferred callbacks. Runtime probes execute callback-backed public shapes through the metadata-only renderer with deferred-callback tripwires and a counting render-target/program/callback harness; every observed callback, allocation, GPU execution, synchronization, readback-capable opaque execution, and program creation remains zero. Debug builds may additionally maintain a request-local recording guard checked by engine allocation/execution entry points.
...
- separate output-cache identity for built-in values, Shader uniforms, state-derived or request-local callback state, and resources;
- node/root/cache provenance;
- request-owned resources.

The recorder evaluates each descriptor's pure forward bounds/cardinality/scale/hit-test declaration immediately when every required input is concrete. An owning-target-dependent fragment may carry internal provisional bounds, scale, and hit-test hints needed to continue recording, but it and every ordinary descendant remain publicly symbolic throughout recording; placeholders are not authoritative handle metadata. After public handles invalidate, graph-wide resolution establishes the internal concrete result. `RenderValueCardinality` counts values only and remains immediately available because fragment existence is separate. The initial compatibility migration classifies existing callbacks as guarded opaque values, typed target effects, or `LegacyRawCanvas` scope/command fragments. Every opaque form describes graph shape and metadata but forms an execution boundary.

### Scope-local target-token lowering

After the complete fragment graph is committed, lowering creates one initial target token for each externally owned root, each finite or engine-internal resolved owning-domain `Layer`, and each non-empty `TargetLayerScope`. Replaying a sequence consumes children in order. Clear, target command, capture, backdrop/snapshot, target-dependent operation, readback, and presentation consume the preceding token and produce the next inside that same scope. A capture also produces a reusable value edge but remains non-contributing until `ContributeValues`. A `TargetScope` decorates the child's replay without changing ownership of its token. A Layer consumes its own local child-token chain and yields one outer value. `TargetLayerScope` resolves its symbolic `TargetRegion` through every enclosing scope map. A non-empty result allocates a transparent offscreen isolation target, replays the mixed child sequence once, then composites the isolated result once onto the current target; the planner may elide that target only after proving exact equivalence. `Empty` remains an ordered effect fragment but creates no local target or pixel work. The scope has no public value input. Value inputs are explicit edges. These scoped chains are the canonical painter-order dependency and are discovered before cache selection; query bounds are metadata only and never authorize reordering or deletion.

Lowering preserves these invariants:

- Root sequence `[A, Clear, B]` draws `A`, clears that root, then draws `B`; `Layer([A, Clear, B], region)` performs the same sequence on a fresh local target and exposes only the resulting layer value outside. Every finite Layer publishes `EffectiveScale.Unbounded`: this advertises lossless replayability rather than a preselected working density. Lowering selects the Layer materialization density from downstream demand, child supplies, `OutputScale`, and `MaxWorkingScale` (`RenderNodeContext.Layer`, `RenderScaleUtilities.ResolveWorkingScale`), so a denser consumer can raise the Layer without changing its public supply contract. With concrete children, Layer content bounds union contributing child values with potentially pixel-writing target-effect regions after scope transforms and clip to the Layer domain, so a full clear yields a full-domain value even though its command query bounds are empty. Read/order-only accesses do not add content and hit testing remains query-metadata-driven. With any symbolic child, the Layer's author-readable bounds/hit testing are conservatively the complete finite domain and domain containment; its internal dependency on those children remains for final resolution and fan-out analysis.
...
Process fragments and embedded values topologically, resolve owning-target-dependent declarations from their internal hints and contracts, validate concrete declarations, and compute:
...
A selected miss inserts a capture point after the value's scheduled producer. It does not start another recorder/renderer. Captured resources remain request-owned and unpublished until the entire request succeeds. Failure, cancellation, invalid runtime output, or a cleanup fault observed before the atomic publication commit point prevents publication. Publication records the exact output identity, coverage, density, format, and device/context. Once every replacement is installed atomically, failure while disposing superseded cache storage is a secondary cleanup failure: the complete replacement set remains accepted and owned by the caches, while the request still reports the cleanup failure.
...
Render-output identity additionally includes every pixel-affecting built-in parameter, every canonical Shader uniform value, the engine-owned complete field-wise identity of state supplied to each reusable description or custom metadata factory, every description-declared `RenderResourceIdentity` key/version, source/subtree revision, output bounds, coverage, effective density, format, request policy where behavior differs, internal `FusionMode`, and device/context identity. Request-local execution-callback factories receive a fresh identity for every recording and therefore cannot cause unsafe cross-request pixel reuse. Custom bounds/scale/hit-test factories store one state for all metadata phases and require non-capturing callbacks; production does not recursively reject mutable state types, so callers must keep reusable state stable. No separate author-supplied runtime key can omit either kind of state. Custom Shader binders are request-unique by default; `ReuseFromSnapshot` derives their binding component from the copied canonical uniform value or resource key/version and accepts only non-capturing callbacks. Per cache candidate, a Shader subtree containing such a reusable callback also keys request `OutputScale` and `MaxWorkingScale` plus the resolved complete bounds and required region of each reusable stage and every fragment in its upstream input closure. Those values combine with the existing subtree density and device-grid identity to determine the callback-visible working scale, device bounds, logical origin, and grid offset, including materialized bounds passed through transparent value wrappers and requirement unions changed by graph fan-out; an external reusable sibling does not make a binder-free candidate region-sensitive. Nested requests inherit `FusionMode`; enabled and disabled schedules therefore cannot share an output-cache payload even when their other runtime inputs match. Resources owned without stable cache keys likewise get fresh request-local identities. Undeclared token use is rejected. Structural keys never substitute for output-cache state identity. Hashes select buckets; complete key equality decides reuse.
...
Final logical bounds, required region, effective supply, working density, clamped device size, frame parameter values, and resource providers bind after plan selection. A fused run first applies the runtime buffer clamp to its one adopted density. Every stage binder receives that actual run density and canonical device footprint; the first stage receives the materialized input's actual effective scale, and each later stage receives the run density as its input effective scale, while logical input/output/required bounds remain stage-local. This is the same metadata sequence exposed by disabled standalone runs even when the runtime clamp lowers the planned fragment density. Parameter-only changes cannot alter merged source or binding layout. A resource/provider failure is a render failure and publishes no partial output/cache.
...
`RenderScaleContract.PreserveInputSupply` copies the resolved supply for an element-wise one-input map or per-fragment replay scope. `RenderScaleContract.MapInputSupply<TState>(TState state, Func<TState, EffectiveScale, EffectiveScale>, structuralKey)` instead applies a pure transform and one stored author-stable state to the resolved supply of exactly one corresponding input; it may return `EffectiveScale.Unbounded`, and the callback shape/key participates in structural identity while complete field-wise state identity participates in runtime metadata/cache identity. Transform and DrawableGroup use this contract. A symbolic input may produce an internal recording hint, but the map is evaluated again from the resolved supply with the same state during forward analysis.

For every materializing value once complete concrete bounds and input supplies are available—during recording for a concrete chain or during forward resolution for a symbolic chain:
...
6. apply the per-buffer clamp against every complete physical footprint so each device axis is at most 16,384 pixels; immediately before a legacy Custom callback, remove any renderer-owned apron and materialize the historical dimension-sized local backing at the resulting density, preserving its local origin and direct final placement;
7. store the actual clamped density as downstream `EffectiveScale`; expose it to an active recording handle only when that handle's metadata is concrete.

Public `TargetCapture` has two explicit density families. `TargetCaptureScaleContract.MaterializeAtWorkingScale` and `Custom` resolve a concrete sampling density from `OutputScale` and declared bounds because no value input supplies density; `Custom` sees an empty `InputSupplies` list and may use only `OutputBounds`, `OutputScale`, and `MaxWorkingScale`. These modes are deliberate materialization/resampling boundaries and may downsample inside a denser finite Layer or TargetLayerScope. `TargetCaptureScaleContract.PreserveTargetSupply` instead remains `EffectiveScale.Unbounded` in the recorded graph and late-binds the resolved density of its owning root, finite Layer, or TargetLayerScope during execution. The built-in backdrop uses this same public mode. CurrentPixel Shader and other vector-preserving stages remain `Unbounded` until the run's eventual materialization, subject to the analytic/AA coverage boundary above. Backward ROI later chooses `Full`, `Empty`, or a finite cropped logical allocation. It never recomputes or raises a concrete declared density; a preserving capture uses the already allocated active target density. The root target remains at the destination density. `PixelRect.FromRect(croppedBounds, density)` remains the canonical logical-to-device rounding. Device/density-dependent Shader uniforms bind after ROI using that stable density and final device bounds.
...
1. **Same target / same request**: drawable-brush content, referenced-scene children, particle drawable sources, and NodeGraph output use `RecordNode`/`RecordSubtree` and remain in the same graph, transaction, cache policy, ROI, scale analysis, and failure owner.
2. **Separate target**: work such as a Scene3D drawable texture records a `NestedRenderRequest`. It has its own complete fragment/value graph, scoped target tokens, and island plan but inherits target-factory ownership, intent, purpose, scale limits, cache policy, and primary-failure owner. Its requested region is selected in child coordinates as described below.
...
`Scene3DRenderNode.Process` records scene resource/version, full declared bounds, scale behavior, and current-main failure policy through `OpaqueRenderDescription.CreateBackendBoundary(RenderBackendBoundary.Graphics3D, ...)`, published as `RenderNodeContext.OpaqueSource`. Execution later:
...
Bounds and hit-test requests use the same recorder and metadata analysis. They do not resolve pixel caches as execution substitutes, allocate targets, read media frames, execute deferred callbacks, increment frame render counts, or mutate persistent frame caches. They stop before pixel execution and create no completed-request telemetry object.
