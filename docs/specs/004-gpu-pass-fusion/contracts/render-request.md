...
Render-output identity additionally includes every pixel-affecting built-in parameter, every canonical Shader uniform value, every explicit description/binding `RenderRuntimeIdentity`, every description-declared `RenderResourceIdentity` key/version, source/subtree revision, output bounds, coverage, effective density, format, request policy where behavior differs, internal `FusionMode`, and device/context identity. Nested requests inherit `FusionMode`; enabled and disabled schedules therefore cannot share an output-cache payload even when their other runtime inputs match. Null runtime identities and resources owned without stable cache keys get fresh request-local identities and therefore cannot cause unsafe cross-request pixel reuse. Undeclared token use is rejected. Structural keys never substitute for runtime identities. Hashes select buckets; complete key equality decides reuse.
...
Public `TargetCapture` follows the same rule from `OutputScale` and its declared bounds because no value input supplies density. It is a deliberate sampling/materialization boundary, not a lossless or scope-density-preserving snapshot: a capture inside a denser finite Layer or TargetLayerScope may downsample into its declared concrete density. Its `Custom` resolver sees an empty `InputSupplies` list and may use only `OutputBounds`, `OutputScale`, and `MaxWorkingScale`; it never receives the enclosing target's resolved density. The engine-internal backdrop capture may late-bind the resolved density of its owning root, finite Layer, or TargetLayerScope target. While that owning domain is symbolic, its nested public handle returns false from `TryGetMetadata` and `TryHitTest`; a finite Layer may reestablish conservative concrete metadata without discarding the internal dependency. CurrentPixel Shader and other vector-preserving stages remain `Unbounded` until the run's eventual materialization, subject to the analytic/AA coverage boundary above. Backward ROI later chooses `Full`, `Empty`, or a finite cropped logical allocation. It never recomputes or raises the final resolved density; the crop uses that scale. The root target remains at the destination density. `PixelRect.FromRect(croppedBounds, density)` remains the canonical logical-to-device rounding. Device/density-dependent Shader uniforms bind after ROI using that stable density and final device bounds.
