...
`RenderScaleContract.PreserveInputSupply` copies the resolved supply for an element-wise one-input map or per-fragment replay scope. `RenderScaleContract.MapInputSupply<TState>(TState state, Func<TState, EffectiveScale, EffectiveScale>, structuralKey)` instead applies a pure transform and one deeply immutable stored state to the resolved supply of exactly one corresponding input; it may return `EffectiveScale.Unbounded`, and the callback shape/key participates in structural identity while state participates in runtime metadata/cache identity. Transform and DrawableGroup use this contract. A symbolic input may produce an internal recording hint, but the map is evaluated again from the resolved supply with the exact same state during forward analysis.
