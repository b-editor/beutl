...
6. apply the exact per-buffer clamp against every complete physical footprint so each device axis is at most 16,384 pixels; for legacy Custom work this includes a known retained backing repositioned at its transformed semantic origin, and runtime normalization repeats the clamp if opaque behavior exceeds the pure declaration;
