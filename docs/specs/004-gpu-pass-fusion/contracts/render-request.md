...
6. apply the exact per-buffer clamp against every complete physical footprint so each device axis is at most 16,384 pixels; immediately before a legacy Custom callback, force a canonical materialization of each semantic target bound at the resulting density instead of forwarding an input apron, and repeat the clamp during runtime normalization if later opaque behavior exceeds the pure declaration;
