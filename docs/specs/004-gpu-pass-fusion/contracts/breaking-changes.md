...
BREAKING CHANGE: Opaque, Geometry, target-scope, and target-command callbacks no longer address declared resources by integer position. Description `resources` arguments and `Resources` properties now use `RenderResourceBinding`; create one only with `resource.Bind("stable-name")` and replace `UseDeclaredResource<T>(index, use)` with `UseDeclaredResource<T>("stable-name", use)`. The binding constructor is internal. Binding names are unique ordinal strings and participate in structural/output-cache identity.

BREAKING CHANGE: State-passing callback factories no longer accept a separate author-supplied runtime identity. Reusable callbacks must be non-capturing, and the engine derives runtime/cache identity from complete field-wise equality/hashing of the supplied state without invoking author equality overrides. The engine does not recursively validate state types against an immutability allowlist; callers are responsible for keeping reusable state stable for the description/cache lifetime. Migrate state that cannot remain stable to `CreateRequestLocal`, whose capturing callback intentionally disables cross-request output-cache reuse. The public `RenderRuntimeIdentity` type and description `RuntimeIdentity` properties are removed.

BREAKING CHANGE: Custom metadata factories no longer accept capturing delegates. `RenderBoundsContract.Create`/`CreateFullInput`, `OpaqueRenderBoundsContract.Combine`/`FullInputs`, `RenderHitTestContract.Custom`, `RenderScaleContract.Custom`/`MapInputSupply`, and `TargetCaptureScaleContract.Custom` now use `Factory<TState>(TState state, Func<TState, ...> staticCallback, ...)`. Forward/backward pairs receive the same stored state, the engine derives runtime metadata/cache identity from its complete field graph, and callback method plus optional structural key remain structural identity. The callback must be non-capturing and the caller must keep state stable across every metadata phase; mutable state types are not synchronously rejected, and there is no request-local metadata-callback form.
...
BREAKING CHANGE: `IRenderTargetFactory` receives a `RenderTargetAllocationDescriptor` for each renderer-owned allocation. Custom factories must create a fresh compatible target for the exact descriptor, and any maximum-dimension query must be side-effect-free and consistent with `Create`. There is no public request-wide byte/target budget contract; allocation failure continues to follow `RenderIntent` at the actual materialization.
```

No `[Obsolete]` shim, returning overload, `V2` type, or executable compatibility wrapper remains after the same change.

Interim feature-branch revisions exposed four `RenderNodeContext.PaintedSource`/`PaintedSourceRequestLocal`
overloads, paint-handle/session types, custom-effect brush/pen registration and lowering APIs, and the protected
`EngineObject.Resource.SetOwnedResource` helper. None exists on `origin/main`, and none is part of the final feature
surface, so they are intentionally absent from the downstream migration footer above. Engine-owned sources use the
internal plain canvas/brush/pen callback and executor-owned `DrawableBrush` materializer; legacy custom effects use
ordinary `Brush.Resource?`/`Pen.Resource?`; generated nested-resource properties retain their ordinary direct setter
surface. No compatibility shim is retained for any interim API.
...
Use `RenderScaleContract.MapInputSupply<TState>(TState state, Func<TState, EffectiveScale, EffectiveScale> map, object? structuralKey)` for a pure density transform over an element-wise one-input map. Unlike `Custom`, it receives exactly the corresponding input's resolved supply and may return `EffectiveScale.Unbounded`. The non-capturing callback and optional immutable key identify the mapping shape; complete field-wise identity of the author-stable state is runtime metadata/cache identity. Transform and DrawableGroup use this contract, so a symbolic upstream supply is mapped again with the same stored state after graph-wide resolution rather than freezing a provisional recording value. It is rejected for source, capture, combine, and expansion topologies.
...
Generated EngineObject-valued resource properties retain their existing plain setter shape. Ordinary `Update`
reconciliation and containing-resource disposal keep their pre-feature behavior; feature 004 adds no setter-time
ownership transfer, replacement gate, terminal-disposal protocol, or new sharing/concurrency guarantee.
