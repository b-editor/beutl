...
| `ComputeNodeDescriptor.Create(..., bool requiresDepth, ...)` | `Create(..., int colorScratchCount, int depthScratchCount, ..., bool cpuFallbackRequiresReadback, ComputeDispatchFailureBehavior dispatchFailureBehavior = Throw)` | Declare the exact maximum concurrent scratch acquisitions. The executor rejects callbacks that exceed either count, CPU fallbacks that call `EffectInput.Snapshot()` declare readback explicitly, and ordinary dispatch failures propagate unless the effect explicitly selects `IdentityInPreview`. |
