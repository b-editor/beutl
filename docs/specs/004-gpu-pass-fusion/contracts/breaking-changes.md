...
| `ComputeNodeDescriptor.Create(..., bool requiresDepth, ...)` | `Create(..., int colorScratchCount, int depthScratchCount, ..., bool cpuFallbackRequiresReadback)` | Declare the exact maximum concurrent scratch acquisitions. The executor rejects callbacks that exceed either count, and CPU fallbacks that call `EffectInput.Snapshot()` declare readback explicitly. |
