...
BREAKING CHANGE: `SKSLShader.ApplyToNewTarget` and the disposable `SKSLShader.Effect` escape hatch are removed in favor of explicit legacy-custom allocation, input mapping, building, and rendering. Use `CustomFilterEffectContext.CreateTargetLike` for same-bounds output or `CreateTarget` for changed bounds, borrow a GPU-backed mapped input through `UseMappedInputShader`, configure and build through `SKSLShaderBuilder`, and finish with `SKSLShader.RenderToTarget` inside that scope. The caller owns shaders returned by `SKSLShaderBuilder.Build()`. Uniforms must use the allocated destination's actual `Scale` and backing dimensions.
