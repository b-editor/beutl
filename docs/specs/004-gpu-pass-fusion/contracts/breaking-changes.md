...
`RenderNodeContext.Inputs` is read-only. Use `TryCalculateInputBounds(out Rect)` instead of `CalculateBounds()`, `DisableRenderCache()` instead of assigning `IsRenderCacheEnabled = false`, `RenderScaleUtilities` for `SanitizeMaxWorkingScale` and `ResolveWorkingScale`, and `BufferDimensionBudget` for `EngineCeiling` and `ClampWorkingScale`.
