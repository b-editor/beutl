...
Materialized targets now expose immutable `DeviceBounds` and derived `RasterBounds`. `CustomFilterEffectContext.DeviceBufferBounds(bounds, w)` is the canonical allocation footprint, and `DeviceBufferSize` returns its size. This intentionally replaces the old independent width/height rounding: fractional logical origins may contribute an additional device pixel. Immediately before a legacy Custom callback, a forced compatibility Flush materializes each surviving target to that canonical footprint for its semantic `Bounds`; unchanged effects therefore do not receive a renderer-owned apron that disagrees with `CreateTarget(Bounds)`. Legacy effects must use the returned target `Scale`, `DeviceBounds`, or `RasterBounds` for buffer-coordinate math and keep semantic `Bounds` for effect bounds/placement metadata. A callback may subsequently move or shrink only `Bounds`; the retained physical backing remains visible to later execution and normalization.
