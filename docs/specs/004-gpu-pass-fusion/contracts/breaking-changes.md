...
Breaking changes use `refactor!:` / `feat!:` Conventional Commits with a `BREAKING CHANGE:` footer naming `Beutl.Engine` (`Beutl.Graphics.Effects`). Commit `95bc30cfa` was already published with the non-conforming `fix!:` type; its footer correctly records the compiler/pool removals, and the commit is retained rather than rewriting shared history. This correction and subsequent breaks use the required type. No `[Obsolete]` shims or v2 duplicates are retained. Legacy C# script identifiers such as `Context` and `Session` receive targeted Roslyn validation errors naming `Builder` and this guide without retaining public members on `CSharpScriptEffectGlobals`.
...
| Concrete `FilterEffectRenderNode` with an inherited default `Process(RenderNodeContext)` implementation | Abstract `FilterEffectRenderNode` with abstract `Process(RenderNodeContext)`; the default `Resource.RenderNodeFactory` creates an internal sealed plan-execution node | A custom `RenderNodeFactory` subclass MUST implement the complete `Process` operation construction itself. There is no callable `base.Process(context)` fallback, and a custom node does not inherit the default node's compiled-plan or prefix caches. Effects expressible as descriptors should keep the default factory. |
...
| `RenderTargetPool.Acquire(int, int, TextureFormat, ...)` | `Acquire(int, int, ...)` for the fixed RGBA16F Skia render-target format; `AcquireTexture(int, int, TextureFormat, ...)` for surface-less raw textures | The old overload keyed accounting as the requested format while always allocating an RGBA16F Skia surface. Choose the API by resource kind so the bucket identity, backing texture, and byte accounting agree. |
...
| `ComputeNodeDescriptor.Create(..., bool requiresDepth, ...)` and the interim `depthScratchCount` parameter | `Create(..., int colorScratchCount, ..., bool cpuFallbackRequiresReadback, ComputeDispatchFailureBehavior dispatchFailureBehavior = Throw)` | Declare the exact successful `Run` count and maximum concurrent color scratch acquisitions. Fullscreen GLSL dispatches disable depth testing and depth writes, so the final authoring contract has no depth declaration. |
...
| Implicit callback readback synchronization | `requiresReadback: true` on `GeometryNodeDescriptor` / `SplitNodeDescriptor`, or `cpuFallbackRequiresReadback: true` on compute | `EffectInput.Snapshot()` rejects undeclared readback. The executor owns and counts the declared synchronization; shader sampling and draw-only callbacks leave it false. |
...
| `FilterEffectRenderNodeFactory` as a defaultable `readonly struct`; `Of<TNode>(Func<FilterEffect.Resource,TNode>)` | sealed reference type; `Of<TResource,TNode>(Func<TResource,TNode>)` | Recompile plugins, retain one static factory per resource type, and remove runtime resource casts. The factory validates the resource before invoking plugin code and validates the exact returned node type. There is no invalid all-null `default` value. |
...
| Preview/delivery behavior inferred from `MaxWorkingScale` | Explicit `RenderIntent` propagated by `Renderer`, `SceneRenderer`, `RenderNodeProcessor`, `RenderNodeContext`, `ImmediateCanvas`, and `BrushConstructor` | Pass `RenderIntent.Preview` for interactive work and `RenderIntent.Delivery` for export/final output. Omitted intent preserves the prior finite-ceiling/`+Inf` inference for existing direct callers, but engine-owned render paths set intent explicitly. |
...
- **Custom render-node seam reshaped: `Resource.CreateRenderNode()` + `Resource.RenderNodeType` → one typed `Resource.RenderNodeFactory`.** The 003 escape hatch required a constructor and a separately overridden runtime type, which could drift. They are replaced by a sealed `FilterEffectRenderNodeFactory` capturing exact resource/node types and the constructor through `FilterEffectRenderNodeFactory.Of<MyResource, MyNode>(Func<MyResource, MyNode>)`. `Create(resource)` validates the resource before plugin code runs, then validates/disposes a mismatched result. **Migration:** retain a static factory and return it from the override. `Resource.Push` and embedded `EffectGraphBuilder.Effect` both use that captured value; the latter carries the same instance into the compiled pass, so a stateful virtual getter is never re-read at execution. `FilterEffectRenderNode` remains abstract and a custom node implements its complete `Process` path.
...
>
> `FilterEffectRenderNode` is now abstract and no longer provides the default plan pipeline through `base.Process(context)`. The default resource factory selects the engine's internal plan node; a custom `RenderNodeFactory` node must implement `Process` completely and owns any custom caching.
>
