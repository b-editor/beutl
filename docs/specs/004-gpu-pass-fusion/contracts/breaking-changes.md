...
BREAKING CHANGE: `EngineObject.Resource` assignment/disposal of nested `EngineObject`-typed properties now serializes on a per-resource ownership gate and rejects after the first disposal attempt. The generated setter and `Update` reconciliation both run through this gate; a nested resource is disposed before the replacement is committed, assigning the same instance is a no-op, and disposal of the containing resource disposes its current value. Three protected helpers are added for hand-written resource owners: `SetOwnedResource(ref location, value)` (dispose-before-install assignment), `ExchangeOwnedResource(ref location, value)` (transfer without disposal), and `ReplaceOwnedResource(ref location, replacement)` (non-null atomic exchange returning the old value). Callers must not retain or share a resource after a successful assignment into an owning slot. For nullable owning properties, generation additionally exposes `Detach<Property>()` (transfer without disposal); for non-null owning properties it exposes `Replace<Property>(replacement)` (atomic exchange).

BREAKING CHANGE: `EngineObject.Resource` disposal now serializes on an ownership gate and publishes a terminal state: the first `Dispose()` attempt invokes the most-derived author `Dispose(bool)` entry once under the gate and then marks the resource disposed; every update, setter, or ownership helper rejects after that. The finalizer still invokes the `Dispose(bool)` chain (`DisposeCore(false)`), so an out-of-tree resource that releases unmanaged state from `Dispose(false)` continues to work; explicit `Dispose()` is the only path that calls `GC.SuppressFinalize`. Generated `Dispose(bool)` overrides invoke `PreDispose`/`PostDispose` partial hooks and call `base.Dispose(disposing)`. Because the terminal state is published after the override returns, an exception thrown by author cleanup still completes the transition.

The mutable generated resource-list properties remain `List<TResource>`; this change does not introduce a read-only view, `AddToItems`/`RemoveFromItems`/`ClearItems`/`DetachItemsAt`/`ReplaceItemsAt`/`MoveItems` mutation methods, or ownership-gated list helpers. `CompareAndUpdateList` reconciles the list from the engine object's `IListProperty` and disposes replaced and trimmed items; generated `Dispose(bool)` disposes every current item. The flow-backed list properties (`DrawableGroup.Resource.Children`, `DrawableDecorator.Resource.Children`, `SoundGroup.Resource.Children`, `Scene3D.Resource.Lights`/`Objects`) keep their public `List<TResource>` shape unchanged. `DelayAnimationEffect.Resource.DelayedResources` remains a public list; no removal is made in this change.
