...
| `ComputeNodeDescriptor.Create(..., bool requiresDepth, ...)` | `Create(..., int colorScratchCount, int depthScratchCount, ..., bool cpuFallbackRequiresReadback, ComputeDispatchFailureBehavior dispatchFailureBehavior = Throw)` | Declare the exact successful `Run` count and maximum concurrent scratch acquisitions. A callback may use one exclusive terminal `CopySourceToDestination()` instead of dispatching. The executor rejects under/over-dispatch, mixed copy/dispatch, or scratch overrun; CPU fallbacks that call `EffectInput.Snapshot()` declare readback explicitly, and ordinary dispatch failures propagate unless the effect explicitly selects `IdentityInPreview`. |
| `ComputeNodeDescriptor.RequiresDepth` | `DepthScratchCount` (with `ColorScratchCount` for color ping-pong resources) | This is a binary/source break: recompile plugins and declare the exact maximum concurrent scratch counts instead of a depth yes/no hint. |
...
- **Custom render-node seam reshaped: `Resource.CreateRenderNode()` + `Resource.RenderNodeType` → one `Resource.RenderNodeFactory`.** The 003 escape hatch (override to supply a custom `FilterEffectRenderNode` with a non-supply working scale) was a virtual `FilterEffectRenderNode CreateRenderNode()`; keeping the render-graph diff's reuse check alive across re-renders then required a *separately overridden* `virtual Type RenderNodeType`, and overriding one but not the other silently recompiled the plan every frame. Both are replaced by a single `virtual FilterEffectRenderNodeFactory RenderNodeFactory` — a `FilterEffectRenderNodeFactory` value that captures the node type (`typeof(TNode)`) and its constructor together via `FilterEffectRenderNodeFactory.Of<TNode>(Func<Resource, TNode>)`. `Create(resource)` validates that the returned instance's exact runtime type equals `NodeType`, disposes a mismatch, and throws, so even a deliberately broad `TNode` cannot make the declaration drift from the constructor. **Migration:** `public override FilterEffectRenderNode CreateRenderNode() => new MyNode(this);` (with any paired `RenderNodeType`) becomes `public override FilterEffectRenderNodeFactory RenderNodeFactory => FilterEffectRenderNodeFactory.Of(static r => new MyNode(r));`. `Resource.Push` stays overridable; a direct caller of `CreateRenderNode()` uses `resource.RenderNodeFactory.Create(resource)`. `FilterEffectRenderNode` itself is now abstract and its `Process` override is abstract: a custom node implements the full operation path and cannot call `base.Process(context)`. The engine's compiled-plan/prefix-cache implementation lives only in the internal default node. This seam is new-on-this-branch relative to a release only in that `RenderNodeType` was added and removed here; `CreateRenderNode()` and the concrete base `Process` shipped in 003, so removing them is a real break.
