...
Which of the two addressing modes a session exposes is decided by the channels that can reach it, not by convenience. A callback holds a token only by capturing it, and capture is permitted exactly on the channels whose runtime identity does not come from the callback's state: `CreateRequestLocal`, and the declared-identity form where the author supplies a `RenderRuntimeIdentity` separately from what the callback captures. So `OpaqueRenderSession`, `GeometrySession`, `TargetScopeSession`, and `TargetCommandSession` expose both `UseResource(token, use)` and `UseDeclaredResource(index, use)`; `RawTargetScopeSession` and `RawTargetCommandSession` expose only the token form; and `PaintedRenderSession` exposes only the positional form, because `PaintedSource` is state-passing only — its `draw` may not capture, and its `state` is the produced value's runtime identity, which rejects a `RenderResource` element. Adding a token form to `PaintedRenderSession` would add a member no caller could reach with a valid token, so the asymmetry is the invariant rather than a gap. `DeclaredResourceAddressingContractTests` pins both rejections and the per-session surface. When `structuralKey` is null, the description uses the execution callback's method identity plus operation kind. `RenderScaleContract.Custom`, custom bounds, and custom hit-test contracts likewise default to their delegate method identities. A captured choice that changes operation/binding/topology shape belongs in an explicit equality-stable structural key. Pixel-affecting data is not declared separately: the channel the API provides for it is `state`, which the description publishes as its `RuntimeIdentity`, subject to the six open channels above.
