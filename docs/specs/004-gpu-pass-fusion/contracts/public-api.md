...
An opaque, Geometry, TargetScope, or TargetCommand callback that closes over mutable fields, services, or other captured state MUST NOT pair that closure with a reusable (non-null, equality-stable) `RenderRuntimeIdentity`: the recorder cannot observe state changes after recording, so pixels could be published under an identity describing earlier state and reused incorrectly across requests. Such a callback either (a) passes a null identity so every recording is request-local and never cache-reused, or (b) binds a copied immutable state value to a static callback and keys the identity by that value (the same snapshot rule `ShaderBindingCachePolicy` applies to binders). The recorder MAY mechanically reject a reusable identity whose callback cannot be proven non-capturing.
