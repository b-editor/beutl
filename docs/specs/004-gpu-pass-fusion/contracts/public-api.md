...
`CompositionContext.TargetDomain` carries the finite scene-frame domain into auxiliary composition consumers. `GraphSnapshot` copies and refreshes it on every evaluation, and standalone `PreviewNode`/`MeasureNode` pass it to `RenderNodeRendererOptions.DefaultRequest.TargetDomain`. A graph snapshot builds its next resource/item-value set transactionally, commits that set before `Initialize` so initialization-time graph queries observe it, and rolls back partial creation/binding/initialization with attempt-all cleanup. It calls `Uninitialize` only after initialization started, retains only failed `EngineObject.Resource` cleanup for an explicit retry, and always clears abandoned arrays/maps so a failed build cannot expose a partially committed binding graph. `RenderNodeContext.TargetDomain` exposes the same request value to nested semantic consumers without resolving an owning-target-dependent handle early.

```csharp
namespace Beutl.Engine;

[AttributeUsage(AttributeTargets.Constructor, AllowMultiple = false, Inherited = false)]
public sealed class ResourceDefaultValuesConstructorAttribute : Attribute;
...
public class EngineObject
{
    ...
    public class Resource : IDisposable
    {
        ...
        protected void MarkResourceBorrowedOnly();

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void SetResourceValue<TValue>(
            ref TValue location,
            TValue value);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void SetResourceOperationValue<TValue>(
            ref TValue location,
            TValue value);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void ClearResourceOperationValueForCleanup<TValue>(
            ref TValue location);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void MarkResourceChanged();

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void MarkResourceChanged(ref bool updateOnly);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void ExecuteResourceOperation(Action callback);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void ExecuteOwnedResourceCallback(
            Resource resource,
            Action callback);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void CompleteResourceConstruction(Type currentResourceType);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void ClaimOwnedResourceForConstruction(Resource resource);

        protected void ExecuteOwnedResourceReconciliation(
            Action action,
            ref bool changed,
            ref bool updateOnly);

        protected void ExecuteOwnedResourceListReconciliation<TResource>(
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void SetCreatedOwnedResource<TResource>(
            ref TResource? location,
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void DisposeAuthorOwnedDisposable<TDisposable>(
            ref TDisposable? location)
            where TDisposable : class, IDisposable;

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void CaptureGeneratedResourceCleanupFailure(Exception failure);

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected virtual void DisposeGeneratedResources();

        [EditorBrowsable(EditorBrowsableState.Never)]
        protected void DisposeGeneratedResourcesAfterConstructionFailure();

        [EditorBrowsable(EditorBrowsableState.Never)]
        [DoesNotReturn]
        protected void AbortResourceInitialization(Exception primary);

        [EditorBrowsable(EditorBrowsableState.Never)]
        [DoesNotReturn]
        protected void AbortResourceConstruction(Exception primary, Action cleanup);
        ...
        protected virtual void DisposeAuthorResources();
        public void Dispose();
    }
}
```
...
`ResourceDefaultValuesConstruction` and the matching protected constructor are a generator-only extension contract. When a type has no explicit defaults constructor, generated constructors use this path to execute the author's declaration and instance initializers without running ordinary constructor bodies. The resulting object is only a defaults source, not an application object; plugin code must not call the marker constructor directly. Its instance initializers may create recognized non-disposable engine `IProperty`/`IListProperty` declaration storage and provably empty exact BCL `List<T>`/array containers, but they must otherwise be no-throw and ownership-free. In particular they cannot acquire an `IDisposable`, `EngineObject.Resource`, subscription, custom property implementation, or populated/custom collection because the short-lived defaults owner has no application lifetime or cleanup phase. `BESG008` rejects mechanically visible ownership on this automatic marker path. A type that needs owned or fallible ordinary-instance state initializes it in every ordinary constructor while leaving the `ResourceDefaultValuesConstruction` path ownership-free. A valid `[ResourceDefaultValuesConstructor]` marks one dedicated instance constructor that takes the marker and passes it directly to `base(construction)`; it bypasses ordinary application constructors rather than returning an ordinary owner from a factory.
...
`BESG003` rejects every generated value/object `IProperty` outside those shapes, including a declaration-initialized property or backing field that an ordinary constructor replaces. `BESG004` rejects a primary-constructor owner because every secondary constructor would execute the primary path before its body; migrate it to an ordinary constructor or suppress generation and implement `Resource`/`ToResource` manually. An ordinary-constructor owner can preserve generated detached defaults without hand-writing that complete contract by declaring exactly one defaults constructor:

```csharp
public partial class PluginEffect : EngineObject
{
    public PluginEffect(float amount)
    {
        Amount = Property.CreateAnimatable(amount);
    }

    [ResourceDefaultValuesConstructor]
    private PluginEffect(ResourceDefaultValuesConstruction construction)
        : base(construction)
    {
        Amount = Property.CreateAnimatable(100f);
    }
...
The marked constructor may be non-public, but it must be the only attributed constructor, take exactly one `ResourceDefaultValuesConstruction`, and pass that exact marker directly through the base-constructor chain. Its body may only assign stable generated property storage from recognized non-disposable `Property` factories; `BESG005` rejects any other signature, chain, statement, target, or factory shape. Factory argument expressions still obey the author no-throw/no-owned-state rule when an opaque method call prevents mechanical proof. The direct concrete generated `Resource()` invokes the most-derived marked constructor exactly once. An abstract owner may declare a protected marked constructor so derived providers can initialize its storage, but is never instantiated as a defaults source. If any base owner declares one, each generated derived owner therefore declares its own marked constructor and passes the same marker to base. `BESG006` prevents the derived type from falling back to the initializer-only chain and bypassing that explicit construction contract. Provider constructors are not inherited.

The attached `ToResource` path uses a separate internal construction path. It invokes neither the marker defaults source nor an explicit defaults constructor, so it does not evaluate or allocate detached-only defaults that its first update would overwrite. Generated abstract `Resource` types no longer expose a protected parameterless constructor: a hand-written or generation-suppressed attached resource explicitly chains to `base(skipDefaultInitialization: true)`, while a detached resource that promises declared-default parity explicitly chains to `base(defaultValues)` with an ownership-free defaults owner. This makes an omitted default-initialization decision a compile error instead of silently preserving `default(T)`. Implementing the complete `Resource`/`ToResource` contract manually remains the escape hatch when neither automatic declaration-time storage nor a defaults constructor represents the intended contract.

Every attached factory completes object construction before it calls `Update`. If that first update throws, the factory calls `AbortResourceInitialization(primary)`: the resource enters the ordinary explicit-disposal lifecycle, author cleanup and generated child cleanup are both attempted, the primary exception remains first and opaque, and cleanup aggregates are recursively flattened in their original order after it. No partially initialized attached resource is returned.

Detached construction is staged separately at every generated inheritance level. The base constructor begins in an unpublished constructing state that cannot be claimed, updated, disposed, or mutated through a leaked reference. Inside that level's constructor-body `try`, the generator creates its empty list backing, invokes private `partial void InitializeResourceConstructionStateCore()`, then copies value defaults and materializes object defaults. Its catch passes the lexically bound, non-virtual current-level cleanup method to `AbortResourceConstruction`; private `partial void DisposeResourceConstructionStateCore()`, current-level generated slots in source declaration order, and a `base.`-qualified non-virtual cleanup chain are attempted. On success, `CompleteResourceConstruction(typeof(CurrentResource))` moves to published-ready only when that lexical generated type is the runtime-most-derived Resource; a generated base leaves the one-way constructing state for its derived constructor, and there is no protected reopen seam after Ready. A base-constructor failure therefore cannot dispatch into zero-initialized derived fields. The primary remains first, followed by recursively flattened construction-cleanup failures. These construction-only seams never invoke author `DisposeAuthorResources()` and do not create retry semantics.

`BESG007` rejects a generated partial `Resource` field or auto-property initializer that immediately acquires `IDisposable` or `EngineObject.Resource` state. Its semantic-operation walk follows every immediately evaluated receiver, argument, array-size/indexer/interpolation/query/conditional/collection/object/member branch and a directly invoked anonymous function; it does not inspect a deferred lambda/local-function body or guess whether an opaque API invokes a callback. The narrow exception is a provably empty exact BCL `List<T>` or array initializer, which owns no element. Custom collection builders, non-empty collections, and disposable containers are rejected. An author moves such acquisition to `InitializeResourceConstructionStateCore`, provides partial-initialization-safe construction cleanup in `DisposeResourceConstructionStateCore`, and also supplies its normal lifecycle cleanup: arbitrary non-`Resource` disposables are attempted once from parameterless `PostDispose` through `DisposeAuthorOwnedDisposable`, while owned `Resource` values/lists use `DisposeGeneratedResource`/`DisposeGeneratedResourceList` from `DisposeGeneratedResourcesCore`. A construction-state cleanup core treats every field as potentially uninitialized, attempts each initialized field independently, passes each exception to `CaptureGeneratedResourceCleanupFailure`, and does not let one failure skip later fields. Other instance initializers remain a no-throw/no-owned-state contract; an opaque factory/callback whose execution cannot be proven mechanically remains the author's responsibility.
...
A generated property backed by an `IProperty<T>` whose `T` is an `EngineObject` type, nullable or non-null, is an owning resource slot rather than a borrowed reference. A non-null declared object default is materialized through `ToResource(CompositionContext.Default)`. Assigning a different resource first disposes the previous value and transfers ownership to the containing resource only after that cleanup succeeds; assigning the same instance is a no-op. Every install operation validates and exclusively claims the incoming resource before mutating current ownership. An already-terminal, borrowed-only, cyclic, pending, or already-claimed replacement is rejected, including a second slot on the same owner; the current slot, incoming ownership, and `Version` remain unchanged. A failed-cleanup terminal child remains claimed only so the generated cleanup hook can retry its cleanup; it cannot be installed as live authoring state elsewhere. Disposing the containing resource disposes the currently held value through the generator-reserved retry hook.

Each `Resource` owns one internal lifetime node that never references the `Resource` object back. Claims form an acyclic owner-node graph under a short-lived registry lock. Installation reserves a claim, releases all registry/resource locks before waiting for preexisting child work or invoking virtual code, then reacquires the owner gate only to commit or roll back. Whole-list replacement reserves every incoming node and waits in stable node order. Every failure releases pending claims that did not commit, while a committed failed-cleanup child stays claimed. Finalizer abandonment detaches node edges without virtual cleanup, so an externally retained child cannot keep its unreachable parent/native subtree alive through the registry.

The public `Update` entry is non-virtual. It validates terminal/claim state and dispatches to protected `UpdateCore`; generated and hand-written subclasses override only the core. A Resource subclass that defines another public lifecycle entry implements the same template with protected `ExecuteResourceOperation(callback)`: the public entry remains non-virtual, and its protected virtual core runs only after terminal/claim validation. An owner invokes another synchronous lifecycle callback on one exact claimed child through `ExecuteOwnedResourceCallback(resource, callback)`, which verifies the owner-child edge, establishes only that authorization, and holds neither the registry nor either Resource gate while user code runs. Owner reconciliation uses the same exact authorization internally. A retained external reference cannot call `Update`, `Dispose`, direct value/list authoring helpers, or another ownership transition until `Detach<Property>`/`Detach<Property>At` releases the claim. Arbitrary plugin methods outside these engine authoring seams remain ordinary C# code and are not presented as thread-safe or claim-aware.

For a nullable owning property `Child`, generation also exposes `public Child.Resource? DetachChild()`. It atomically clears that slot, releases the lifetime claim without disposing its previous value, and returns the detached resource, or null when the slot is empty. The caller owns a non-null return until it assigns the value into another owning slot or disposes it. `destination.Child = source.DetachChild()` is the canonical nullable-slot transfer.
...
The getter returns a cached read-only view, so a retained reference cannot call a working `Add`, `Remove`, `Clear`, or index setter behind the owner's gate. The setter snapshots and validates its complete replacement before cleanup. The exact same ordered reference sequence is a no-op; any other replacement rejects null entries, duplicate reference identities, an already-terminal/borrowed-only resource, or any resource claimed by this or another owner. Validation and pending claim reservation complete before old cleanup; a rejected/failed transition rolls back every uncommitted reservation and leaves caller ownership explicit. It disposes every old item; successful items are removed and their claims released, failed items remain claimed in stable order, and the validated replacement stays caller-owned unless all old cleanup succeeds and the new list commits atomically. `AddToItems` transfers one active unclaimed resource. `RemoveFromItems` performs owner-authorized disposal before removing/releasing a matching item and returns false when absent. `ClearItems` attempts every item and retains only claimed failures. `DetachItemsAt` removes and releases the claim without disposal, transferring the returned item to the caller. `ReplaceItemsAt` reserves an active unclaimed replacement, atomically installs it, releases the old claim, and transfers the old item to the caller. `MoveItems` reorders one existing item under the same gate without disposing or changing ownership; callers use it instead of passing current entries back through the whole-list setter. Every mutation/transfer linearizes with generated update and disposal and throws `ObjectDisposedException` after the first disposal attempt. The read-only view may reflect cleanup of already-owned items, but cannot introduce a new retry candidate; later disposal retries exactly the items whose earlier cleanup failed. Cross-owner and same-owner/different-slot alias attempts are rejected mechanically rather than relying on caller discipline.
...
`DelayAnimationEffect.Resource.DelayedResources` is not an authoring list at all, and its public mutable member is removed. Deferred rendering cannot mutate the claimed Delay Resource without its parent authorization, so the built-in path moves mutable state into a private retryable `DelayedResourceCache`. That aggregate owns an unclaimed private Resource owner, which claims, updates, trims, and retries cleanup of delayed child resources under its own gate; the deferred callback captures the aggregate rather than the claimed Delay Resource. The outer Delay Resource clears the aggregate only after its generated cleanup succeeds and retains it after failure for the next `Dispose()` retry. No internal list or live cache view becomes an authoring surface.
...
`SetCreatedOwnedResource` is the generated/manual counterpart for a replacement the owner just created and still owns locally. It preserves `SetOwnedResource`'s validation and old-slot commit rules, but if claim preparation or old-child cleanup rejects the transition it also disposes the uninstalled replacement. The transition primary remains first and any replacement-cleanup aggregate is recursively flattened after it. Image, Video, Geometry, Text, and Filter graph nodes use this seam when rebuilding their private child resource; a caller-supplied assignment continues to use `SetOwnedResource` and retains ownership after failure.

`Resource.Version` is the invalidation signal for direct detached authoring as well as attached updates. Every direct generated value-property assignment and direct `IsEnabled` assignment writes the supplied value and increments `Version` exactly once, even when author equality or reference identity says the value is the same; reassigning a mutable reference is an explicit invalidation signal. Generated code uses `SetResourceValue` for that rule. Constructors and attached `Update` write backing fields and aggregate their detected changes into one version increment, so they do not double count. Generated private `PreUpdate`/`PostUpdate` partial signatures receive the same `ref bool updateOnly` as `Update`; custom partial code calls `MarkResourceChanged(ref updateOnly)` for each committed manual change, which increments only the first time. Successful object/list assignment, detach, replace, add, remove, clear, or move increments once when it changes observable membership/order; same-resource assignment, exact same ordered list replacement, and a same-index move are true no-ops. A partial list cleanup that removes at least one successfully disposed item increments before rethrowing later cleanup failures. Nested object/list `Update` propagates a child's version change through `finally`, so a later child failure cannot hide an already committed change from the parent cache identity.

`Resource.Version` is publicly readable with a private setter. Derived/manual resources invalidate an explicit detached-authoring change through parameterless `MarkResourceChanged()`, whose operation/terminal/claim gate increments once per call. During `UpdateCore`, they use `MarkResourceChanged(ref updateOnly)` to coalesce the whole attached update. Ownership/value helpers apply the same gate. No derived raw setter can bypass a claim, reset/decrement the revision, or expose an ungated authoring mutation.

`SetResourceOperationValue` is reserved for generated NodePort value setters that run inside a custom lifecycle wrapper. It reuses the current thread's active `ExecuteResourceOperation` reservation, or creates the same reservation when invoked as the top-level generated setter, then assigns and advances `Version`. Ordinary generated value properties keep using the stricter `SetResourceValue`; neither helper admits a retained caller around a claimed/terminal Resource or a competing callback/disposal transition.

`ClearResourceOperationValueForCleanup` is the terminal counterpart generated for NodePort backing fields. It is valid only while the base-owned `DisposeAuthorResources` phase is active, clears the field without advancing the already-terminal Resource's `Version`, and rejects before touching the field from any ordinary/late call. Generated private `Clear<Property>NodePortValueForResourceCleanup` wrappers let Text/Filter and plugin author cleanup release their output value without routing through the terminal public setter.

`GraphNode.Resource` binding membership is not a public authoring or inspection escape hatch. `ItemValues` and `ItemIndexMap` are protected binding infrastructure for generated/derived resource code, while `GraphSnapshot` uses separate internal array/map access. They are not exposed to an unrelated caller as either mutable collections or nominally read-only collections whose `IItemValue` elements still expose mutation and disposal. Per-frame item-value mutation remains internal graph evaluation state, so external code cannot replace, mutate, or dispose a binding behind generated bound fields and cache identity. Its Bind/Initialize/Update/Uninitialize public lifecycle entries follow the same non-virtual-wrapper/protected-core pattern through `ExecuteResourceOperation`. A `GraphSnapshot` owns its node resources through a private Resource owner and invokes each lifecycle callback with `ExecuteOwnedResourceCallback`; standalone snapshots therefore get the same exclusive claims as snapshots nested in drawable/effect resources. A live mutable snapshot is not exposed from those outer resources, and render-node collections use a non-downcastable read-only view rather than a retained `List` alias.
...
`ResourceReconciler` is public for out-of-tree `Resource` implementations, but its methods no longer infer an absent owner and perform raw child mutation. A derived `UpdateCore` runs reconciliation through protected `ExecuteOwnedResourceReconciliation`; a mixed or fully-owned list that needs before/after membership and terminal-state detection uses `ExecuteOwnedResourceListReconciliation`. The scope establishes the exact owner authorization, coalesces `Version` through the supplied `ref updateOnly`, and releases the owner gate before child quiescence or callbacks. Calling a public `ResourceReconciler.Reconcile*` method without that active owner scope throws before mutation. Generated detached construction uses the reserved `ClaimOwnedResourceForConstruction` only after a materialized default child has passed active-state validation; the lexically bound construction-abort path releases every claim that cannot survive the failed constructor.

The first public `Dispose()` attempt atomically transitions the resource to its terminal state before invoking author code. `IsDisposed` is therefore true during `DisposeAuthorResources()`, and every update, setter, detach, replace, or ownership helper rejects immediately even when cleanup later throws. The most-derived author-facing virtual `DisposeAuthorResources()` entry is explicit-disposal-only and invoked at most once. Generated implementations catch each parameterless `PreDispose`/`PostDispose` failure, pass it to `CaptureResourceAuthorCleanupFailure`, attempt their remaining one-shot phases, and invoke the base chain. A hand-written override is a conforming part of that chain only when it likewise captures its own failure with that reserved helper and invokes `base.DisposeAuthorResources()` through finally-equivalent control flow. `DisposeAuthorOwnedDisposable` is the matching one-shot helper for non-`Resource` author-owned disposables: it validates the active author phase before touching the slot, cycle-safely rejects a direct `EngineObject.Resource` or a tuple/array/dictionary/generic/field-backed container whose runtime type shape owns one, attempts the arbitrary disposable once, clears only after success, and captures a failure without preventing later author slots/base phases. The exact engine-owned `RenderNode` hierarchy is the sole Resource-shaped non-owning leaf: its captured Resource/version snapshots are borrowed and `RenderNode.Dispose` does not dispose them. This is not an extensible arbitrary-container whitelist. All other Resource-shaped ownership belongs in explicit generated retry slots; hiding it behind the one-shot author helper is not supported. Neither author helper may be called outside the active base-owned author collector. The outer `Dispose` orchestrator recursively flattens captured cleanup aggregates while preserving their original derived-to-base/slot order, rethrows one leaf through `ExceptionDispatchInfo`, or creates one final flat aggregate with author failures before generated child failures. It cannot resume a base phase that an arbitrary override skipped. A failure cannot cause a later `Dispose()` call to re-enter any author phase that was reached on the first attempt. There is no false/finalizer branch on this virtual hook; unmanaged ownership uses `SafeHandle` or an author-owned finalizer instead of a misleading dead `disposing == false` path.

Generated owning object and list cleanup lives in the separate protected `DisposeGeneratedResources()` retry hook. Generated overrides process their own slots in declaration order and then call the base override, giving deterministic derived-to-base ordering. A generated partial resource additionally declares private `partial void DisposeGeneratedResourcesCore()`: the generated override invokes it first, captures a thrown failure through `CaptureGeneratedResourceCleanupFailure`, and still continues through generated slots and the base override. A generated partial author implements that partial method rather than declaring a competing override; if it needs to continue after multiple independent custom failures, it catches each and calls the same reserved capture helper. A hand-written non-generated subclass may override `DisposeGeneratedResources()`, but it likewise captures its failures and reaches `base.DisposeGeneratedResources()` through finally-equivalent flow. `DisposeGeneratedResource` clears an object slot only after the child disposes successfully. `DisposeGeneratedResourceList` removes null or successfully disposed entries in place and retains only failed entries in stable order. The helpers and capture method append failures only to a base-owned collector active during this hook; the override cannot enumerate, remove, reorder, or replace already recorded author/generated failures, and calling any of them outside the active generated-cleanup phase throws. The first attempt invokes this hook even when author cleanup failed; later `Dispose()` calls invoke only this hook while pending generated children remain. One cleanup failure is rethrown with its original type and stack. Multiple failures are aggregated in reached author-phase order, then generated derived-to-base/declaration order. Recursive or concurrent disposal observes the terminal state and cannot repeat author cleanup or transfer a child. Finalization never invokes author or generated retry hooks, and no finalizer exception escapes.

This concurrency guarantee covers engine authoring/lifetime seams: generated/manual object/list ownership transitions, owner-authorized child reconciliation/disposal, the non-virtual public `Update` entry, direct generated value/list helpers, and terminal disposal. Pending two-phase reservations reject new child work until commit/rollback, and no registry lock is held while waiting on a resource gate or invoking virtual code. Claim preparation may wait for an already-started operation on the incoming child to become quiescent, but a competing operation that finds the owner already inside a mutation fails immediately instead of waiting. This fail-fast boundary prevents a child callback from waiting on the parent while the parent waits for that child; the rejected caller may retry after the active transition completes. It does not make getters, arbitrary plugin methods, or a complete multi-property `UpdateCore` transaction generally thread-safe. Callers continue to serialize ordinary reads and custom mutation; the claim graph prevents an owned child from being installed in two slots/owners, mutated or disposed through a retained external engine seam, double-transferred, disposed after detach, installed after disposal starts, or appended through a retained list alias when documented operations race.

The built-in static `Brushes.Resource` values are borrowed-only lifetime nodes. They remain reusable render-borrow identities but cannot be installed into an owning property/list, directly disposed, updated, or mutated as detached authoring state. A plugin-owned derived Resource can establish the same one-way state with protected `MarkResourceBorrowedOnly()` while constructing an inactive, unowned, childless instance before publishing its shared/static reference; a later or repeated transition rejects. An author needing owned state creates an independent resource instead of transferring a shared singleton.
...
- `LoweredBrush`/`LoweredPen` carry the execution lease that resolved them, not just the payload. Every `PaintedRenderCanvas` draw overload that accepts one asks that execution whether the payload is still held. `ImmediateCanvas` exposes no public lowered-paint overload, so an author cannot redirect the session capability to an unrelated canvas; a retained copy also fails once its painted callback ends. Retention is a deterministic failure rather than a released native handle.
...
Every explicit `structuralKey` and resource `cacheKey` may be retained by a structural/program/render cache beyond the recording request. It must therefore be a lightweight, deeply immutable, equality-stable CPU identity such as a runtime `Type`, runtime metadata-backed `MethodInfo`, string, primitive/value tuple, immutable record composed of such values, or the copied `RenderStateSequence<T>` below. Keys must not be a context/session/handle/facade, `RenderResource`, captured delegate, mutable/open collection or graph, `IDisposable`, native/target object or handle, or a large payload. Custom `MethodInfo` subclasses and `DynamicMethod` are rejected for reusable identity. Accepted runtime methods normalize to module MVID plus metadata token, closed declaring type, and closed generic method arguments; neither top-level nor nested method identity invokes author equality. Public descriptions and `RenderResourceIdentity(Key, Version)` keep the authored immutable key for inspection. The engine separately derives a private complete-field identity for structural/program lookup, repeated-resource registration, Shader resource identity, and output-cache components; author `Equals`/`GetHashCode` cannot omit a field or corrupt/alias a cache bucket, and no opaque private wrapper replaces the public key value. When a large or native object needs identity, authors supply a small immutable ID/version key instead of the object itself.
...
public static class RenderStateSequence
{
    public static RenderStateSequence<T> CopyOf<T>(IEnumerable<T> source)
        where T : notnull;
}

public sealed class RenderStateSequence<T> :
    IReadOnlyList<T>,
    IEquatable<RenderStateSequence<T>>
    where T : notnull
{
    public int Count { get; }
    public T this[int index] { get; }
    public IEnumerator<T> GetEnumerator();
    public bool Equals(RenderStateSequence<T>? other);
    public override bool Equals(object? obj);
    public override int GetHashCode();
}
```

Every reusable execution-description factory takes its pixel-affecting values as one `state` argument and a non-capturing `Action<TSession, TState>`. Every custom bounds, scale, and hit-test factory follows the same state-first rule with its matching static `Func<TState, ...>`. A capturing callback is rejected synchronously. `state` is copied into the description or metadata contract and must be deeply immutable: primitive/string/type values, immutable value tuples/record structs, sealed immutable records whose instance fields are readonly and recursively immutable, or `RenderStateSequence<T>`. Mutable reference holders, arrays and other mutable/open collections, delegates, `object`/interface-typed escape hatches, disposable/native/request/session/resource objects, and recursive/cyclic state type graphs are rejected. `RenderStateSequence.CopyOf` validates the closed element shape and every element, enumerates the source into private storage before publication, rejects null or invalid elements, and exposes no mutable backing array. Source mutation after `CopyOf` cannot change the snapshot. Nested sequences compare by length and ordered element content with the same engine-owned field-graph equality/hashing as ordinary state; author element `Equals`/`GetHashCode` is never trusted. Output-cache and runtime-metadata identity walk this accepted graph, so an override cannot omit a callback-read field. A caller snapshots mutable authoring state into an immutable value/version record or copied sequence before creating a reusable execution description or metadata contract; execution descriptions alone provide a request-local capturing form because mutable metadata would make forward/backward analysis internally inconsistent. `RenderCacheVerification` remains an evidence/debugging check, never the production safeguard for an invalid identity.
...
Every custom forward, backward, multi-input bounds, scale, and hit-test factory uses the same state-first generic form: one copied `TState` plus non-capturing callbacks whose first argument is that state. `TState` is validated with the same recursive deeply immutable rules as reusable execution descriptions. A capturing callback, mutable holder, raw array/collection, delegate field, resource/session/context/facade, native handle, recursive graph, or other mutable escape is rejected synchronously; variable-length state uses `RenderStateSequence.CopyOf`. Paired forward/backward callbacks retain and receive the exact same stored state snapshot; later forward metadata, backward ROI, scale reevaluation, and hit testing therefore cannot observe different values. The engine owns deep state equality/hashing for runtime metadata and output-cache identity and never invokes author-defined equality. Callback methods and an optional explicit `structuralKey` describe only structural identity; a non-null key is itself deeply validated, remains publicly inspectable as authored, and has a separate internal complete-field identity for plan/cache lookup. Identical inputs and the same state must produce the same deterministic, side-effect-free CPU result; invalid rectangles or non-finite scale results fail validation rather than falling back silently.
