...
- `PaintedSource` has a second, primary-resource form. Every painted source in the tree draws exactly one resource, and every one of them reached it the same way: `UseDeclaredResource<TResource>(0, value => ...)` inside the draw callback. That is four things the author had to keep in step by hand — the index, the type argument, the call itself, and a nested closure — for a resource the recorder was going to declare anyway. The primary form takes `RenderResource<TResource> primary` and an `Action<PaintedRenderSession, TResource, TState> draw`; the recorder declares `primary` ahead of `resources`, leases it for the duration of the callback, and passes the leased value in as a parameter. The resource is type-checked at compile time, and the form has no ambiguity mode: there is exactly one primary and it is named, not looked up.
  ...
  - It is cheaper, not merely no more expensive: the recorder's single lease closure replaces the author's, and the author's `resources` array is one element shorter. Measured over a whole rasterization at `GeometryRenderNode`'s shape it allocates 56 bytes/rasterization less than the positional form; the absolute figures drift with the shared rasterization scaffold, the difference does not. `PrimaryPaintedSourceTests` pins the direction of that comparison.
...
- The same shape is deliberately **not** added to `OpaqueRenderDescription.Create`, `TargetScopeDescription.Create`, or `TargetCommandDescription.Create`, even though `ParticleRenderNode`, `GeometryClipRenderNode`, and `NodeGraphFilterEffectRenderNode` each address one resource at index 0 through them. Those are caller-owned description factories, not recorder methods: nothing in them owns the resource list the way `PaintedSource` owns the merge with the lowered paint, so a `primary` parameter there would be a description constructor quietly rewriting its own `resources` argument. Their sessions also already expose the token-taking `UseResource`, which is the compile-time-safe form for a callback recorded through a capturing channel, so those three sites have a supported alternative that `PaintedRenderSession` — which exposes no token form at all — does not. Adding the parameter to three more factories would triple the new surface to remove an index from three call sites that are not the ones the hazard is worst for.
