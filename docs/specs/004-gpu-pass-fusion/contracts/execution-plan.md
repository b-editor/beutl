...
5. **Interplay & correctness.** The outer `RenderNodeCache` is unchanged: when the whole node is stable it still wins (tile replay before `Process`), and a fully static group never engages C10 (the whole-plan guard declines it). Single-frame renders (the SC-001 counter fixtures) never reach the engagement threshold, so those counters are unaffected. The capture frame retains one buffer past its plan-declared last use, so the FR-007 peak-live DEBUG assertion is skipped on capture frames only (the retained lease is the cache's, not a plan intermediate); resume frames subtract the retained lease as the execution baseline and still assert.
