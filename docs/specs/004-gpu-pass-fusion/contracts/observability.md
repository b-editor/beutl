...
- Strong percentage-valued Gamma/Invert/Threshold/Negaposi/Saturate/Brightness/HighContrast/ColorGrading cases compare against immutable `004-parity-strong/` references generated by the actual pre-removal `FilterEffectActivator` at commit `8dad1ba3`. They are independent legacy gates; never regenerate them from the declarative executor. Post-redesign determinism-only workloads belong under `004-review/`.
- The `004-baseline/effect-CSharpScriptEffect` blob is the original pre-removal `Context.Blur(...)` reference. The migrated fixture uses the directly equivalent `Builder.Blur(...)`; it is not a post-redesign re-freeze.
