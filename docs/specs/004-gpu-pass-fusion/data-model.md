...
`Own<T>` requires `T : class, IDisposable`; `Borrow<T>` and `RenderResource<T>` require only `T : class`. The request-family raw-reference table rejects duplicate Own and any Own/Borrow conflict. Repeated Borrow with an explicit non-null key coalesces only with the same deeply normalized key/version; an explicit mismatch is rejected, while each null-key registration gets a distinct request-local identity and cannot reuse output cache across requests. The raw value is exposed only inside an authorized execution callback. Every consuming description lists its token, causing key/version to enter output-cache identity; undeclared use is rejected. An owned slot is discharged exactly once by rollback/teardown disposal or by an atomic ownership transfer into an accepted persistent cache payload; the request then holds no lease to it. Borrowed teardown invalidates only the token; both owner and callback keep pixel-affecting state read-only throughout the request. Metadata-only requests never access the raw value. Keys retained by caches are lightweight immutable equality-stable CPU IDs, never resource/context/native objects or large payloads. Public structural/resource keys remain the authored values for inspection, while private registration/Shader/cache identities walk every validated immutable field and ordered `RenderStateSequence` element without author equality.

### EngineObject.Resource terminal disposal

Generated nested object/list properties are owning slots. The containing resource has a two-part lifecycle:

```text
Active --first Dispose()--> Terminal + AuthorCleanupComplete
                               | generated children pending
                               +--later Dispose()--> retry generated children only
```

The active-to-terminal transition linearizes under the ownership gate before callbacks, so `IsDisposed` is true and update/assignment/detach/replace reject from the first attempt onward. Each installed child has one acyclic internal lifetime claim; pending two-phase reservations reject competing work and commit/rollback outside virtual callbacks. Claim preparation may wait for preexisting incoming-child work, but an external operation encountering an active owner mutation fails immediately and commits nothing, preventing parent/child callback wait cycles. The ownership-node graph has no Resource backreferences, and finalizer abandonment orphans edges without extending an unreachable parent lifetime. An active claimed child accepts Update/Dispose/generated authoring only under its owner's internal authorization; detach releases the claim. Public `ResourceReconciler` methods require a protected owner scope opened by the derived Resource and never fall back to raw unclaimed mutation. Borrowed-only static resources never accept a claim. The most-derived explicit-only author `DisposeAuthorResources()` entry is invoked once; it replaces the old dead-branch `Dispose(bool)` shape. Generated implementations pass each parameterless `PreDispose`/`PostDispose` failure to the base-owned author collector and continue through their base phase; a hand-written override uses the reserved capture helper and the same finally-equivalent base-call discipline because the orchestrator cannot resume a skipped virtual base phase. Non-`Resource` author-owned disposables use the one-shot author helper; nested `Resource` ownership is rejected there and belongs in the retryable generated hook. A separate generator-reserved hook processes object slots and list entries in source declaration order, then base types in derived-to-base order. A generated partial customizes its first phase through private `DisposeGeneratedResourcesCore`; a manual subclass overrides the protected hook. Both capture custom failures in the base-owned generated collector and continue to generated slots/base. Successful children are removed/released exactly once; failed children stay claimed in stable order. Generated and hand-written list state is visible only through a cached read-only view; owner-gated add, dispose-remove, clear, detach-at, replace-at, move, and whole replacement are the only public mutations and all reject after terminal transition. Flow-backed manual lists additionally preserve a non-owned reconciler prefix that cannot cross the ownership transfer boundary; `DelayAnimationEffect` keeps its delayed-resource cache non-public. One leaf failure preserves its type/stack and nested cleanup aggregates are recursively flattened into deterministic author-first/generated-second order. Reentrant/concurrent calls cannot re-enter reached author code. Finalization invokes neither cleanup hook.

Attached resource initialization failures enter ordinary explicit disposal and preserve the initialization exception before flattened cleanup failures. Detached generated constructors remain unpublished/unclaimable and stage empty list backing, author construction state, and declared defaults in one current-level `try`. Failure invokes only the statically bound current-level construction cleanup and its base-qualified chain, never a virtual derived hook on zero-initialized fields. Only the runtime-most-derived successful constructor performs the one-way completion to Ready. `BESG007` moves mechanically visible disposable/`Resource` initializer ownership into this staged path; arbitrary author-owned state receives one construction-abort cleanup attempt and one normal one-shot author cleanup, while generated `Resource` slots additionally participate in normal retry cleanup. The temporary owner used as an automatic or explicit-constructor defaults source has no cleanup lifetime; `BESG008` requires its initializers to be ownership-free apart from recognized engine property storage and provably empty exact BCL list/array containers. An explicit `[ResourceDefaultValuesConstructor]` path is one restricted marker-forwarding instance constructor, never an ordinary factory/constructor path; primary-constructor owners cannot use it.

Every direct generated value/`IsEnabled` assignment is an explicit invalidation event and increments `Resource.Version`; ownership helpers increment only for a proven membership/reference/order change. Attached `Update` passes one `updateOnly` token through generated `PreUpdate`, base/generated fields, nested children, and `PostUpdate`; manual partials call the ref `MarkResourceChanged`, so all changes coalesce into one parent increment, including a nested change committed before a later exception. The revision setter is private. A manual detached-authoring seam calls parameterless `MarkResourceChanged()`, which validates terminal/claim authorization and increments once. Graph-node binding membership is protected/internal rather than public, preventing external item mutation/disposal or array/dictionary replacement from splitting generated bound fields from the snapshot that execution indexes.

### Callback state identity

Reusable opaque, Geometry, target-scope, target-command, and painted factories copy and validate complete deeply immutable callback state, then derive its render-output cache identity without exposing a separate author-supplied key. Their request-local execution forms may capture mutable or otherwise unsnapshotable state and receive a fresh identity for every recording, safely disabling cross-request pixel-cache reuse. Custom bounds, multi-input bounds, scale, and hit-test factories have no request-local form: each stores one deeply immutable `TState` and non-capturing state-first callbacks, and paired phases share that exact state. The callback method and optional explicit key form structural identity; engine-owned recursive state equality/hashing forms runtime metadata/output-cache identity. There is no overlapping explicit runtime-key channel. Built-in semantic descriptors include their scalar values automatically, and Shader uniforms contribute their canonical validated values automatically. Callback-state identity participates in render-output cache identity but never structural plan/program identity.
...
| structural identity | Stable static callback method/kind identity excluding immutable runtime state values. |
| runtime metadata identity | Engine-owned deep identity of the stored immutable state shared by forward/backward phases. |
...
`RenderScaleContract` is vector/unbounded, input-supply preserving, one-input supply mapping, standard supply-driven materialization, or a pure CPU custom resolver. `MapInputSupply<TState>(TState state, Func<TState, EffectiveScale, EffectiveScale> map, object? structuralKey)` is valid only for an element-wise one-input map. It applies `map` and the stored immutable state to that input's resolved supply, may preserve `EffectiveScale.Unbounded`, and uses the callback/key as structural identity; Transform and DrawableGroup use it for their density transform. If the input is symbolic, any recording-time result is only an internal hint and the map is evaluated again from the resolved supply with the exact same state during forward resolution.

A custom resolver uses its stored immutable state, input supplies, and complete conservative output bounds and cannot observe the later requested region. With concrete inputs it establishes the recording-time result; with symbolic inputs any provisional result remains internal and the resolver is reevaluated with the same state after graph-wide bounds/supply resolution. It must return a finite value greater than zero; a throw or invalid result fails rather than selecting a fallback. A valid finite result is capped by `MaxWorkingScale` plus the per-buffer dimension clamp against the complete resolved output bounds. Only a concrete recording result is available through `TryGetMetadata`; later ROI crops logical allocation bounds without changing the final density. The callback/key identity is structural, while state and resolved density are runtime metadata.
...
Failure from any non-terminal state transitions to `Failed`, runs the same request-owner cleanup, preserves the primary exception, rejects any cache publication that has not reached its atomic commit point, produces one failure diagnostic, and then transitions to `Disposed` (`src/Beutl.Engine/Graphics/Rendering/Planning/RenderRequest.cs:95-151`). If the complete replacement set was already committed, a later failure disposing superseded cache storage remains a cleanup failure and does not roll back the accepted replacements.
