...
3. **Slice 2 — render-target (Custom) effects**: `FilterEffectExecutor`/`CustomFilterEffectContext` allocate `ceil(bounds×w)`; `FilterEffectContext` primitives × `WorkingScale`; SKSL `iScale` = `w` (GLSL derives it from the device-px `Width`/`Height`); the supply-driven `w` is honored at the effect boundary.
...
- **Filter-effect sinks** (`FilterEffectExecutor`/`CustomFilterEffectContext`) use component-wise `(int)` truncation that differs from `FromRect`; migrating them is a scale-1.0 behavior change → golden-test it.
