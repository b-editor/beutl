...
- **Effects multiply by the working scale `w`, not the output scale `s_out`.** A 0.5 proxy fed to a default-`Inherit` effect runs at 0.5 (no upsample); a 2.0 source runs at 2.0 (no downsample, downscaled only at the final stage). Use `ClampToOutput` to opt a heavy effect out of carrying high res; `PreserveSource`/`Oversample(k)` to keep/force it (FR-036).
