...
- [ ] T044 [US2] Audio-visualizer drawables: classify pixel-magnitude params (bar width, block gap, hard-coded minimums) under FR-008 × `w` (FR-030) across `src/Beutl.Engine/Graphics/AudioVisualizers/`. **Needs its own probe** — the "shared BrushConstructor scale" framing was dropped (PerlinNoise disproved it, 2026-06-09). Most visualizer geometry is drawn in logical units under the CTM (likely already scale-correct, like ShakeEffect); only genuinely device-baked constants (hard-coded `0.5f`/`1f` pixel minimums) would need attention, and that is a localized clamp, not brush-path plumbing. Verify with a reduced-scale golden before changing anything.
