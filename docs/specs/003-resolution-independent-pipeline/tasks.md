...
- [~] T060 [P] SC-003 benchmark: commit both required variants: (1) a vector/text/Skia-filter-heavy `bench.scene` with a pinned-seed schedule of 11 paired samples—exactly five 0.5×/1.0× orders, five 1.0×/0.5× orders, one seed-selected unmatched order, then a seeded permutation—requiring 0.5× to win at least 9 pairs (one-sided exact sign test `p < 0.05`) and `median(0.5)/median(1.0) < 0.85`; report the seed, realized order, and exact-tie count, with each tie counted as not a 0.5× win and never discarded or rerun; and (2) a source-heavy scene with a concrete bitmap source through a resolution-sensitive effect whose separately reported expected median ratio is approximately 1.0 and therefore demonstrates little or no preview speed-up under the supply-driven model. Put both `[Explicit][Category("Benchmark")]` cases in `tests/Beutl.UnitTests/Engine/Graphics/Rendering/Golden/RenderScaleBenchmarkTests.cs` and both BenchmarkDotNet variants in `tests/Beutl.Benchmarks/RenderScaleBenchmark.cs`.
