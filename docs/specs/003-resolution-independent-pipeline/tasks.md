...
**The supply-driven model in one line**: output scale `s_out` is the final target only; each op carries an `EffectiveScale` (vector = `Unbounded`); each boundary computes a working scale `w = ResolveWorkingScale(inputs, s_out, maxWorkingScale)` (supply-driven, no policy); effects follow the FR-008 coordinate-space rule (logical geometry under the CTM unchanged; device-buffer dims / device-space uniforms `× w` once; readback geometry `÷ w`); **byte-identical at `s_out=1.0` with unit-scale inputs.**
...
- [~] T002 [P] ~~Create `ResolutionPolicy` value type + `ResolutionPolicyKind` enum~~ **REVERTED (2026-06-09; customization route amended by 004).** `ResolutionPolicy.cs` was created then deleted — the policy concept was removed (zero in-tree users). A narrow non-supply scale now uses `FilterEffect.Resource.PlanRenderNodeFactory` plus `PlanFilterEffectRenderNode.ResolveWorkingScale`; fully opaque execution uses `CustomRenderNodeFilterEffect`. See the requirements.md amendment log. The default working scale is supply-driven only.
...
- [ ] T007 [P] SC-008 non-GPU search test `NoPixelCouplingOnRenderPathTest`. **Status:** NOT added — the strict "no `ToSize(1)` / no `(int)` cast on the render path" premise does not hold and shouldn't (e.g. converting a logical `PixelSize` such as `FrameSize` into a `Size` is legitimate; effect sinks deliberately keep `(int)` truncation at `w=1` for byte-identity). A meaningful version needs an allowlist of legitimate sites; deferred in favor of the behavioural buffer-activation goldens (`CustomEffectSupersampleTests`, etc.).
...
- [X] T011 Add `float outputScale = 1f` ctor param + `OutputScale` getter to `RenderNodeProcessor`; seed the context at the single `Pull` site; add internal `RasterizeAt(op, w)` seam generalizing `RasterizeToRenderTargets` in `src/Beutl.Engine/Graphics/Rendering/RenderNodeProcessor.cs`. **Feature 004 amendment:** construction is now `new RenderNodeContext(input, RenderIntent, OutputScale, MaxWorkingScale, PullPurpose)`; intent is mandatory and nested pulls inherit policy through `CreateChildProcessor`.
...
- [X] T013 [P] Add `float renderScale = 1f` ctor input + `OutputScale`/`DeviceSize` (`ceil(FrameSize×s_out)`) to `Renderer` and `IRenderer` (default-interface-impl → `1f`/`FrameSize`); keep `FrameSize` logical; `HitTest`/boundary math stays logical in `src/Beutl.Engine/Graphics/Rendering/Renderer.cs` and `src/Beutl.Engine/Graphics/Rendering/IRenderer.cs`. **Feature 004 amendment:** `Renderer` also requires an explicit `RenderIntent` before the scale arguments.
- [X] T014 [P] Add mandatory `RenderIntent` and `float renderScale = 1f` to the `SceneRenderer` ctor, forwarding intent, scale, and working-scale ceiling to `base(...)` in `src/Beutl.ProjectSystem/SceneRenderer.cs`
- [X] T015 [P] Add `float outputScale = 1f` + `OutputScale` to `GraphicsContext2D` ctor in `src/Beutl.Engine/Graphics/Rendering/GraphicsContext2D.cs`. **As shipped**: `canvasSize` is an exact logical `Size`, so `DrawBackdrop` uses `new Rect(canvasSize)` with no pixel rounding; the root CTM applies output scale and capture-scale reconciliation is handled by T046 (FR-021).
...
- [ ] T017 Byte-identity gate test vs a committed pre-feature `.bin` baseline. **Status:** the frozen-baseline form is DEFERRED pending the CI GPU-lane decision (plan.md residual) — a MoltenVK-specific `.bin` would break on a different golden backend. Byte-identity at `s=1` is currently guarded by: (a) every scale-aware path keeping a `w == 1` / `scale == 1` branch character-identical to the pre-feature code; (b) `ScaleOne_IsDeterministic` / `BlurredShape_ScaleOne_IsDeterministic` (live-path determinism); (c) every reduced-scale + export golden comparing against a ground-truth/structural reference. When the GPU lane is pinned, freeze the `.bin` and flip to `AssertByteIdentical`.
...
**Checkpoint**: dual-target build green; `dotnet test` green + no-migration (T018). **Byte-identity (T017) is DEFERRED, not satisfied here**: the `s=1` guarantee rests on the `w==1` path-identity branches + the determinism goldens (`ScaleOne_IsDeterministic`), NOT a frozen pre-feature `.bin` baseline — this is not "byte-identity gate passing". The SC-008 `NoPixelCouplingOnRenderPathTest` was also **deferred** (T007); completeness is carried by the behavioural buffer-activation goldens.
...
- [X] T023 [US1] Root output scale in `Renderer`: bake one `Matrix.CreateScale(OutputScale)` into the root `ImmediateCanvas`; `OutputScale==1` keeps the exact pre-feature transform, the FPS overlay re-enters device space, the device surface is `ceil(FrameSize×OutputScale)`, and `GraphicsContext2D(node, FrameSize)` stays logical in `src/Beutl.Engine/Graphics/Rendering/Renderer.cs`
- [~] T024 [US1] **Satisfied by the root CTM for the MVP**: `TextRenderNode` emits a lambda calling `canvas.DrawText(textBlob)` directly, so under `CreateScale(s)` Skia re-rasterizes glyph outlines at device resolution (crisp, like vector) — no functional gap for uniform output scale. Exact target-scale **reshaping** of the `FormattedText` shaping cache (`_textBlob`/paths/metrics, hinting-level) is **deferred** as a quality refinement, left untouched to preserve scale-1 byte-identity. Hit-test fill/stroke remain logical (D3).
...
> The earlier ledger concluded `×w` was unnecessary on the sole evidence of 0.5× DOWNSCALE fidelity
> (where every effect rides the root CTM), missing the shipped s_out>1 export-supersampling feature,
> for which render-target ("Custom") effects were proven to render at LOGICAL density and be soft-upscaled
> by the CTM. Buffer activation is now implemented:
...
> **FIXED (2026-06-09):** DrawableBrush / TileBrush FILL content (`BrushConstructor`) is now rasterised at the
> render density: the tile intermediate is sized `ceil(IntermediateSize × s)`, a DrawableBrush child is
> re-rendered at `× s`, and the tile shader's local-matrix is compensated by **`Scale(1/s)`** (because
> `texel = localMatrix⁻¹ · CTM⁻¹ · device`, un-densifying the texture coords needs `1/s`, not `s` — the
> first attempt used `s` and mistiled to SSIM 0.21). Verified by `TileBrushFillDensityTests`: a `None`-mode fill
> at 2× SSAA matches the content drawn directly (**SSIM 0.9999**, up from 0.998), a `Tile` (repeat) brush stays
> consistent across render scales (**SSIM 0.9999**, no mistiling), and the fill is byte-identical at `s = 1.0`.
> No regression across 249 rendering/brush/canvas/golden tests. `s == 1` is a no-op everywhere (byte-identical).
...
- [X] T033 [US2] `EffectTarget`: added `EffectiveScale Scale` (default `Unbounded`) set from the producing op, threaded through the `RenderTarget` ctor + `Clone`, and now CONSUMED by `EffectTarget.Draw` (resampled blit when `At(w)`); removed obsolete `Empty`/`Size`. Byte-identical (Scale defaults Unbounded). **`EffectTargets.MaxScale()`/`ResolveScale()` and the inert `RenderNodeContext.PreserveFloor` property were removed** — they had no consumer (the supply `max` is computed inside `ResolveWorkingScale`; there is **no** `preserveFloor` and **no** `policy` resolver parameter — `ResolveWorkingScale(inputs, outputScale, maxWorkingScale)`).
...
- [X] T039 [US2] **Confirmed magnitude-invariant → no `w`**: the `SKColorFilter`-based color effects (`Brightness`/`Gamma`/`Saturate`/`HueRotate`/`Invert`/`Negaposi`/`ColorGrading`/`Curves`/`HighContrast`/`LumaColor`/`LutEffect`/`Threshold`/`ChromaKey`/`ColorKey`) have no spatial params and need no change. **`ColorShift` pixel-offset `×w` is DONE** (`ColorShift.OnApply` multiplies every offset + `minOffset` by `context.WorkingScale`; `TransformBoundsCore` stays logical and is scaled by the `ceil(bounds×w)` buffer allocation — the earlier "deferred" note was stale). `ContourTracer`-based effects ride buffer activation (T040).
...
**Goal**: preview-scale control (per-edit-view, non-persisted) with rebuild-by-replacement renderer/cache swap (FR-031); hit-test/handles identical across scales; export supersampling AA.
...
- [X] T055 [US4] `RenderScale` enum (`Full`/`Half`/`Quarter`/`FitToPreviewer`) + `ToFloat(PixelSize, Size)` clamped to `(0,1]` (preview never upscales) in `src/Beutl/Models/RenderScale.cs`; compiles in the `Beutl` app. **Test-project gap**: no test project references the `Beutl` app, so `RenderScale` (and the US4 ViewModel tests T052–T054, which target `tests/Beutl.UnitTests/ViewModels/` — that project references `Beutl.Editor`, not `Beutl`) cannot be unit-tested as-specced. Resolve by adding a `Beutl`-app test project (or relocating the tested logic) when wiring Phase 6.
- [X] T056 [US4] `EditViewModel`: add non-persisted `ReactivePropertySlim<RenderScale> PreviewScale`; combine `(FrameSizeProperty, PreviewScale)` into one observable that rebuilds `SceneRenderer` + `FrameCacheManager` (`DisposePreviousValue`) **on the UI thread (two independent reactive-property swaps, NOT an atomic render-dispatcher swap — FR-031 clarification)**; leave `SaveState`/`RestoreState` untouched in `src/Beutl/ViewModels/EditViewModel.cs`. Any XAML surfacing the preview-scale selector MUST declare `x:CompileBindings="True"` + `x:DataType` (Constitution IV)
- [~] T057 [US4] `PlayerViewModel`: subscribe `PreviewScale` → `QueueRender()`; render lambda reads `Renderer.Value`/`FrameCacheManager.Value` fresh inside the serial render dispatcher (**NOT an atomic snapshot swap** — renderer and cache are two independent UI-thread swaps with a narrow, self-healing tear window the change's own re-queued render corrects; FR-031 clarification 2026-06-10) in `src/Beutl/ViewModels/PlayerViewModel.cs`
...
- [X] T059 [US4] Export supersampling: `OutputViewModel` offers **Off / 2× / 4×** and threads the factor into `SceneRenderer`; `FrameProviderImpl.RenderCore` downscales `Snapshot()` to `FrameSize` when `OutputScale>1` (**Mitchell for ≤2×, trilinear+mipmaps for 4×**); derive `VideoEncoderSettings.SourceSize` from the actual buffer and assert `== FrameSize` before encode (FR-026/FR-034) in `src/Beutl/ViewModels/Tools/OutputViewModel.cs`, `src/Beutl/Models/FrameProviderImpl.cs`

**Checkpoint**: preview-quality control rebuilds by replacement (FR-031); clicks/handles land identically at any scale; export SSAA delivers output-resolution AA; T052–T054 green.
...
- [X] T061 Wire the pinned values. **As shipped (2026-06-15 amendment)**: `MaxWorkingScale` = **`2 × s_out` preview / `+∞` export** (FR-037) is threaded `Renderer → RenderNodeProcessor → RenderNodeContext → FilterEffectRenderNode` and seeded at the editor preview `SceneRenderer` and `OutputViewModel`. The earlier finite `max(8, 4 × s_out)` export cap was a quality clip masquerading as an OOM backstop (it silently discarded detail from a source denser than 8 on the delivery render); export now imposes no quality ceiling and the per-buffer dimension clamp (`ClampWorkingScaleToBufferBudget`, 16384 px/axis) is the sole allocatability bound. **The per-effect `ResolutionPolicy` was removed entirely (2026-06-09)** — every built-in is supply-driven; `MaxWorkingScale` is the sole **global** upper bound (the per-buffer FR-037(b) dimension clamp may further reduce `w` at effect boundaries). Supersample factors **Off / 2× / 4×** (SC-009) are in `src/Beutl/ViewModels/Tools/OutputViewModel.cs`.
- [X] T062 [P] Document `iScale` (SKSL) and the GLSL `scale` push constant (= working scale `w`, mirroring SKSL's `iScale`; added 2026-06-15 — the earlier "GLSL has no scale uniform, derive from device-px `Width`/`Height`" was replaced) + the working-scale shader contract for authors in `docs/ai-workflow/` (reference `contracts/shader-uniforms.md`)
- [ ] T063 If any effect property's type/unit changes (e.g. `ColorShift` `PixelPoint`→`Point`): keep the source generator green with a snapshot case in the `tests/SourceGeneratorTest` NUnit suite (asserting on the generated output for the changed property) plus behavioral coverage in `tests/Beutl.UnitTests/`
- [X] T064 Ship as a breaking change: `refactor!:`/`feat!:` + `BREAKING CHANGE:` footer (naming `Beutl.Engine`/`Beutl.NodeGraph`/`Beutl.ProjectSystem`); route the public surface through `beutl-design-reviewer`; run `dotnet format Beutl.slnx` + `dotnet build Beutl.slnx` + `dotnet test Beutl.slnx -f net10.0 --settings coverlet.runsettings`. **Verified (2026-06-16):** 6 breaking-change commits carry proper `refactor!:`/`feat!:` + `BREAKING CHANGE:` footers; `dotnet format` clean; `dotnet build` 0 errors / 61 warnings (all pre-existing); `dotnet test -f net10.0` 3,445 passed / 0 failed; `beutl-design-reviewer` routed.
- [~] T065 Run the [quickstart.md](./quickstart.md) validation loop (the four golden gates + the preview/export walkthroughs) — runbook step, no source file. **Automated gates verified (2026-06-16):** `Rendering.Golden` + `ResolutionScaleTests` all green in the full suite (3,415 UnitTests passed). **Manual walkthroughs** (preview quality switching, export 2×/4×, copy-frame-as-image, 3D gizmo picking) require the running app.
