...
- [~] T035 [US2] Compositor: `targetScale = max(concrete child EffectiveScale)` (Unbounded excluded), cap to `s_out` only at the root (no per-effect clamp — the `ResolutionPolicy` was removed); resample off-target bitmap ops, regenerate `Unbounded` ops at `targetScale` (via `RasterizeAt`) in `src/Beutl.Engine/Graphics/Rendering/RenderNodeProcessor.cs`. **As shipped (partial — Codex review 2026-06-09):** the rule is **distributed, not centralized in `RenderNodeProcessor`** — `LayerRenderNode.Process` computes the `max(concrete child)` density and per-op resampling rides `ImmediateCanvas.Draw*Scaled` (T034). **Open gap:** `LayerRenderNode` reports `max` density upward but its buffer is a plain `Canvas.SaveLayer()` at the *current* CTM density (`PushLayer`), so a child denser than the current canvas (a transform-densified source under reduced preview) over-reports its detail — the M3 "true `At(w)`" invariant is violated for layer buffers. Fix = allocate the layer at its reported density + golden coverage.
...
- [ ] T042 [US2] Brushes: `PerlinNoiseBrush.BaseFrequency ÷ w` (centralized in `CreatePerlinNoiseShader`); tile/image/drawable intermediate raster × `w`; `DrawableBrush` child inherits `w`; opacity-mask threads `w` in `src/Beutl.Engine/Graphics/BrushConstructor.cs`, `src/Beutl.Engine/Graphics/TileBrushCalculator.cs`, `src/Beutl.Engine/Graphics/Rendering/OpacityMaskRenderNode.cs`. **Empirically confirmed needed (2026-06-09):** `Tier1ParameterScaleProbeTests.PerlinNoiseBrush_FillFidelity_AcrossScale` shows **SSIM 0.70** at 0.5× — `SkPerlinNoiseShader` generates in device space and does not follow the CTM, so the noise period is wrong without `÷w`. **Root blocker:** `BrushConstructor` is a scale-blind `readonly struct` (`(Rect, Brush.Resource?, BlendMode)`); the render/working scale must be threaded into it first — the **same plumbing** T044 (audio-viz) and the TileBrush-fill-density limitation need. Treat T042+T044+tile-brush as one **Tier-2 PR** ("thread scale into BrushConstructor"), not a per-effect ×w one-liner.
...
- [ ] T044 [US2] Audio-visualizer drawables: classify pixel-magnitude params (bar width, block gap, hard-coded minimums) under FR-008 × `w` (FR-030) across `src/Beutl.Engine/Graphics/AudioVisualizers/`. **Shares the T042 root:** the visualizer shapes fill via the scale-blind `BrushConstructor` and bake hard-coded pixel minimums, so they need the same "thread scale into BrushConstructor" plumbing — bundle into the Tier-2 brush-scale PR.
- [~] T045 [US2] 3D as mixed-scale bitmap: `Scene3DRenderNode` renders at `ceil(size×s_out)` and tags the surface op `EffectiveScale.At(w)` (w==s_out), resampled at the boundary — **done**; nested `SceneDrawable` now inherits the outer `s_out`/ceiling into its own `Renderer` and reports `At(w)` (FR-022) — **done** (`src/Beutl.ProjectSystem/ProjectSystem/SceneDrawable.cs`). **Remaining**: append the root device scale after the user transform in `Rotation3DTransform` (perspective `S·P≠P·S`) (`src/Beutl.Engine/Graphics/Transformation/Rotation3DTransform.cs`) — open part of FR-033.
