...
- [X] T041 [US2] SKSL/GLSL: scale uniform. SKSL exposes `iScale = w`; GLSL exposes the matching `scale = w` push constant. Device-pixel `width`/`height`/`iResolution` values remain distinct from the explicit density. Scale-unaware shaders behave as `w = 1.0`. In `src/Beutl.Engine/Graphics/FilterEffects/SKSLScriptEffect.cs`, `GLSLScriptEffect.cs`, `GLSLShader.cs`
...
- [~] T060 [P] SC-003 benchmark: commit both required variants: (1) a vector/text/Skia-filter-heavy `bench.scene` with pinned seed `20040719` producing 11 paired samples—exactly five 0.5×/1.0× orders, five 1.0×/0.5× orders, one seed-selected unmatched order, then a seeded permutation—requiring 0.5× to win at least 9 pairs (one-sided exact sign test `p < 0.05`) and `median(0.5)/median(1.0) < 0.85`; report the seed, realized order, and exact-tie count, with each tie counted as not a 0.5× win and never discarded or rerun; and (2) a source-heavy scene with a concrete bitmap source through a resolution-sensitive effect whose separately reported expected median ratio is approximately 1.0 and therefore demonstrates little or no preview speed-up under the supply-driven model. Put both `[Explicit][Category("Benchmark")]` cases in `tests/Beutl.UnitTests/Engine/Graphics/Rendering/Golden/RenderScaleBenchmarkTests.cs` and both BenchmarkDotNet variants in `tests/Beutl.Benchmarks/RenderScaleBenchmark.cs`.
- [X] T061 Wire the pinned values. **As shipped (2026-06-15 amendment)**: `MaxWorkingScale` = **`2 × s_out` preview / `+∞` export** (FR-037) is threaded `Renderer → RenderNodeRendererOptions → RenderRequestOptions → RenderNodeContext → FilterEffectRenderNode` together with `OutputScale`; materializing boundaries call `RenderScaleUtilities.ResolveWorkingScale(inputs, OutputScale, MaxWorkingScale)` (`src/Beutl.Engine/Graphics/Rendering/Renderer.cs:409-422`, `src/Beutl.Engine/Graphics/Rendering/RenderNodeRenderer.cs:694-763`, `src/Beutl.Engine/Graphics/Rendering/RenderNodeContext.cs:24-32`, `src/Beutl.Engine/Graphics/Rendering/FilterEffectRenderNode.cs:99-117`, `src/Beutl.Engine/Graphics/Rendering/RenderScaleUtilities.cs:19-35`). The values are seeded at the editor preview `SceneRenderer` and `OutputViewModel`. The earlier finite `max(8, 4 × s_out)` export cap was a quality clip masquerading as an OOM backstop (it silently discarded detail from a source denser than 8 on the delivery render); export now imposes no quality ceiling. `RenderScaleUtilities.ClampWorkingScaleToBufferBudget` provides only the per-buffer 16384-px-per-axis safeguard; aggregate byte/area/live-buffer budgeting and backend-reported limits remain out-of-scope follow-up work. **The per-effect `ResolutionPolicy` was removed entirely (2026-06-09)** — every built-in is supply-driven; `MaxWorkingScale` is the sole **global** upper bound (the per-buffer FR-037(b) dimension clamp may further reduce `w` at effect boundaries). Supersample factors **Off / 2× / 4×** (SC-009) are in `src/Beutl/ViewModels/Tools/OutputViewModel.cs`.
