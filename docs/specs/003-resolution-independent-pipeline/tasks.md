...
> re-rendered at `× s`, and the tile shader's local-matrix is compensated by **`Scale(1/s)`** (the key:
> `texel = localMatrix⁻¹ · CTM⁻¹ · device`, so un-densifying the texture coords needs `1/s`, not `s` — the
...
- [~] T057 [US4] `PlayerViewModel`: subscribe `PreviewScale` → `QueueRender()`; render lambda reads `Renderer.Value`/`FrameCacheManager.Value` fresh inside the serial render dispatcher (**NOT an atomic snapshot swap** — the renderer and cache are two independent UI-thread swaps with a narrow, self-healing tear window that the change's own re-queued render corrects; FR-031 clarification 2026-06-10) in `src/Beutl/ViewModels/PlayerViewModel.cs`
