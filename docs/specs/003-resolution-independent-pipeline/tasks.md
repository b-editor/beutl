...
- [X] T002 [P] Create `ResolutionPolicy` value type + `ResolutionPolicyKind` enum (`Inherit`/`ClampToOutput`/`Oversample`; statics + validating `Oversample(factor)`) in `src/Beutl.Engine/Graphics/Rendering/ResolutionPolicy.cs`. **As shipped**: no `PreserveSource` member (specced then removed — it duplicated `Inherit`); the only ctor is the private validating one behind the factories.
...
- [X] T036 [US2] `ResolveWorkingScale` policy branches (`Inherit`/`ClampToOutput`/`Oversample`) + the `maxWorkingScale` ceiling implemented in `RenderNodeContext` (landed with T009) and **validated by non-GPU unit tests** (`ResolutionScaleTests.cs`) incl. R1 (0.5 proxy not upsampled) and R2 (2.0 source not clamped). **No `PreserveSource` branch and no `preserveFloor` parameter** — both were specced then removed (FR-036); `Inherit` already runs at the supply density with no floor. Also: `default(ResolutionPolicy)` value-equals `Inherit` (Factor default 0f) and the only `ResolutionPolicy` constructor is the validating factory set (`Oversample(factor<=0)` throws).

### Implementation for User Story 2 — per-effect clusters (FR-009 traceability over the ~40-effect matrix)

- [X] T037 [US2] **Default `Inherit` + buffer activation landed.** `Dilate`/`Erode`/`FlatShadow` keep the default `Inherit` (no override — there is no `PreserveSource`). The sigma/offset/radius are Skia `SKImageFilter` primitives that ride the root CTM (no `×w` — verified Blur 1.0000), and any flushed buffer they share is sized `ceil(bounds×w)` by the activator, so they gain density under supersampling without per-param scaling.
- [X] T038 [US2] **Default `Inherit` + buffer activation landed + verified.** `Mosaic`/`Displacement`/`PixelSort` keep the default `Inherit`; `Mosaic.tileSize×w`, DisplacementMap translate/pivot `×w` (with a null-shader guard at `w≠1`), PixelSort is content-relative (no abs param, auto-scales). Verified by `CustomEffectSupersampleTests`.
