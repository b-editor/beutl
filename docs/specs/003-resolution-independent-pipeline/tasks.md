...
- Tests are **included** under the project test policy: new logic in `src/` ships with NUnit tests. Pixel/golden tests are Vulkan-gated (`VulkanTestEnvironment` → `Assert.Ignore` on GPU-less CI); the SC-008 search and SC-002 migration tests run without a GPU.
...
> - `FilterEffectExecutor.Flush` sizes flushed buffers `ceil(OriginalBounds×w)` + `CreateScale(w)` and
...
- [X] T030 [US2] **Landed.** `FilterEffectContext` ctor `(outputScale, workingScale)` + `WorkingScale`/`OutputScale` accessors (FR-015), propagated through `Clone`/`CreateChildContext`; `w` resolved & threaded by `FilterEffectRenderNode` into the executor. (Skia `SKImageFilter` primitives intentionally NOT `×w` — they ride the root CTM; see the note above.)
...
- [X] T032 [US2] **Landed.** `FilterEffectExecutor.Flush`: `ceil(OriginalBounds×w)` for `w≠1` keeping `(int)` at `w=1`; `CreateScale(w)`; tags result ops `EffectiveScale.At(w)`; `FilterEffectRenderNode` threads `w` into the executor and propagates `i.Scale` to the output op in `src/Beutl.Engine/Graphics/FilterEffects/FilterEffectExecutor.cs`
...
- [X] T036 [US2] `ResolveWorkingScale` (supply-driven) + the `maxWorkingScale` ceiling implemented in `RenderNodeContext` (landed with T009) and **validated by non-GPU unit tests** (`ResolutionScaleTests.cs`) incl. R1 (`At(0.5)` resolves to `1.0` at `s_out = 1.0` and remains `0.5` at a `0.5` preview) and R2 (2.0 source not clamped by output scale). **Updated 2026-06-09:** the policy branches (`Inherit`/`ClampToOutput`/`Oversample`) and the `policy` parameter were **removed** with the `ResolutionPolicy` type; the standard rule is `min(max(s_out, densest concrete input), maxWorkingScale)` for all input types (concrete, vector-only, and mixed; use `s_out` when there are no concrete inputs). The policy-specific tests (`ClampToOutput`/`Oversample`/C8 escape) were dropped.

### Implementation for User Story 2 — per-effect clusters (FR-009 traceability over the ~40-effect matrix)

- [X] T037 [US2] **Supply-driven + buffer activation landed.** `Dilate`/`Erode`/`FlatShadow` run supply-driven (no per-effect policy). The sigma/offset/radius are Skia `SKImageFilter` primitives that ride the root CTM (no `×w` — verified Blur 1.0000), and any flushed buffer they share is sized `ceil(bounds×w)` by the executor, so they gain density under supersampling without per-param scaling.
...
- [X] T062 [P] Document `iScale` (SKSL) and the GLSL `scale` push constant (= working scale `w`, mirroring SKSL's `iScale`; added 2026-06-15 — the earlier "GLSL has no scale uniform, derive from device-px `Width`/`Height`" was replaced) in `contracts/shader-uniforms.md`
...
- [X] T064 Ship as a breaking change: `refactor!:`/`feat!:` + `BREAKING CHANGE:` footer (naming `Beutl.Engine`/`Beutl.NodeGraph`/`Beutl.ProjectSystem`); complete an independent public-API design review; run `dotnet format Beutl.slnx` + `dotnet build Beutl.slnx` + `dotnet test Beutl.slnx -f net10.0 --settings coverlet.runsettings`. **Verified (2026-06-16):** 6 breaking-change commits carry proper `refactor!:`/`feat!:` + `BREAKING CHANGE:` footers; `dotnet format` clean; `dotnet build` 0 errors / 61 warnings (all pre-existing); `dotnet test -f net10.0` 3,445 passed / 0 failed; public-API design review completed.
