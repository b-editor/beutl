...
- [~] T060 [P] SC-003 benchmark: committed `bench.scene` + interleaved paired timing test (0.5× faster in at least 9 of 11 pairs; one-sided exact sign test `p < 0.05`, median ratio reported diagnostically) `[Explicit][Category("Benchmark")]` in `tests/Beutl.UnitTests/Engine/Graphics/Rendering/Golden/RenderScaleBenchmarkTests.cs` + a BenchmarkDotNet entry in `tests/Beutl.Benchmarks/RenderScaleBenchmark.cs`
