...
- **Preview**: `EditViewModel` rebuilds `SceneRenderer(Scene, RenderIntent.Preview, outputScale, maxWorkingScale: WorkingScaleCeiling.Preview(outputScale))` when the resolved `(FrameSize, OutputScale)` changes; `PlayerViewModel.RenderOnRenderThread` renders + snapshots. Editor display zoom is applied to the already-rendered bitmap at the UI layer and is orthogonal to render scale.
- **Export**: `OutputViewModel.StartEncode` sets `VideoEncoderSettings.SourceSize == DestinationSize == Model.FrameSize`, builds `SceneRenderer(Model, RenderIntent.Delivery, renderScale, disableResourceShare: true, maxWorkingScale: WorkingScaleCeiling.Export(), forceOriginalSource: true)`, and drives `FrameProviderImpl`, which renders + snapshots each frame. The encoder already supports a Source→Destination rescale; the supersampling path feeds a larger source and downsamples to the configured destination before encode.
...
6. **`DelayAnimationEffect`** (`FilterEffects/DelayAnimationEffect.cs`): describes one nested child graph per stable branch ordinal at the branch builder's carried working scale. Its sparse `DelayedResources` cache is keyed by that ordinal, and the executor supplies the completed pass's live ordinal set so disappeared branches are disposed in the same pull rather than at a later describe.
...
| **DelayAnimationEffect** (`DelayAnimationEffect.cs`) | — (Delay ms, inv.); wraps a child effect | Scale-invariant itself; each nested branch builder carries the parent branch's working scale into the wrapped declarative graph | N | Nested execution preserves each branch's carried scale; delayed child resources are sparsely keyed by stable branch ordinal and pruned from the completed pass's live set | No serialized pixel params; render policy and carried scale come from the branch builder |
