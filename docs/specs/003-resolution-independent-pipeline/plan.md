...
|---|---|---|
| **I. License Firewall (NON-NEGOTIABLE)** | ✅ PASS | Proxy *decode* (the only GPL/MIT-boundary work) is **out of scope** (FR-025); `MediaOptions` stays additively extensible but no FFmpeg IPC change in 003. GLSL/SKSL effects run **in-process** (MIT Vulkan/SkSL — `SKSLScriptEffect.cs:53` `SKRuntimeEffect`, `GLSLShader.cs:16` shared context), not the GPL `FFmpegWorker`, so the SKSL `iScale` uniform is an MIT-side change (GLSL ships no scale uniform — the author derives `w` from the device-px `Width`/`Height`; see shader-uniforms.md). **No NEW** MIT→GPL `ProjectReference` (the app-host `Beutl.csproj` conditional worker reference is pre-existing and intentional). |
...
1. **Slice 0 — plumbing skeleton** (no behavior change). `RenderNodeContext.OutputScale` + `ResolveWorkingScale` + `EffectiveScale` (value type), `RenderNodeProcessor.OutputScale`, `Renderer.OutputScale`; 3 sinks → `PixelRect.FromRect(rect, w)` (w=1). Land the golden harness (`NoPixelCouplingOnRenderPathTest` deferred — T007/SC-008). **Gate: byte-identical at 1.0.**
...
3. **Slice 2 — render-target (Custom) effects** (historical 003 implementation: `FilterEffectActivator`/`CustomFilterEffectContext` `ceil(bounds×w)`; `FilterEffectContext` primitives × `WorkingScale`; supply-driven `w` at the effect boundary; SKSL `iScale` = `w`). **Superseded by 004:** current GLSL compute scripts receive an explicit `scale` push constant alongside `width`/`height`; see `contracts/shader-uniforms.md`.
...
- **Built-in working scale** (FR-036) — **as shipped, every built-in is supply-driven** (no per-effect knob; runs at the input supply density, which keeps a high-res source through the effect). The `ResolutionPolicy` type (`Inherit`/`ClampToOutput`/`Oversample`/`PreserveSource`) was removed entirely — a narrow exception selects a `PlanFilterEffectRenderNode` through `FilterEffect.Resource.PlanRenderNodeFactory` and overrides `ResolveWorkingScale`, retaining standard plan execution. The working scale MUST NOT change `s_out=1.0` output.
...
- **`IRenderer.OutputScale`/`DeviceSize`** as hard breaking members vs default-interface-impls — resolved with default interface implementations so legacy renderers remain source-compatible.
