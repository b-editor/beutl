...
1. **Slice 0 — plumbing skeleton** (no behavior change). `RenderNodeContext.OutputScale` + `ResolveWorkingScale` + `EffectiveScale` (value type), `RenderNodeProcessor.OutputScale`, `Renderer.RenderScale`; 3 sinks → `PixelRect.FromRect(rect, w)` (w=1). Land the golden harness + `NoPixelCouplingOnRenderPathTest`. **Gate: byte-identical at 1.0.**
