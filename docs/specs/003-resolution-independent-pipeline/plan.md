...
**Performance Goals**: reduced-scale preview render-stage time scales ~`s²` for the rasterization-bound portion (SC-003 gate: 0.5× is faster in at least 9 of 11 interleaved 0.5×/1.0× pairs, a one-sided exact sign test with `p < 0.05`; the median ratio remains diagnostic rather than a cross-backend hard threshold — note 2026-07-24: backend and fixed render-stage overhead remain outside the rasterization-bound portion, so this proves **direction**, not the full `s² ≈ 0.25` value; a **required** source-heavy benchmark variant anchors the supply-driven model's deliberate non-speedup); reduced-scale "exact" effects SSIM ≥ 0.985 vs 1.0 (SC-004); `s=1.0` byte-identical for vector / Skia-filter / unscaled-bitmap content (SC-001)
