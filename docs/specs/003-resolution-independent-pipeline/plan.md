...
**Performance Goals**: reduced-scale preview render-stage time scales ~`s²` for the rasterization-bound portion (SC-003 gate: `median(0.5)/median(1.0) < 0.6`, ratio-based/hardware-independent); reduced-scale "exact" effects SSIM ≥ 0.985 vs 1.0 (SC-004); `s=1.0` byte-identical (SC-001)

**Constraints**: `s=1.0` raw-frame byte-identical to the pre-feature renderer (RgbaF16, zero epsilon); origins round **toward-zero** (not floor); uniform `float` scale v1 (Vector primitives pre-exist for later widening); no MIT→GPL boundary crossing; no `[Obsolete]` shims; preview scale per-edit-view, non-persisted

**Scale/Scope**: ~28 `RenderNode.Process` overrides reached via one `RenderNodeContext` construction site; ~40 filter effects + particles + audio visualizers adopt the scale contract + a per-effect resolution policy; ~12 breaking public symbols + 2 new value types (`EffectiveScale`, `ResolutionPolicy`); 6 implementation slices; FR-001..037
...
|---|---|---|
| **I. License Firewall (NON-NEGOTIABLE)** | ✅ PASS | Proxy *decode* (the only GPL/MIT-boundary work) is **out of scope** (FR-025); `MediaOptions` kept additively extensible but no FFmpeg IPC change in 003. GLSL/SKSL effects run **in-process** (MIT Vulkan/SkSL — `SKSLScriptEffect.cs:53` `SKRuntimeEffect`, `GLSLShader.cs:16` shared context), not the GPL `FFmpegWorker`, so the new `uScale` push-constant is an MIT-side ABI change. **No NEW** MIT→GPL `ProjectReference` is introduced (the app-host `Beutl.csproj` conditional worker reference is pre-existing and intentional). |
| **II. Dual-Target Framework** | ✅ PASS | No new TFM; both `net10.0` / `net10.0-windows` keep building. |
| **III. Test-First with NUnit** | ✅ PASS | New logic ships with NUnit: golden byte-equality + SSIM harness, per-effect manifest `[TestCaseSource]`, `NoPixelCouplingOnRenderPathTest`, cache-scale-invalidation, atomicity, perspective-append unit test. Benchmarks excluded from the default gate. |
...
tests/Beutl.UnitTests/Engine/Graphics/Rendering/NoPixelCouplingOnRenderPathTest.cs   # SC-008 (no GPU)
...
1. **Slice 0 — plumbing skeleton** (no behavior change). `RenderNodeContext.OutputScale` + `ResolveWorkingScale`/`ResolutionPolicy`/`EffectiveScale` (value type), `RenderNodeProcessor.OutputScale`, `Renderer.RenderScale`; 3 sinks → `PixelRect.FromRect(rect, w)` (w=1). Land the golden harness + `NoPixelCouplingOnRenderPathTest`. **Gate: byte-identical at 1.0.**
...
3. **Slice 2 — render-target (Custom) effects** (`FilterEffectActivator`/`CustomFilterEffectContext` `ceil(bounds×w)`; `FilterEffectContext` primitives × `WorkingScale`; per-effect `ResolutionPolicy`; SKSL/GLSL `iScale`/`uScale` = `w`).
...
6. **Slice 5 — editor + export** (`PreviewScale` control + atomic rebuild; export supersampling + `SourceSize` from `DeviceSize`; finalize logical hit-test/handles).
...
- **CI GPU lane**: whether to add SwiftShader/llvmpipe so SC-001/SC-004 pixel goldens run in CI, or keep them dev/self-hosted (maintainer + CI-workflow approval; do not change `.github/workflows/*` without it). If no GPU lane, the non-GPU SC-008 test is the CI-enforced guard and goldens run on dev/self-hosted GPU.
...
- **Built-in resolution-policy assignment** (FR-036) — pin each built-in effect's `ResolutionPolicy` in the FR-009 manifest (default `Inherit`; resolution-sensitive → `PreserveSource`; heavy → `ClampToOutput`); the policy MUST NOT change `s_out=1.0` output.
- **Global working-scale ceiling value** `MaxWorkingScale` (FR-037) — pin the default (e.g. preview `w ≤ 2 × s_out`) and where it's configured.
