...
3. **Slice 2 — render-target (Custom) effects** (`FilterEffectActivator`/`CustomFilterEffectContext` `ceil(bounds×w)`; `FilterEffectContext` primitives × `WorkingScale`; per-effect `ResolutionPolicy`; SKSL `iScale` = `w` (GLSL derives `w` from device-px `Width`/`Height`, no uniform)).
...
- **Built-in resolution-policy assignment** (FR-036) — **as shipped, every built-in uses the default `Inherit`** (which already keeps a high-res source through the effect). `ClampToOutput` / `Oversample` are plugin-opt-in; `PreserveSource` was removed (it equaled `Inherit`). The policy MUST NOT change `s_out=1.0` output.
