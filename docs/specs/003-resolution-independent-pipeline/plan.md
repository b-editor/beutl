...
**Performance Goals**: reduced-scale preview render-stage time scales ~`s²` for the rasterization-bound portion (SC-003 gate: 0.5× is faster in at least 9 of 11 interleaved 0.5×/1.0× pairs, a one-sided exact sign test with `p < 0.05`, and the pinned vector-heavy workload has `median(0.5)/median(1.0) < 0.85`; report the ~0.25 target without asserting that fixed overhead scales with `s²`; a **required** source-heavy benchmark variant anchors the supply-driven model's deliberate non-speedup); reduced-scale "exact" effects SSIM ≥ 0.985 vs 1.0 (SC-004); `s=1.0` byte-identical for vector / Skia-filter / unscaled-bitmap content (SC-001)
