...
| `EffectiveScale` (new type) | `Rendering/EffectiveScale.cs` | — | `readonly record struct EffectiveScale(float Value, bool IsUnbounded)`; `Unbounded`/`At(float)`; `default == Unbounded` | FR-018; consumed by the compositor |
| `ResolutionPolicy` (new type) | `Rendering/ResolutionPolicy.cs` | — | `readonly record struct ResolutionPolicy(ResolutionPolicyKind Kind, float Factor=1f)`; `Inherit`/`ClampToOutput`/`Oversample(k)`/`PreserveSource` | FR-036 |
| `FilterEffect.ResolutionPolicy` / `RenderNode.ResolutionPolicy` | `FilterEffects/FilterEffect.cs` / `Rendering/RenderNode.cs` | — | + `virtual ResolutionPolicy ResolutionPolicy => Inherit` | FR-036; quality effects override (non-breaking; default `Inherit`) |
