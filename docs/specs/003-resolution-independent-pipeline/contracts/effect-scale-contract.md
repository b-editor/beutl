...
- **Anisotropic transforms** (FR-019): a scalar `EffectiveScale` projects onto the most-detailed axis, which can over-allocate; general policy uses `BufferDimensionBudget.ClampWorkingScale`, while allocation contexts exact-check their canonical device footprint (FR-037 backstop).
...
public override void ApplyTo(FilterEffectContext context)
{
    ...
    {
        EffectTarget t = c.CreateTarget(bounds);          // legacy local dimensions after the per-buffer clamp
...
`FilterEffect.Resource.Push` honours `CreateRenderNode()` on every push path. `GetWorkingScaleContract()` keeps the base isolation, transactional `ApplyTo`, typed lowering, publication, and resource transfer. Its policy is folded into the first surviving Shader, Geometry, or legacy operation; it does not create an identity map or an extra pass. A no-item result is a true pass-through and commits neither provisional isolation nor unused owned resources. The hook and resolver remain lazy when `ApplyTo` does not probe `WorkingScale`; an explicit probe can evaluate them even when no item is ultimately authored. The pure contract is reevaluated after a symbolic owning domain resolves. Use `GetWorkingScaleContract()` for a different working density; override `Process` only when the effect needs genuinely different topology or lowering. The engine owns the standard isolation and publication lifecycle rather than exposing partial hooks that can weaken it.

## Working scale — what scale an effect runs at

Built-in effects use the **standard supply-driven working scale `w`** from `RenderScaleContract.MaterializeAtWorkingScale` unless a backend limit requires a narrower declarative contract. There is no closed per-effect policy enum. The standard rule is `w = min( max(s_out, densest concrete supply), MaxWorkingScale )`:
