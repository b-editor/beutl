...
public override void ApplyTo(FilterEffectContext context)
{
    ...
    {
        EffectTarget t = c.CreateTarget(bounds);          // DeviceBufferBounds after grid offset/clamp (CustomFilterEffectContext.cs:150-166)
...
> **Footgun — legacy multi-input `w` is shared, while allocation topology can change.** A dense sibling can still raise the nominal working scale used by every surviving branch, so shrinking a high-resolution source can make the effect more expensive. Before an opaque `CustomEffect`, the dimension clamp uses each branch's actual local-origin footprint and preserves transformed fractional origins; empty space between distant independent branches is not backing storage. The forced compatibility `Flush` immediately before callback entry canonicalizes every surviving target to the device cover of its semantic `Bounds` at the actual working scale, preserving the dimensions unchanged legacy code derives through `CreateTarget(Bounds)` and `DeviceBufferSize(Bounds, WorkingScale)`. `CustomEffect` exposes no combine/split topology, so the planner unions the callback's transformed branch results and conservatively tracks subsequent footprints in that aggregate domain. That can choose a lower safe density when the implementation actually preserves separate targets, but it cannot under-clamp a callback that combines them. Typed Shader/Geometry and pure Skia branch paths remain per-branch. A request-scoped aggregate area/byte budget remains separate from this per-axis safety rule.
