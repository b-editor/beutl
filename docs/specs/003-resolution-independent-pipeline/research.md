...
3. **Working scale `w`** — computed, not stored on the context: each buffer-allocating boundary computes `w = RenderScaleUtilities.ResolveWorkingScale(inputs, OutputScale)` (D7), sizes `ceil(bounds × w)`, opens an `ImmediateCanvas` that **bakes the base CTM `CreateScale(w)`** at construction (feature 003 — no manual push), and tags its emitted value `e = w`. Nested requests forward `OutputScale`.
