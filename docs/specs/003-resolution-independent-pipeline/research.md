...
- **Benchmark (SC-003)**: committed `bench.scene` (1920×1080, ~12 vector shapes w/ fills+strokes, 3 TextBlocks, 2 gradients, Blur+DropShadow chain, 1 nested scene, 1 particle, 1 audio visualizer, 1 3D scene — doubles as the SC-001 representative set). Measure `Render+Snapshot` wall-clock only, warm cache excluded (discard frame 1). Interleave 11 paired 0.5×/1.0× samples and require 0.5× to win at least 9 pairs, which is a one-sided exact sign test with `p < 0.05`; report `median(0.5)/median(1.0)` diagnostically, with ~0.25× documented as the target rather than hard-asserted across backends. Lives in `[Explicit][Category("Benchmark")]` NUnit + a `tests/Beutl.Benchmarks` entry, **not** in the default CI gate.
