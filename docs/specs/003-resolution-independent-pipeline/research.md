...
> **Refined after maintainer review — supersedes the original top-down D1.** The first draft propagated a single top-down render scale on `RenderNodeContext.Scale` that every effect multiplied by and sized buffers at. The maintainer requires **supply-driven** behavior: an intermediate effect runs at its *input's* density — whether **lower** than the output (a 0.5 proxy: do NOT upsample) or **higher** than the output (a 2.0 / 4K source on a 1080 timeline: do NOT downsample, so quality effects can use the detail) — and the output scale is applied **only at the final stage** ("最終的な部分でスケールを調整する"). See **D7** for the policy/negotiation rule.
...
- `Inherit` → `baseline` — supply-driven: `0.5→0.5`, `2.0→2.0`; `s_out` is **not** a ceiling.
...
**Global working-scale ceiling `MaxWorkingScale`** *(maintainer choice: ceiling ON)*: a configurable, **per-render-request** cap (value pinned in tasks; e.g. preview `w ≤ max(input-native, 2 × s_out)` or an absolute cap) applied as the last step of `ResolveWorkingScale`, so no combination of `Inherit`/`Oversample`/preserved high-res sources can blow up worst-case **preview** memory (RgbaF16 is 8 bytes/px; `w²` memory). It caps **only the high side**: it never pulls `w` below a proxy's supply (R1 unaffected) and is inert at `s_out=1.0` with 1.0 inputs (byte-identity unaffected). **Export uses no ceiling (or a much higher one)** so a `PreserveSource` / high-resolution source stays full-fidelity at delivery (FR-013/FR-037) — the ceiling is a preview backstop, not an export constraint.
...
**Final adjustment & SSAA are one machine**: the root device surface is `ceil(FrameSize × s_out)`; an op whose `e ≠ s_out` is Mitchell-resampled **once** at its `DrawSurface`/`DrawRenderTarget` blit onto the root grid (down for a preserved 2.0, up for a forced 0.5). Export-SSAA (`s_out=2.0`) is literally R2: the device grid is `ceil(FrameSize×2.0)` and the `FrameProviderImpl` 2.0→1.0 downscale (D2) is the terminal normalization to delivery resolution.
...
- `PixelRect.FromRect(Rect, float/Vector)` (`PixelRect.cs:391`) = origin `(int)` toward-zero + bottom-right `(int)Math.Ceiling` → asymmetric, growing rect. **This IS the FR-007 shared helper** for the main rasterization sink. The two **filter-effect sinks** (`FilterEffectActivator.cs:29`, `CustomFilterEffectContext.cs:52`) currently do component-wise `(int)width`/`(int)height` truncation (different from `FromRect`'s corner-based rounding) — they must migrate to the shared helper and are **scale-1.0-sensitive** sites that must be golden-tested.
...
- **Downsample (`s>1` export SSAA)**: Mitchell adequate for `s ≤ 2` (pin `2×` as the sweet spot); for `s ≥ 3` switch to trilinear+mipmaps. **Cap offered factor at 2× and use Mitchell** to keep one code path; document `>2×` out of scope.
