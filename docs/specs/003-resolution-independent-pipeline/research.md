...
**Global working-scale ceiling `MaxWorkingScale`** *(maintainer choice: ceiling ON)*: a configurable, **per-render-request** cap (**preview default `2 × s_out`** in `EditorConfig`; **export unbounded**) applied as the last step of `ResolveWorkingScale`, so no combination of `Inherit`/`Oversample`/preserved high-res sources can blow up worst-case **preview** memory (RgbaF16 is 8 bytes/px; `w²` memory). It caps **only the high side**: it never pulls `w` below a proxy's supply (R1 unaffected) and is inert at `s_out=1.0` with 1.0 inputs (byte-identity unaffected). **Export uses a generous but finite ceiling** so a high-resolution source stays full-fidelity at delivery (FR-013/FR-037) — the preview ceiling is a tight backstop, export's is loose. *(Superseded value: as shipped, export = `max(8, 4 × s_out)` (finite, OOM-safe on long renders), NOT "unbounded / +∞" as this historical D7 body originally said — see FR-037.)*
