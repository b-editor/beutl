...
**There is no closed resolution-policy value type.** Built-in/default filter-effect materialization starts with `MaterializeAtWorkingScale`: it runs at the **densest concrete supply, floored at `s_out`**, then is capped by the global ceiling and clamped against the concrete allocation footprint. `Blur` and `DropShadow` additionally cap their working density when needed to keep the largest device sigma at or below 512. The one standard rule (amended 2026-06-15: `s_out` floors **every standard materializing** boundary, not only the vector-only/mixed cases) is:
