...
**There is no resolution-policy value type.** Every buffer-allocating boundary runs at the **densest concrete supply, floored at `s_out`**, capped only by the global ceiling. The one rule (amended 2026-06-15: `s_out` floors **every** boundary, not only the vector-only/mixed cases) is:
...
- Equivalently: `w = min( max(s_out, densest concrete supply), maxWorkingScale )`. `s_out` is the **floor**, **never** a ceiling — a denser concrete supply runs above it (FR-016), and a sub-output concrete supply (`At(0.5)` at a `1.0` export) is lifted to `s_out`. The former special-cased "vector-only fallback" and "C4/C5 mixed bitmap+vector floor at `s_out`" are now instances of this universal floor (conclusions unchanged — they still land at `s_out`). At `s_out = 1.0` with unit-scale / vector inputs `w = max(1, 1) = 1`, so byte-identity is untouched.

- **`ResolutionPolicy` removed (and `FilterEffect.ResolutionPolicy`, `RenderNode.ResolutionPolicy`)**: earlier drafts declared a per-effect policy (`Inherit` / `ClampToOutput` / `Oversample(k)` / `PreserveSource`) to pick `w`. No built-in ever needed a non-default value (all are supply-driven). A narrow exception selects a `PlanFilterEffectRenderNode` through `FilterEffect.Resource.PlanRenderNodeFactory` and overrides `ResolveWorkingScale`, preserving standard plan execution. So the policy enum, `virtual FilterEffect.ResolutionPolicy`, `RenderNode.ResolutionPolicy` (never added — a dead duplicate), the `policy` parameter of `RenderNodeContext.ResolveWorkingScale`, and the earlier `PreserveSource` floor / `preserveFloor` channel were all removed.
...
| feature 004 policy/ownership amendment | public ctor is `RenderNodeProcessor(root, useRenderCache, RenderIntent, outputScale = 1f, maxWorkingScale = +∞, diagnostics = null, pullPurpose = Frame)`; no public pool parameter/property | A nested `Process` implementation calls `RenderNodeContext.CreateChildProcessor(...)`, which inherits the opaque executor-owned pool together with diagnostics, scale, requested bounds, intent, and purpose. |
...
| `Pull` | `new RenderNodeContext(input, RenderIntent, OutputScale, MaxWorkingScale, PullPurpose)` | Single production construction site; reaches all overrides with immutable failure/cache policy. Nested nodes use `CreateChildProcessor` rather than reconstructing this state. |
...
| Member | Change | Rule |
|---|---|---|
| ctor | `(int width, int height)` → **`(int width, int height, RenderIntent renderIntent, float renderScale = 1f, float maxWorkingScale = +∞)`** | width/height stay **logical** FrameSize; device surface = `ceil(FrameSize × renderScale)`. Intent is mandatory and independent of the scale ceiling. BREAKING (FR-028; feature 004 amendment). |
| `OutputScale` | **+ `float OutputScale { get; }`** | Immutable per instance (D4). |
| `FrameSize` | unchanged (logical) | |
| `Render`/`RenderDrawable` | the root `ImmediateCanvas` bakes the base CTM `CreateScale(OutputScale)` at construction (no per-frame push); the FPS overlay re-enters device space via `FpsDrawer.Dispose` → `PushDeviceSpace` so it stays unscaled | |
| `HitTest`/boundary operations | remain in logical coordinates and use purpose-provenance frames | Output scale stays out of hit-test/handle math (FR-027); auxiliary pulls do not mutate the retained frame tree. |

### `SceneRenderer` *(changed)* — `Beutl.ProjectSystem/SceneRenderer.cs`
| Member | Change |
|---|---|
| ctor | **+ mandatory `RenderIntent renderIntent`, then `float renderScale = 1f` and `float maxWorkingScale = +∞`**; forward all policy to `base(scene.FrameSize.Width, scene.FrameSize.Height, renderIntent, renderScale, maxWorkingScale)` and keep `scene.FrameSize` as the logical size. BREAKING. |

### `CompositionFrame` — `Beutl.Engine/Composition/CompositionFrame.cs`
`Size` remains the logical frame size and render scale remains a render-request property (FR-002). **Feature 004 amendment:** the existing three-argument constructor and three-value deconstruction stay unchanged and mean `RenderIntent.Preview` / `RenderPullPurpose.Frame`; a five-argument constructor adds validated, get-only intent and purpose provenance. This is execution-policy metadata, not a scale field. Renderer and composer paths reject frames composed for a different intent, graphics operations additionally validate purpose, and the renderer records the current purpose only after the complete node-tree update succeeds. Frame and auxiliary graphics/audio node caches are separate so an auxiliary resource update cannot reset retained frame state.

`SceneCompositor` keeps one weak `EngineObject.Resource` cache per pull purpose. Re-evaluating an auxiliary frame therefore updates only the auxiliary resource, never the frame resource retained by normal rendering. Per-object generation and per-purpose slot reservations serialize creation/update, but the cache gate protects bookkeeping only: public/virtual `ToResource`, `Resource.Update`, event add/remove, and `Resource.Dispose` always execute outside it. Detach or compositor teardown invalidates the generation without waiting for a busy callback, disposes every idle entry immediately, and transfers a busy entry's exactly-once cleanup to its operation completion. Callback/invalidation failures remain primary over cleanup failures.
...
`EffectTargets`: no scale accessor — `w` is resolved once by `RenderNodeContext.ResolveWorkingScale` and threaded through the activator, so the targets do not derive it. (Earlier drafts' `MaxScale()`/`ResolveScale(...)` were both dropped.) `CalculateBounds` (`:27`) stays logical (scale-invariant).
...
| `TextureSource` / `DrawableTextureSource` *(added 2026-06-15; amended by 004)* | `Graphics3D/Textures/{TextureSource,DrawableTextureSource}.cs` | 003 added `surfaceDensity`; feature 004 changes the abstract signature to `GetTexture(IGraphicsContext, RenderIntent, RenderPullPurpose, float surfaceDensity = 1f)`. `DrawableTextureSource` rasterizes its re-rasterizable `Drawable` at `ceil(authorSize × surfaceDensity)` (clamped by `ClampWorkingScaleToBufferBudget`) and keeps separate frame/auxiliary caches so hit tests cannot replace retained frame state. A decoded-bitmap source ignores density but still validates/accepts the explicit policy. | FR-033/FR-022 |
...
*(No `ResolutionPolicy` type and no `FilterEffect.ResolutionPolicy` — removed; the working scale is supply-driven, and a narrow exception selects a `PlanFilterEffectRenderNode` through `FilterEffect.Resource.PlanRenderNodeFactory` and overrides `ResolveWorkingScale`. See FR-036.)*
...
**Export scale** (FR-034): `OutputViewModel` builds `SceneRenderer(Model, RenderIntent.Delivery, supersampleScale, disableResourceShare: true, maxWorkingScale: WorkingScaleCeiling.Export(), forceOriginalSource: true)`; `FrameProviderImpl.RenderCore` downscales `Snapshot()` to `FrameSize` when `OutputScale > 1`, asserts size == `FrameSize` before encode (FR-026). The explicit delivery intent and original-source routing are independent of preview scale.
