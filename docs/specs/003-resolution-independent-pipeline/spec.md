...
- **FR-037**: A configurable **global working-scale ceiling** MUST cap `w` (`w = min(w, MaxWorkingScale)`) so no combination of preserved high-resolution sources or transform-rescaled densities can exceed a bounded worst-case buffer memory. The ceiling caps only the high side (it never reduces `w` below a proxy's supply) and MUST be inert at `s_out = 1.0` with unit-scale inputs (byte-identity preserved). It is a **per-render-request** value: the **preview default is `2 × s_out`** (seeded at the editor preview `SceneRenderer`) to bound interactive memory; **export uses a generous but finite ceiling** (`max(8, 4 × s_out)`, seeded at `OutputViewModel`) — high enough never to clip a legitimate high-resolution source (so a 4K source still exports at full fidelity, FR-013) yet finite, so a pathological / transform-rescaled density cannot ratchet the working surface without limit and OOM a long render. *(As shipped the ceiling is the **sole** upper bound on the working scale — no built-in resolves a working scale above `s_out` on its own, so in practice it only catches a high-density source or an extreme transform.)*
