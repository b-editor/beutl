...
- **FR-016**: Compositing MUST happen in logical coordinate space, enforced at a single point (the render processor). When a container gathers child operations of heterogeneous effective scale, it MUST composite at `target = max(concrete child effective scales)` (lossless / `Unbounded` children are excluded from the max — they regenerate at the target). The output-scale cap applies **only at the root composite (final normalization)** — no built-in clamps an intermediate to `s_out` (FR-036), so an intermediate composite MAY exceed `s_out` (a preserved high-resolution source / SSAA) and MAY stay below `s_out` (a preserved proxy). The only upper bound on an intermediate is the global memory ceiling (FR-037).
...
- **FR-036**: Each buffer-allocating boundary (effect, container, sink) MUST compute a **working scale `w`** from its inputs' effective scales and run/allocate at `w` — NOT at the output scale `s_out`. The rule is purely **supply-driven**: `w` = the max concrete input density (a 0.5 proxy stays 0.5, a 2.0 source stays 2.0; vector / `Unbounded` inputs impose no supply and fall back to `s_out`, with a mixed bitmap+vector floor at `s_out` so crisp vector siblings are not dragged down), bounded only by the global memory ceiling (FR-037). `s_out` is **not** a ceiling on an intermediate. There is **no per-effect resolution-policy knob** — every built-in runs supply-driven, and an effect that genuinely needs a different working scale (clamp-to-output for perf, oversample for SSAA) overrides `Process` in a `FilterEffectRenderNode` subclass returned from `FilterEffect.Resource.CreateRenderNode()`, computing `w` itself. *(Earlier drafts declared a per-effect `ResolutionPolicy` — `Inherit` / `ClampToOutput` / `Oversample(k)` / `PreserveSource`; no built-in ever needed a non-default value, and a custom render node is strictly more flexible, so the policy type was removed.)* Out-of-tree effects are supply-driven by default and MUST render byte-identically at `s_out = 1.0`.
- **FR-037**: A configurable **global working-scale ceiling** MUST cap `w` (`w = min(w, MaxWorkingScale)`) on the high side (it never reduces `w` below a proxy's supply) and MUST be inert at `s_out = 1.0` with unit-scale inputs (byte-identity preserved). It is a **per-render-request** value: the **preview default is `2 × s_out`** (seeded at the editor preview `SceneRenderer`) to bound interactive working scale; **export uses a generous but finite ceiling** (`max(8, 4 × s_out)`, seeded at `OutputViewModel`) — high enough never to clip a legitimate high-resolution source (so a 4K source still exports at full fidelity, FR-013) yet finite. **Two distinct bounds, do not conflate them** *(clarified 2026-06-09 — Codex reviews #1/#4: the original "bounded worst-case buffer **memory**" wording overclaimed; a working-scale cap does not bound memory, which scales with `bounds × w²`)*: (a) `MaxWorkingScale` bounds the **working scale**; (b) because buffer **memory and the GPU texture limit** scale with `bounds × w` — an **anisotropic** transform projected onto its most-detailed axis (FR-019) inflates the stretched-axis bounds while raising density — a separate per-buffer **dimension** clamp (`RenderNodeContext.ClampWorkingScaleToBufferBudget`, `MaxBufferDimension = 16384`) reduces `w` at the **effect boundary** (`FilterEffectRenderNode`, which is the only sink that allocates by a transform-rescaled per-op density) so the effect buffer stays allocatable. This bounds the density-driven blow-up; it is **not** a total cross-sink memory budget (a single buffer may still approach `16384² × 8 ≈ 2 GiB`, and the non-effect sinks — `RenderNodeProcessor` rasterization, brush/tile intermediates, particles, 3D, the root surface — size by `s_out` / authored dimensions and are not dimension-guarded, an independent pre-existing huge-geometry concern). A request-scoped allocator with backend-reported limits + a byte/area + live-buffer budget is the complete fix and remains a follow-up.
...
- **Working scale (`w`)**: the supply-driven scale a given effect / boundary actually rasterizes at, computed from its inputs' effective scales (FR-036) and capped by the global ceiling (FR-037). Spatial-length parameters multiply by `w` (FR-008).
...
- **Mixed-scale composites at `max` concrete child scale** (confirmed), using Mitchell resampling; the output-scale cap is **deferred to the final root normalization** per the supply-driven model — it does not clamp intermediates (FR-016/FR-036); the only intermediate bound is the global memory ceiling (FR-037).
