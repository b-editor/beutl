...
- **FR-013**: Inherently resolution-sensitive effects (PixelSort, contour-based StrokeEffect / FlatShadow / PartsSplitEffect, AutoClip, integer-structuring Dilate/Erode, MosaicEffect, PerlinNoise-driven effects, and custom SKSL/GLSL shaders) MUST still apply the parameter-scaling contract; their reduced-scale preview MAY differ from the full-scale result and that approximation is accepted. Full fidelity MUST hold at export scale 1.0. (No force-full-scale subtree mechanism and no mismatch-warning UI are required for v1.) These effects MUST declare `PreserveSource` (or `Oversample`) (FR-036) so a higher-resolution source feeding them is kept through the effect and downsampled only at the final stage; declaring `ClampToOutput` on them is forbidden (it would discard the resolution they exist to use).
