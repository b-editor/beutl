...
- **Generated `.gitignore`** (project root; also written inside the project dir in the nested case): `**/.beutl/`, `*.[tT][mM][pP]` (portable case-insensitive `.tmp` matching).
...
- **Commit message**: subject per R-5; Beutl-created commits carry the canonical trailer `Beutl-Snapshot: <kind>`, including `manual` for named milestones.
