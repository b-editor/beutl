...
**Primary Dependencies**: none new — the user's installed `git` (≥ 2.36) as a child process; optional `git-lfs`. No LibGit2Sharp (R-1). Avalonia for the tool tab UI.
...
- **Watcher**: debounce and `.git`/`.beutl`/case-insensitive `.tmp` exclusion (TimeProvider-based); 1000-edit burst asserts bounded status calls (R-8).
