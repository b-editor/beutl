...
6. await ProjectService.CloseProject()
...
While a project is created with tracking or a version-controlled project closes, `EditorService.LifecycleActivity` replaces the editor area with a progress view (`ProjectLifecycleOverlay` in `MainView`), so the user never sees an editor that looks usable while it waits for Git. Creation shows it from `BeginNewProject` until the setup is disposed after the editor has opened. Closing shows it from the start of `ClosingPreparing` when an operation is running, an activation is pending, or the owned backend is tracked, until the close completes or is aborted. Overlapping activities show the most recent one.
