...
public interface IProjectVersionControlService : IDisposable
{
    ...
    Task CreateBranchAsync(string name, CancellationToken ct);
    ...
    Task SetLocalIdentityAsync(string name, string email, CancellationToken ct); // repo-local only
...
4. **`RestoreWorktreeFromAsync`**: `git restore --source=<sha> --worktree -- <pathspec>` then `git clean -fd -- <pathspec>`. Precondition (enforced by the coordinator, asserted by the service): the project is closed. Ignored files (`.beutl/`, `*.tmp`) survive the clean.
...
9. **Errors**: git non-zero exits surface as `GitOperationException { ExitCode, Stderr }` with stderr preserved verbatim for the error dialog; remote operations map to `RemoteOpResult` instead of throwing for the four expected outcomes.
