...
**All 13 in-scope effects propagate `CorrectionScale` correctly through the pipeline**, and 11 also adjust their length-typed authored parameters at action time so the shader / pen / draw semantics match authoring intent.

A two-stage SSIM divergence investigation hit two engine bugs that have since been fixed:

1. **Wrong `CorrectionScale` on the primitive-filter Lambda branch.** `FilterEffectRenderNode.Process`'s `builder.HasFilter()` branch was emitting an operation tagged with `unifiedScale`, which made the compositor apply a second `Scale(scale, scale, op.Bounds.X, op.Bounds.Y)` push. But that Lambda materialises via `PushPaint` / `SaveLayer`, which Skia allocates at the compositor's output canvas scale — the content was already full-resolution. The duplicate Scale push (around the filter-extended bounds, which sit at negative coords for halo-extending filters like Blur and DropShadow) shifted the rendered content off-canvas. **Fix**: the `HasFilter` branch now emits the Lambda with the factory default `Identity`. The no-filter branch keeps `unifiedScale` because those RTs do live at upstream proxy scale.
2. **`EffectTarget.Draw` ignored upstream `CorrectionScale`.** When `t.Draw(canvas)` calls `op.Render(canvas)` for a `NodeOperation`-backed `EffectTarget`, the wrapped upstream's proxy scale was not being honored inside the filter Lambda's `PushPaint` scope. The upstream raster was drawn raw, so it appeared at 1/4 size in the corner of the SaveLayer. **Fix**: `EffectTarget.Draw` now pushes the same `Scale(scaleX, scaleY, op.Bounds.X, op.Bounds.Y)` matrix that the top-level compositor uses, before invoking `op.Render`. At Identity this is a no-op (preserving pre-Phase 3 behavior); at proxy it expands the small raster to fill the filter's authoring bounds inside the SaveLayer.

After both fixes, the proxy and full-resolution outputs land at the same logical coordinates (centre pixel sample at (50, 50) is now `BGRA(255, 255, 255, 255)` for both Identity and Proxy upstream with Blur). Remaining SSIM divergence at proxy 4× is purely the bicubic-upscale boundary-softening artifact on the pre-rasterized ellipse fixture:

| Effect | SSIM | Effect | SSIM |
|---|---|---|---|
| Blur | 0.52 | MosaicEffect | 0.52 |
| DropShadow | **0.90** | ColorShift | 0.54 |
| InnerShadow | 0.53 | ShakeEffect | 0.53 |
| Erode | 0.85 | SplitEffect | 0.47 |
| Dilate | 0.83 | Clipping | 0.53 |
| FlatShadow | 0.46 | StrokeEffect | 0.55 |

The SC-001 ≥ 0.97 envelope is still not met for most effects with the current proxy 4× and single-ellipse fixture, but the empirical floor is now legitimately set by the **bicubic resampling smoothing the upstream raster's edges**, not by a geometric misplacement bug. Improving these numbers further requires either a softer test fixture (gradient instead of binary ellipse) or accepting a per-effect threshold below 0.97. The harness logs the values via `TestContext.Out`; no threshold is asserted yet pending a follow-up that decides the production SSIM envelope. The 2 contour-trace effects took different conventions:
