...
**All 13 in-scope effects propagate `CorrectionScale` correctly through the pipeline**, and 11 also adjust their length-typed authored parameters at action time so the shader / pen / draw semantics match authoring intent. The remaining gap is a **rendering-path mismatch** for primitive filters: `FilterEffectContext`'s length-typed helpers divide their authored params assuming the resulting `SKImageFilter` will run on an upstream-scale `SaveLayer`, but the actual rendering applies the filter via `PushPaint` / `SaveLayer` at the compositor's full-resolution output canvas. The divided sigma therefore under-blurs at non-Identity upstream when the primitive chain runs the no-custom-effect path. Empirical SSIM measurements via `tests/Beutl.UnitTests/Engine/Graphics/FilterEffects/ResolutionEquivalenceTests.cs` confirm the divergence (Blur SSIM ≈ 0.17 at 4× proxy, DropShadow ≈ 0.47, others 0.78–0.91). The SC-001 ≥ 0.97 claim cannot be honored for the primitive-filter path without restructuring `FilterEffectRenderNode.Process` to materialize the filter at upstream scale (e.g., always force-flush before emitting output). That restructure is **out of scope for the current PR** (preserve-existing-behavior directive); the SSIM harness is in place so it becomes a one-line threshold change once the engine fix lands. The 2 contour-trace effects took different conventions:
