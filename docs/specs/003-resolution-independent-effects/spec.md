...
- Q: Does the scope include non-FilterEffect pixel-absolute rendering primitives — `Transform` translations, `Pen.Thickness`, `Shape.Width/Height`, direct `GraphicsContext2D.DrawRectangle / PushTransform / PushClip` calls? → A: **Yes** for the enumerated raster-space surfaces above. **No** for `Geometry` path coordinates, `TextBlock.Size / Spacing`, and `Brush` interior rectangles — those remain raw-raster in this PR and are tracked as separate follow-ups. The summary phrasing "end-to-end" elsewhere in this spec refers specifically to "the enumerated raster-space surfaces", not "every rendering API in Beutl".
- Q: Which scaling site does Transform use? → A: **Render-node application time** (inside `ImmediateCanvas.PushTransform`), not materialization (`Transform.CreateMatrix`) and not API call site (`GraphicsContext2D.PushTransform`). `TransformRenderNode` stores the authoring-space matrix; `ImmediateCanvas` multiplies the translation column by `RenderScale` when pushing to the underlying `SKCanvas`. The `*Raw` opt-out is implemented as an `IsRaw` flag on the node that bypasses scaling at application time. Rationale: avoids `CompositionContext.RenderScale` propagation entirely, makes custom `Transform` subclasses automatically benefit, keeps `Transform.Resource.Matrix` semantically authoring-space, and lets `RenderNodeCache` survive `RenderScale` changes. See `research.md` § R10 and `contracts/transform-scaling.md`.
...
- **Plugin / user-authored effects**: see `contracts/effect-helper-scaling.md` § "Plugin author migration (authoritative cross-surface table)" for the single source of truth. In summary: plugins that call standard helpers (`FilterEffectContext`, `GraphicsContext2D` direct API, `PushTransform`) and custom `Transform` subclasses automatically benefit on rebuild; plugins that read `pen.Thickness` directly in a rendering path need a one-line switch to `PenHelper.GetScaledThickness`; opt-out for any surface is the `*Raw` twin (Transform / GraphicsContext2D / FilterEffectContext) or keeping the raw `pen.Thickness` read (Pen).
...
- **FR-002**: The exact in-scope surface MUST be enumerated in `data-model.md` and each entry MUST be matched by a test. At a minimum the surface includes:
  ...
  - **`GraphicsContext2D` direct API** — `DrawRectangle(Rect)`, `DrawEllipse(Rect)`, `PushClip(Rect)`, `PushLayer(Rect)`, `PushOpacityMask(..., Rect)` (all scale Rect at API call time); `PushTransform(Matrix)` and `PushTransform(Transform.Resource)` defer scaling to `ImmediateCanvas.PushTransform` (render-node application time).
...
- **FR-008**: The system MUST apply resolution-independent scaling implicitly inside the existing length-taking rendering API entry points:
  ...
  - **`GraphicsContext2D` direct helpers (Rect-typed)** — `DrawRectangle(Rect)`, `DrawEllipse(Rect)`, `PushClip(Rect)`, `PushLayer(Rect)`, `PushOpacityMask(..., Rect)`. Each scales the `Rect` per axis at API call time.
  - **`Pen` consumption** — `pen.Thickness`, `DashOffset`, `Offset` are scaled at each rendering call site through `PenHelper.GetScaledThickness / GetScaledDashOffset / GetScaledOffset`. `Pen.Resource` properties stay raw. `MiterLimit`, `TrimStart`, `TrimEnd`, `TrimOffset` stay raw at all times. Bounds calculations must use the new `PenHelper.GetScaledBounds(rect, pen, scale)` when they live in render space; project-space callers keep using `GetBounds`. See `contracts/pen-scaling.md` § "Project-space vs render-space bounds".
  - **`Transform`** — recorded into `TransformRenderNode` verbatim (authoring-space). `ImmediateCanvas.PushTransform(matrix, op, isRaw)` multiplies the translation column by `RenderScale` at render-node application time unless `isRaw` is set. `Transform.CreateMatrix` itself is unchanged. The `PushTransform` overloads on `GraphicsContext2D` (both `Matrix` and `Transform.Resource`) are the one exception to "scale at the API call site" — they defer to render-node application time. See `contracts/transform-scaling.md` and `research.md` § R10.

  The same system MUST also expose a `*Raw` variant of every scaled API entry point (e.g. `BlurRaw(Size)`, `DrawRectangleRaw(Rect)`, `PushTransformRaw(Matrix)`, `PushClipRaw(Rect)`) that bypasses the scaling step, for the niche case where a caller needs raw-raster pixel semantics. For the Transform path the opt-out is implemented as `IsRaw = true` on the produced `TransformRenderNode`. For Pen the opt-out is simply continuing to read `pen.Thickness` directly without going through `PenHelper.GetScaled*`.

  **Automatic vs manual upgrade behavior** (full table in `contracts/effect-helper-scaling.md`):
  - **Automatic on rebuild** — code that calls `FilterEffectContext` helpers, `GraphicsContext2D` Rect helpers (`DrawRectangle` / `DrawEllipse` / `PushClip` / `PushLayer` / `PushOpacityMask`), `PushTransform(Matrix | Transform.Resource)`, or any built-in / custom `Transform` subclass. No source change required.
  - **Manual one-line switch** — code that reads `pen.Thickness` / `DashOffset` / `Offset` directly in a rendering path. The switch is to `PenHelper.GetScaledThickness(pen, renderScale)` etc. Project-space callers (bounding-box math, animation evaluation) continue to read `pen.Thickness` raw.
