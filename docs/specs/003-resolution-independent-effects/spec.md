...
### Session 2026-05-22 (Phase 3 implementation note)

- Q: When the contract said "no `FilterEffectContext` modification", was that achievable against the real code? → A: **No.** `FilterEffectContext` builds `SKImageFilter` instances through closed factory lambdas embedded in `FEItem_Skia<T>`, and there is no external injection point for dividing length-typed parameters. The directive adopted during Phase 3 splits effects into two categories: **primitive-based** effects benefit automatically via `FilterEffectContext` (which now carries `CorrectionScale` and divides at the call site of `Blur` / `DropShadow` / `InnerShadow` / `Transform` / `Erode` / `Dilate`); **CustomEffect-based** effects opt in by reading `CustomFilterEffectContext.CorrectionScale` from their action. See `data-model.md` § "Implementation deviation" for the full statement and `tasks.md` for the per-task split.
...
- **FR-006**: The 13 in-scope built-in effects MUST participate in per-clip proxy. Split into two sub-categories per the Phase 3 implementation directive (2026-05-22):
  - **Pure-primitive effects** (`Blur`, `DropShadow`, `InnerShadow`, `Erode`, `Dilate`) — participate **automatically** with **zero source modification**. `FilterEffectContext`'s length-typed primitives divide authored parameters by `CorrectionScale` internally.
  - **CustomEffect-based effects** (`StrokeEffect`, `ColorShift`, `DisplacementMapTransform` × 3, `FlatShadow`, `Clipping`, `SplitEffect`, `ShakeEffect`, `MosaicEffect`) — participate **with effect-class source modification**. The effect's `ApplyTo` reads `CustomFilterEffectContext.CorrectionScale` and divides its own length-typed parameters before invoking Skia.
...
- **FR-008**: Plugins / extensions written against the new `Beutl.Engine` MUST NOT have to change their authoring code to benefit, **with one exception**: `CustomEffect`-based filter implementations (effects whose `ApplyTo` calls `FilterEffectContext.CustomEffect(...)`) must opt in by reading `CustomFilterEffectContext.CorrectionScale` and applying the division themselves — adding a single property read at the start of their custom action. Other authoring surfaces (`IProperty<T>`, primitive `FilterEffectContext` helpers like `Blur` / `DropShadow`, `GraphicsContext2D` direct draw, `Pen`, `Transform` subclass `CreateMatrix`, `Shape.Render`, etc.) stay unchanged. The Phase 3 implementation note in `data-model.md` records the constraint that led to this partial relaxation.
