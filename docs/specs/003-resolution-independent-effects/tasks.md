...
- [~] T010 [US1] **Skipped (deferred to follow-up feature).** Original intent: modify `src/Beutl.Engine/Graphics/Rendering/DrawableRenderNode.cs` (and / or whatever class produces the operation for a Drawable's render pass, including `SceneDrawable`'s sub-scene path) to allocate the target raster at `bounds.PixelSize × scaleRatio`, construct the inner `ImmediateCanvas` with `SKCanvas.Scale(1 / scaleRatio)` pre-applied, run the inner render pass in authoring space, and declare `CorrectionScale = scaleRatio` on the produced operation. **Why skipped**: Type B sub-canvas proxy depends on the still-undefined per-clip `ProxyConfig` schema (out of scope per `spec.md` § Future Work) and on a non-trivial `SceneDrawable` / `Drawable.Resource` audit that touches public-API extension points. Without those, any implementation would either be a no-op pass-through (= equivalent to current behavior) or require placeholder design decisions that the follow-up "per-clip proxy UX + persistence" feature will need to revisit anyway. Tracked for the follow-up feature.
...
- [X] T021 [P] [US1] Add SSIM helper in `tests/Beutl.UnitTests/Engine/Graphics/Testing/SsimHelper.cs`.
- [X] T022 [P] [US1] Add bicubic upscale helper in `tests/Beutl.UnitTests/Engine/Graphics/Testing/BicubicResampler.cs`.
- [~] T023 [US1] **Partial.** Harness implemented and exercised against the 13 in-scope effects; SSIM values are logged via `TestContext.Out`. The SC-001 ≥ 0.97 threshold assertion is **deferred** — the current primitive-filter path applies the SKImageFilter via the compositor's full-resolution `SaveLayer` rather than at upstream raster scale, while the primitive helpers in `FilterEffectContext` already divided their length params for an upstream-scale surface, so the proxy path visually under-blurs. Resolving this requires a non-trivial restructure of `FilterEffectRenderNode.Process` (preserve-existing-behavior was the explicit directive for this PR). See `tests/Beutl.UnitTests/Engine/Graphics/FilterEffects/ResolutionEquivalenceTests.cs` class remarks for the full analysis. Add `ResolutionEquivalenceTests.cs` in `tests/Beutl.UnitTests/Engine/Graphics/FilterEffects/`: parameterized over the 13 in-scope effect fixtures. For each, render the fixture twice — once with proxy enabled on a synthetic source (injected `CorrectionScale = (4, 4)`), once at full — and assert SSIM ≥ 0.97 between proxy-after-compositor-blit and the full render.
- [~] T024 [P] [US1] **Partial.** A single synthesized test pattern (ellipse on transparent background, `Rect(15, 20, 70, 60)` filled white) is shared across the 13 effect harness exercises in `ResolutionEquivalenceTests`. The per-effect JSON fixture file format is **deferred** along with T023's SSIM assertions — once the engine restructure lands and proper thresholds are achievable, the fixtures can be split into individual JSON files. Add per-effect fixtures under `tests/Beutl.UnitTests/Engine/Graphics/Fixtures/ResolutionIndependence/`: 13 effects × 1 fixture each (Blur, DropShadow, InnerShadow, StrokeEffect, Erode, Dilate, FlatShadow, ColorShift, DisplacementMapTransform × 3, MosaicEffect, ShakeEffect, SplitEffect, Clipping).
