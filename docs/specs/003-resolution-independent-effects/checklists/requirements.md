...
- **Codex 4th review fixes (2026-05-22)**: Two CRITICAL findings resolved. (C1) `RenderScale` numeric convention pinned: `CorrectionScale = bounds.Size / raster.Size` ≥ 1, the bounds-over-raster upscale ratio. API renamed `Apply*` → `ToRaster*` / `ToAuthoring*` to remove direction ambiguity. (C2) Multi-upstream / hybrid node policy moved from "TBD during audit" to fixed authoritative contracts in `transformer-node-scale-handling.md` — Pattern X (independent-children container, forward verbatim) and Pattern Y (multi-input compositing, unify at MAX upstream `CorrectionScale`). MEDIUM findings: (M1) `Beutl.Graphics3D` / audio / `Beutl.NodeGraph` explicit out-of-scope statements added to `spec.md` § Assumptions; (M2) FR/SC coverage map added to `tasks.md` so every requirement has at least one verifier task.
