...
1. **Given** a request to list editable content types, **When** the agent queries the capability surface, **Then** it receives every built-in element/drawable/effect/encoder type that the GUI can edit, each with its parameters and the parameters' types, units, valid ranges, and defaults.
...
- **FR-006**: The toolkit MUST let an agent enumerate the available element/drawable types, filter effects, and encoders, including for each one the editable parameters and each parameter's type, unit, valid range, and default value.

**Editing**

- **FR-007**: The toolkit MUST let an agent read and write any editable property of a scene, element, drawable, or effect — visual or audio — enforcing the property's declared validity (range/units) and reporting — never silently swallowing — any rejection or coercion.
...
- **FR-028**: Partial declarative edits MUST be expressible as a JSON Merge Patch — a partial subtree in which a null member deletes — applied against the current document, and the toolkit MUST also accept a full desired-state document. Object members follow RFC 7396; **arrays of identity-bearing entities** (elements, content objects, keyframes) MUST be applied by **id-keyed merge** (members matched by `Id`, unmentioned members left untouched, an explicit deletion marker or imperative remove deletes one — deleting an already-absent member is an idempotent no-op) so changing one element never drops its siblings — only scalar / non-identified arrays follow strict RFC 7396 wholesale replacement. Ordering MUST be deterministic: existing member order is preserved, explicit position directives (`$index`/`$after`/`$before`) reposition, and keyframes are ordered by time. An agent MUST NOT have to reconstruct unrelated state in order to change one field.
...
- **Property**: A named, typed, editable attribute of a scene/element/drawable/effect, carrying validity metadata (type, unit, valid range, default) and optionally an animation.
...
- **SC-003**: Across a representative project set, opening a project and saving it back with no edits preserves 100% of project content (zero data loss / zero silently dropped content).
- **SC-004**: For the **v1 capability scope** (defined in Assumptions — element types image/video/text/shape/group/audio-source; all schema-discoverable properties; keyframe add/edit/remove + easing; visual-filter and audio effects attach/configure/reorder/remove; structural move/resize/duplicate/split/group), an agent can perform at least 90% of the editing operations a human can perform through the GUI *within that scope*.
