...
- **Undo scope is the Scene**: the unit that carries an undo history (one `HistoryManager`) is a Scene. Declarative reconcile/plan/apply operates against a Scene root. Project-level operations (create project, add/remove scenes, project variables) are separate, coarser, file-level actions and are not part of a scene's undo stack.
