...
- **Input**: `{ "outputPath": string, "timeSeconds"?: number, "renderScale"?: number, "confirmOverwrite"?: bool, "returnImageContent"?: bool }` (write — **guarded**). Bare filenames are written under `agent-output/`; explicit relative directories and absolute in-workspace paths are preserved. `returnImageContent` defaults to `false`.
...
- **Input**: `{ "shots"?: [ { "name": string, "timeSeconds": number } ], "outputDirectory"?: string, "basename"?: string, "renderScale"?: number, "confirmOverwrite"?: bool, "background"?: bool, "returnImageContent"?: bool, "subdivisionLevel"?: number }` (write — **guarded**). `subdivisionLevel` is an integer clamped to `0..3` and defaults to `0`. Level `0` is the current one-frame-per-shot behavior. Level `1` adds the midpoint of each adjacent resolved shot pair; level `2` adds 1/4, 1/2, and 3/4 points; level `3` adds 1/8 multiples. Generated times are deduplicated with about a half-frame tolerance at the scene frame rate and kept chronological. When `shots` is omitted, one representative time is auto-derived per timeline `Element` (the midpoint of its `[Start, Start+Length)`), deduplicated and ordered before subdivision. Stills and the contact sheet are written under `agent-output/` by default. `returnImageContent` defaults to `false` and cannot be combined with `background=true`. The total rendered frame count is capped at 48; requests above the cap return a `validation_rejected` error asking the agent to lower `subdivisionLevel` or narrow the shot set.
...
- **Input**: `{ "videoType"?: "motion-graphics"|"footage-cut"|"slideshow"|"lyric-captions"|"logo-intro", "timeSeconds"?: number[], "sampleCount"?: number, "renderScale"?: number, "styleProfile"?: string, "allowAllCaps"?: bool, "allowHardCuts"?: bool, "allowRectDominance"?: bool, "relaxAesthetics"?: bool, "allowStillness"?: bool, "allowDenseText"?: bool, "allowMultiObjectElements"?: bool, "allowMonochrome"?: bool, "allowMinimalDensity"?: bool, "plannedForegroundElementsPerShot"?: number, "beatTimesSeconds"?: number[], "paletteRoleColors"?: [ { "role": string, "color": "#RRGGBB" } ] | string, "staticLayout"?: bool }`. When `videoType` is omitted, behavior is issue-for-issue identical to the legacy motion-graphics analyzer. Explicit profiles OR their implied intent flags with caller-provided flags and record the resolved profile/adjustments in `reviewNotes`. Unknown values return `validation_rejected`. When `timeSeconds` is omitted, the tool samples evenly across the scene duration. `plannedForegroundElementsPerShot` is the `quantitativePlanSheet` density target; when it is `> 0`, motion-graphics authored density below half of that target can produce a Major `layerDensity` issue unless `allowMinimalDensity` or a minimal/negative-space role tag is present. `beatTimesSeconds`, when supplied, enables advisory-only `audioSync` cut-boundary checks; omit it to keep existing behavior. `paletteRoleColors`, when supplied and rendered sampling runs, buckets sampled pixels to the nearest role color and records `metrics.paletteBalance`; omit it to keep existing behavior. Prefer an array of `{ "role": "...", "color": "#RRGGBB" }` objects; a JSON string containing that same array is accepted for MCP clients that cannot send a complex array reliably. Malformed values return `validation_rejected`. `staticLayout=true` is the storyboard-phase gate: it skips rendered-motion and rendered-text-contrast checks (`evaluateMotion=false`) so a motionless storyboard is judged on composition, document typography, density, palette, background richness, structure, and document-only transition vocabulary — no Major `motionContinuity` or rendered `typographyContrast` blocker.
...
- **Input**: same intent/profile/videoType flags as `evaluate_edit_quality`, except render sampling is omitted. `plannedForegroundElementsPerShot` can be supplied for document-only density plan checks.
- **Output**: same shape as `evaluate_edit_quality`, with `metrics.motionContinuity.motionEvaluated=false`.
- **Use when**: before a large `apply_edit` or immediately after an intermediate stage, so an agent can catch copy density, foreground rectangle, unclear-shape, missing-motion-intent, multi-object Element, high-tempo rhythm, backing-plate, palette, effect-stack, easing/motion-uniformity, logo-intro arc, and timeline-structure risks before paying render/export cost.

### `suggest_quality_fixes`
Group current quality issues into minimal patch-oriented repair suggestions.
- **Input**: `{ "videoType"?: "motion-graphics"|"footage-cut"|"slideshow"|"lyric-captions"|"logo-intro", "includeMotion"?: bool, "timeSeconds"?: number[], "sampleCount"?: number, "renderScale"?: number, "styleProfile"?: string, "requireAnimatedProperties"?: bool, "allowAllCaps"?: bool, "allowHardCuts"?: bool, "allowRectDominance"?: bool, "relaxAesthetics"?: bool, "allowStillness"?: bool, "allowDenseText"?: bool, "allowMultiObjectElements"?: bool, "allowMonochrome"?: bool, "allowMinimalDensity"?: bool, "plannedForegroundElementsPerShot"?: number }`.
