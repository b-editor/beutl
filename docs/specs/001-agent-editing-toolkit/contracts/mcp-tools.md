...
- **Output**: `{ "passesQualityGate": bool, "verdict": string, "issues": [ { "category": "typography|typographyReadTime|typographyContrast|audioSync|visualHierarchy|shapeDiversity|shapeIntent|motionIntent|elementStructure|layerDensity|tempoRhythm|textBackgroundFit|paletteHarmony|paletteBalance|transitionVocabulary|backgroundRichness|materialUiLook|effectIntent|easingDiversity|motionUniformity|motionArc|motionContinuity|cutRhythm|timelineCoverage", "severity": "critical|major|minor", "message": string, "evidence": string, "suggestedFix": string, "time"?: string, "elementIds": string[], "objectIds": string[] } ], "metrics": { "typography": { ..., "lowContrastTextCount": number, "contrast": [ { "elementId": string, "objectId": string, "text": string, "worstContrastRatio": number, "worstTime": string, "decorativeIntent": bool, "samples": [ { "time": string, "contrastRatio": number } ] } ] }, "shapeDiversity": { ... }, "palette": { "harmonyScore": number, "harmonyScheme": string, "hueRelationshipScore": number, "saturationBalanceScore": number, "lumaBalanceScore": number, ... }, "paletteBalance"?: { "sampledPixelCount": number, "roleShares": [ { "role": string, "color": "#RRGGBB", "share": number } ], "neutralShare": number, "accentFlooded": bool, "dominantBelowFloor": bool }, "transitionVocabulary"?: { "histogram": { "<type>": number }, "boundaries": [ { "timeSeconds": number, "type": "hard-cut|dissolve|sweep|dip-to-color", "evidence": string, "elementIds": string[] } ] }, "structure": { ... }, "tempo": { ... }, "backgroundRichness": { ... }, "layerDensity": { "bands": [ { "startSeconds": number, "endSeconds": number, "visibleLayerCount": number, "foregroundLayerCount": number, "hasBackground": bool, "hasMidground": bool, "hasForeground": bool } ], ... }, "motionCraft": { "easingDiversity": { "animatedTransitionCount": number, "linearTransitionCount": number, "linearTransitionShare": number }, "motionUniformity": { "animatedElementCount": number, "largestClusterElementCount": number, "largestClusterShare": number, "largestClusterStartSeconds"?: number, "largestClusterDurationSeconds"?: number, "largestClusterDirection"?: string }, "motionArc": { "evaluated": bool, "dominantAnimatedElementCount": number, "hasAnticipation": bool, "hasSettle": bool, "holdSeconds": number } }, "motionContinuity": { ... } }, "reviewNotes": string[] }`.
- **Use when**: after `render_still` and `evaluate_motion_variation`, and before `export_video`. Pass `staticLayout=true` for the document-only pass during Phase 1. Critical or major issues are limited to unreadable text and malformed Element structure unless the user explicitly accepts them. The deterministic quality categories that can produce gate-failing Major issues are the typography blocker family (`typographyReadTime` and rendered `typographyContrast`), `elementStructure`, `motionContinuity`, and `layerDensity` when it is a supplied-plan density violation.
- **Detects**: long all-caps text, excessive tracking, low rendered text/background contrast, foreground `RectShape` dominance, unclear large/animated foreground shapes, animated shapes without motion intent, ordinary Elements with multiple EngineObjects, sparse layer density/depth coverage, sparse 120-140 BPM/high-tempo rhythm, optional beat-grid cut boundaries that land 40-120 ms away from a supplied beat, optional 60-30-10 palette role-balance drift when `paletteRoleColors` is supplied, transition vocabulary inconsistency for slideshow/footage-cut cuts, misaligned text backing plates, dark teal/cyan/magenta, oversaturated or low-scoring hue-wheel palettes, flat single-layer backgrounds, outdated card/shadow/blur styling, all-linear easing monotony, uniform start/duration/direction motion clusters, logo-intro anticipation/settle arc gaps, low rendered motion variation, and unmotivated hard-cut rhythm.
...
- **Use when**: an agent thinks a scene is finished. Pass the resolved `videoType` used during planning. For motion-graphics deliverables, pass `requireAnimatedProperties=true` so `animatedPropertyCount=0` blocks export even when rendered samples changed, pass `plannedForegroundElementsPerShot` from `quantitativePlanSheet` so `metrics.layerDensity` can enforce the below-half plan-violation blocker, and pass `beatTimesSeconds` from `analyze_audio_rhythm` when a music bed exists. Pass `allowMinimalDensity=true` only for a recorded minimal/negative-space brief. Pass `staticLayout=true` in the storyboard phase: it skips the motion analyzer, rendered contrast, and motion-required blockers, forces `requireAnimatedProperties=false`, and reports `readyForStoryboard` (never `readyForExport`) — use it to confirm a static storyboard before adding motion.
