...
- **Notes**: every `apply_edit` / `duplicate_object` / `group_elements` mutation is one transaction. Pending debounced editor edits are flushed before the stack is inspected, so a transaction in flight is not missed. In a LiveEditor session the stack is the editor's own, so a step can revert a human's edit — read `nextUndo` first when that matters. File-backed sessions still need `save_project` to persist the reverted state.
- **Errors**: `no_active_editor_session`.
...
- **Advisory-only categories**: `paletteBalance` cites the 60-30-10 rule / Itten contrast of extension and reports when an accent role exceeds about 20% of sampled area or a dominant background role falls below about 40%; it is skipped entirely for `footage-cut` and absent when `paletteRoleColors` is omitted or rendered sampling does not run. `transitionVocabulary` classifies cut boundaries as `dissolve`, `sweep`, `dip-to-color`, or `hard-cut`; for `slideshow` and `footage-cut`, it advises consolidating to one transition vocabulary when more than two non-hard-cut types are mixed or hard cuts and styled transitions alternate with no majority type. `allowHardCuts` or `relaxAesthetics` suppresses the transition-vocabulary advisory. Neither category is a blocker.
- **Video-type adjustments**: `slideshow` implies `allowStillness` and `allowMinimalDensity`, skips tempo/beat-grid advisories, frames cut-rhythm as transition consistency, enables `timelineCoverage`, and enables `transitionVocabulary`. `footage-cut` forces motion-graphics intent off, suppresses background-richness/layer-density motion-graphics advisories, skips tempo unless `styleProfile` explicitly signals high tempo, skips `paletteBalance`, and enables `timelineCoverage` plus `transitionVocabulary`. `lyric-captions` implies `allowMinimalDensity`, skips tempo/background-richness, and suppresses hierarchy overload caused purely by caption-role text. `logo-intro` implies `allowMinimalDensity`, suppresses tempo/cut-rhythm advisories; low motion is advisory in every profile, and `allowStillness` only rewords the finding as expected rather than unexpected. Passed. `timelineCoverage` is advisory-only and reports visible-element gaps longer than 0.5 seconds for footage-cut and slideshow.
...
- **Input**: `{ "videoType"?: "motion-graphics"|"footage-cut"|"slideshow"|"lyric-captions"|"logo-intro", "includeMotion"?: bool, "timeSeconds"?: number[], "sampleCount"?: number, "renderScale"?: number, "styleProfile"?: string, "requireAnimatedProperties"?: bool, "allowAllCaps"?: bool, "allowHardCuts"?: bool, "relaxAesthetics"?: bool, "allowStillness"?: bool, "allowDenseText"?: bool, "allowMultiObjectElements"?: bool, "allowMonochrome"?: bool, "allowMinimalDensity"?: bool, "plannedForegroundElementsPerShot"?: number }`.
...
- **Input**: `{ "videoType"?: "motion-graphics"|"footage-cut"|"slideshow"|"lyric-captions"|"logo-intro", "outputPrefix"?: string, "timeSeconds"?: number[], "sampleCount"?: number, "renderScale"?: number, "styleProfile"?: string, "requireAnimatedProperties"?: bool, "allowAllCaps"?: bool, "allowHardCuts"?: bool, "relaxAesthetics"?: bool, "allowStillness"?: bool, "allowDenseText"?: bool, "allowMultiObjectElements"?: bool, "allowMonochrome"?: bool, "allowMinimalDensity"?: bool, "plannedForegroundElementsPerShot"?: number, "beatTimesSeconds"?: number[], "paletteRoleColors"?: [ { "role": string, "color": "#RRGGBB" } ] | string, "staticLayout"?: bool, "confirmOverwrite"?: bool }` (still-frame writes are guarded). Bare prefixes are written under `agent-output/`. Omitting `outputPrefix` uses a collision-free session/process token; explicit prefixes preserve exact filenames. `paletteRoleColors` is forwarded to the embedded `evaluate_edit_quality` pass when rendered sampling runs; use the same array-or-JSON-string shape documented for `evaluate_edit_quality`.
- **Output**: `{ "readyForExport": bool, "readyForStoryboard": bool, "blockers": string[], "advisories": string[], "stillFrames": [ { "outputPath": string, "time": string, "warnings": string[], "visibilityAnalysis": { ... }, "activeElements": [ ... ] } ], "motion": <evaluate_motion_variation output | null when staticLayout>, "quality": <evaluate_edit_quality output>, "recommendedNextTool": "export_video|render_storyboard|suggest_quality_fixes" }`. `blockers` contains only unreadable text, malformed Element structure, and caller-requested checks such as `requireAnimatedProperties`; motion/visibility and other non-blocking findings are summarized in `advisories`.
...
- **Errors**: `stale_handle` (typed — no cached rendered quality baseline exists for the session), `rendering_unavailable`, `workspace_boundary`, `destructive_intent`.
