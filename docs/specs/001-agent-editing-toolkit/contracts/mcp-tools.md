...
- **Input**: `{ "videoType"?: "motion-graphics"|"footage-cut"|"slideshow"|"lyric-captions"|"logo-intro", "timeSeconds"?: number[], "sampleCount"?: number, "renderScale"?: number, "styleProfile"?: string, "allowAllCaps"?: bool, "allowHardCuts"?: bool, "allowRectDominance"?: bool, "relaxAesthetics"?: bool, "allowStillness"?: bool, "allowDenseText"?: bool, "allowMultiObjectElements"?: bool, "allowMonochrome"?: bool, "allowMinimalDensity"?: bool, "plannedForegroundElementsPerShot"?: number, "staticLayout"?: bool }`. When `videoType` is omitted, behavior is issue-for-issue identical to the legacy motion-graphics analyzer. Explicit profiles OR their implied intent flags with caller-provided flags and record the resolved profile/adjustments in `reviewNotes`. Unknown values return `validation_rejected`. When `timeSeconds` is omitted, the tool samples evenly across the scene duration. `plannedForegroundElementsPerShot` is the `quantitativePlanSheet` density target; when it is `> 0`, motion-graphics authored density below half of that target can produce a Major `layerDensity` issue unless `allowMinimalDensity` or a minimal/negative-space role tag is present. `staticLayout=true` is the storyboard-phase gate: it skips rendered-motion checks (`evaluateMotion=false`) so a motionless storyboard is judged on composition, typography, density, palette, background richness, and structure only — no Major `motionContinuity` blocker.
- **Output**: `{ "passesQualityGate": bool, "verdict": string, "issues": [ { "category": "typography|typographyReadTime|visualHierarchy|shapeDiversity|shapeIntent|motionIntent|elementStructure|layerDensity|tempoRhythm|textBackgroundFit|paletteHarmony|backgroundRichness|materialUiLook|effectIntent|motionContinuity|cutRhythm|timelineCoverage", "severity": "critical|major|minor", "message": string, "evidence": string, "suggestedFix": string, "time"?: string, "elementIds": string[], "objectIds": string[] } ], "metrics": { "typography": { ... }, "shapeDiversity": { ... }, "palette": { "harmonyScore": number, "harmonyScheme": string, "hueRelationshipScore": number, "saturationBalanceScore": number, "lumaBalanceScore": number, ... }, "structure": { ... }, "tempo": { ... }, "backgroundRichness": { ... }, "layerDensity": { "bands": [ { "startSeconds": number, "endSeconds": number, "visibleLayerCount": number, "foregroundLayerCount": number, "hasBackground": bool, "hasMidground": bool, "hasForeground": bool } ], ... }, "motionContinuity": { ... } }, "reviewNotes": string[] }`.
- **Use when**: after `render_still` and `evaluate_motion_variation`, and before `export_video`. Critical or major issues block normal export unless the user explicitly accepts them. The only deterministic quality categories that can produce gate-failing Major issues are `typographyReadTime`, `elementStructure`, `motionContinuity`, and `layerDensity` when it is a supplied-plan density violation.
- **Detects**: long all-caps text, excessive tracking, foreground `RectShape` dominance, unclear large/animated foreground shapes, animated shapes without motion intent, ordinary Elements with multiple EngineObjects, sparse layer density/depth coverage, sparse 120-140 BPM/high-tempo rhythm, misaligned text backing plates, dark teal/cyan/magenta, oversaturated or low-scoring hue-wheel palettes, flat single-layer backgrounds, outdated card/shadow/blur styling, low rendered motion variation, and unmotivated hard-cut rhythm.
...
- **Input**: `{ "videoType"?: "motion-graphics"|"footage-cut"|"slideshow"|"lyric-captions"|"logo-intro", "outputPrefix"?: string, "timeSeconds"?: number[], "sampleCount"?: number, "renderScale"?: number, "styleProfile"?: string, "requireAnimatedProperties"?: bool, "allowAllCaps"?: bool, "allowHardCuts"?: bool, "allowRectDominance"?: bool, "relaxAesthetics"?: bool, "allowStillness"?: bool, "allowDenseText"?: bool, "allowMultiObjectElements"?: bool, "allowMonochrome"?: bool, "allowMinimalDensity"?: bool, "plannedForegroundElementsPerShot"?: number, "staticLayout"?: bool, "confirmOverwrite"?: bool }` (still-frame writes are guarded). Bare prefixes are written under `agent-output/`.
...
- **Use when**: an agent thinks a scene is finished. Pass the resolved `videoType` used during planning. For motion-graphics deliverables, pass `requireAnimatedProperties=true` so `animatedPropertyCount=0` blocks export even when rendered samples changed, and pass `plannedForegroundElementsPerShot` from `quantitativePlanSheet` so `metrics.layerDensity` can enforce the below-half plan-violation blocker. Pass `allowMinimalDensity=true` only for a recorded minimal/negative-space brief. Pass `staticLayout=true` in the storyboard phase: it skips the motion analyzer and motion-required blockers, forces `requireAnimatedProperties=false`, and reports `readyForStoryboard` (never `readyForExport`) — use it to confirm a static storyboard before adding motion.
