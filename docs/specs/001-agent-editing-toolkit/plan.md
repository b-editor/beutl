...
**Primary Dependencies**: `ModelContextProtocol` 1.4.0 (official C# MCP SDK, MIT) + `Microsoft.Extensions.Hosting` for the **stdio** server; `ModelContextProtocol.AspNetCore` for the **in-app loopback HTTP/SSE** endpoint (the running GUI cannot be stdio-spawned, so the live mode needs a connectable transport — bound to loopback only). Internally `Beutl.Editor` (HistoryManager + Operations + editing services), `Beutl.ProjectSystem`, `Beutl.Engine` (CoreSerializer, PropertyRegistry, SceneRenderer), `Beutl.Extensibility` (encoder abstractions), `Beutl.FFmpegIpc` (the only path to the GPL worker). System.Text.Json for documents/merge-patch (no JSON-patch library — RFC 7396 is hand-rolled, ~30 lines).
...
**Constraints**: MIT-only; the GPL FFmpeg worker is reached solely through `Beutl.FFmpegIpc` (no `ProjectReference` to `Beutl.FFmpegWorker`). All writes confined to a configured workspace root. Rendering degrades to a CPU SkiaSharp surface when no Vulkan/MoltenVK device is present (vector/text/bitmap stills work CPU-side; GLSL/SKSL/particle/3D need a GPU — surfaced as a typed "rendering unavailable" error). FFmpeg native libraries are a runtime prerequisite for video export (preflight check + typed error).

**Scale/Scope**: Two new `src/` projects + one test project; ~6 MCP tool groups (schema, read, plan, apply, render-still, export-video) + thin imperative assists; covers visual + audio content, properties, keyframe animations, and effects across built-in and installed-extension types.
...
| II. Dual-Target Framework | PASS | The toolkit single-targets `net10.0`, matching its dependency closure (`Beutl.Editor`/`Engine`/`ProjectSystem`/`Extensibility`/`FFmpegIpc` are all single-target `net10.0`). No new TFM is introduced; the dual-target app/worker keep building untouched. |
...
- [contracts/mcp-tools.md](./contracts/mcp-tools.md) — the MCP tool surface (declarative-first + imperative assists + render/export + discovery), each tool's input/output/errors.
...
**Post-design Constitution re-check: PASS** — the design adds two MIT `net10.0` projects, no generator changes, NUnit-tested, no GPL edge except via `Beutl.FFmpegIpc`. No new violations; Complexity Tracking remains empty.

## Key risks carried into `/speckit-tasks`

- **Headless render fidelity on the CPU fallback** is unverified for GLSL/SKSL effects, particles, and 3D (those need a real Vulkan/MoltenVK device). Plan: render-capability preflight + typed "rendering unavailable" error; GPU-gated tests self-skip.
...
- **Reconciler identity edge cases** — new objects without an `Id` (mint one), duplicate/unknown `Id`s, the keyframe time-sort invariant (prefer the model's `AddKeyFrame`/`RemoveKeyFrame`), and engine-vs-core property dispatch (engine params live in `EngineObject.Properties`, not `PropertyRegistry`).
