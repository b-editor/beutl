...
- [x] T033 [P] [US2] Plan==apply parity test in `tests/Beutl.AgentToolkit.Tests/Reconciliation/PlanApplyParityTests.cs` — `plan_edit` change-set + validation equals the subsequent `apply_edit` for a representative edit set; `plan_edit.expectedChangeSet` can be passed directly and divergence is rejected (SC-009)
...
- [x] T039 [US2] Extend the reconciler with `plan_edit` (dry-run) in `src/Beutl.AgentToolkit/Reconciliation/PlanGenerator.cs` — reconcile on a **deep clone** of the root (no live events), emit change set + validation results, no mutation (FR-030; research §3)
- [x] T040 [US2] Wire `plan_edit`/`apply_edit` patch path + `expectedChangeSet` enforcement into `src/Beutl.AgentToolkit/Tools/EditTools.cs` — merge-patch → desired document → Id-diff → transaction; return `plan_edit.expectedChangeSet` for direct reuse and reject on divergence (SC-009; contracts/mcp-tools.md)
...
- [x] T042 [P] [US2] Keep keyframe editing on the declarative `plan_edit`/`apply_edit` surface — additions, updates, removals, time changes, values, and easing are represented under `Animations.<Property>.KeyFrames` rather than exposed as keyframe-specific imperative tools (FR-008; contracts/mcp-tools.md)
- [x] T043 [P] [US2] Keep visual and audio effect editing on the declarative `plan_edit`/`apply_edit` surface — visual effects are represented through `FilterEffect` / `FilterEffect.Children`, audio effects through `Effect` / `AudioEffectGroup.Children`, and attach/remove/reorder are expressed as id-keyed patch operations rather than effect-specific public tools (FR-009)
