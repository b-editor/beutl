...
- [x] T072 [US4] Implement video export in `src/Beutl.AgentToolkit/Rendering/VideoExporter.cs` (depends on **T070** for the registered encoder + **T071** for the providers) — build a delivery `SceneRenderer` + `FrameProviderImpl` + `new SceneComposer(scene, RenderIntent.Delivery, disableResourceShare: true, forceOriginalSource: true)` + `SampleProviderImpl` and call `EncodingController.Encode(frameProvider, sampleProvider, ct)`; FFmpeg-native-libs preflight ⇒ `codec_unavailable`; reuse the existing `ClampWorkingScaleToBufferBudget` (FR-017/FR-018; research §2)
