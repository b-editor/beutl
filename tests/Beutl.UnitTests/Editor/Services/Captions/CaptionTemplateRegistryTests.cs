...
public sealed class CaptionTemplateRegistryTests
{
    ...
    public void Register_RejectsCaseInsensitiveIdentifierCollision()
    {
        ...
        Assert.That(
            () => registry.Register(CreateText("vendor.template", "Second")),
            Throws.ArgumentException.With.Message.Contains("already registered"));
    }

    [Test]
    public void Register_RequiresExplicitReplacementOfExistingTemplate()
    {
        ...
        registry.Register(replacement, CaptionTemplateRegistrationMode.Replace);
    ...
    [Test]
    public void Register_MetadataObserverFailureDoesNotInterruptRegistryReplacement()
    {
        var registry = new CaptionTemplateRegistry();
        registry.Templates.CollectionChanged += (_, _) =>
            throw new InvalidOperationException("Observer failure");

        Assert.DoesNotThrow(() => registry.Register(CreateText("beutl.tests.safe", "Safe")));
        Assert.That(registry.GetRequired(new CaptionTemplateId("beutl.tests.safe")).Name, Is.EqualTo("Safe"));
    }

    [Test]
    public void Catalog_ComposesCodecAndTemplateExtensionsForNonUiConsumers()
    {
    ...
    [Test]
    public void Catalog_DiscardsAnInvalidExtensionAtomically()
    {
    ...
    [Test]
    public void Catalog_FailureReporterCannotInterruptRegistryChanges()
    {
        var extensions = new ExtensionProvider();
        using CaptionCatalog catalog = CaptionCatalog.Compose(
            "Default",
    ...
    [Test]
    public void Catalog_RefreshObjectTemplatesUpdatesTheSharedMetadataView()
    {
        var extensions = new ExtensionProvider();
        using CaptionCatalog catalog = CaptionCatalog.Compose("Default", [], extensions);
    ...
    [Test]
    public void Catalog_ObjectTemplatesRemainAlphabeticalAcrossComposeAndRefresh()
    {
        var extensions = new ExtensionProvider();
