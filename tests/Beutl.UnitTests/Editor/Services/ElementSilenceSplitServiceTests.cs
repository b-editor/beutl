...
public class ElementSilenceSplitServiceTests
{
    ...
    [Test]
    public void SplitBySilence_LockedElement_DoesNotDeleteOrCommit()
    {
        Element element = AddElement(TimeSpan.Zero, TimeSpan.FromSeconds(5));
        ...
        SilenceSplitOutcome outcome = _service.SplitBySilence(
            _scene,
            [element],
            regions,
            SilenceSplitMode.SplitAndDeleteSilence);
    ...
    [Test]
    public void SplitBySilence_DeletedElement_IsRemovedFromGroup()
    {
        Element element = AddElement(TimeSpan.Zero, TimeSpan.FromSeconds(5));
        Element firstSurvivor = AddElement(TimeSpan.FromSeconds(10), TimeSpan.FromSeconds(5));
        Element secondSurvivor = AddElement(TimeSpan.FromSeconds(20), TimeSpan.FromSeconds(5));
        _scene.Groups.Add([element.Id, firstSurvivor.Id, secondSurvivor.Id]);
        var regions = new[]
        {
            new SilenceRegion(TimeSpan.Zero, TimeSpan.FromSeconds(5)),
        };
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(_scene.Groups[0], Is.EquivalentTo(new[] { firstSurvivor.Id, secondSurvivor.Id }));
        });
    }
