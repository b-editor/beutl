...
public class GitInstallationLocatorTests
{
    ...
    [Test]
    public async Task Version_below_2_36_is_reported_as_too_old()
    {
        ...
        probe.Results[("/usr/local/bin/git", "--version")]
            = new GitProbeResult(0, "git version 2.35.9", "");
        var locator = new GitInstallationLocator(config, probe, GitHostPlatform.Linux);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(result.Version, Is.EqualTo(new Version(2, 35, 9)));
            Assert.That(result.LfsInstalled, Is.False);
        });
    }

    [Test]
    public async Task Version_2_36_is_supported()
    {
        var config = new VersionControlConfig();
        var probe = new FakeProbe { Paths = ["/usr/local/bin/git"] };
        probe.Results[("/usr/local/bin/git", "--version")]
            = new GitProbeResult(0, "git version 2.36.0", "");
        probe.Results[("/usr/local/bin/git", "lfs version")]
        ...
        {
            Assert.That(result.State, Is.EqualTo(GitAvailabilityState.Installed));
            Assert.That(result.Version, Is.EqualTo(new Version(2, 36, 0)));
            Assert.That(result.LfsInstalled, Is.False);
        });
    }
    ...
    [Test]
    public void Path_lookup_resolves_posix_empty_components_to_the_current_directory()
    {
        using var currentDirectory = new TemporaryDirectory();
        using var firstDirectory = new TemporaryDirectory();
        using var lastDirectory = new TemporaryDirectory();
        string currentGit = CreateFile(currentDirectory.Path, "git");
        string firstGit = CreateFile(firstDirectory.Path, "git");
        string lastGit = CreateFile(lastDirectory.Path, "git");

        Assert.Multiple(() =>
        {
            Assert.That(
                ProcessGitInstallationProbe.FindOnPath(
                    "git",
                    $":{firstDirectory.Path}",
                    pathExtensionsValue: null,
                    GitHostPlatform.Linux,
                    currentDirectory.Path),
                Is.EqualTo(new[] { currentGit, firstGit }),
                "A leading empty component should search the current directory first.");
            Assert.That(
                ProcessGitInstallationProbe.FindOnPath(
                    "git",
                    $"{firstDirectory.Path}:",
                    ...
                    GitHostPlatform.Linux,
                    currentDirectory.Path),
                Is.EqualTo(new[] { firstGit, currentGit }),
                "A trailing empty component should search the current directory last.");
            Assert.That(
                ProcessGitInstallationProbe.FindOnPath(
                    "git",
                    $"{firstDirectory.Path}:::{lastDirectory.Path}",
                    pathExtensionsValue: null,
                    GitHostPlatform.Linux,
                    currentDirectory.Path),
                Is.EqualTo(new[] { firstGit, currentGit, lastGit }),
                "Repeated empty components should retain PATH order without duplicating candidates.");
            Assert.That(
                ProcessGitInstallationProbe.FindOnPath(
                    "git",
                    string.Empty,
                    ...
                    GitHostPlatform.Linux,
                    currentDirectory.Path),
                Is.EqualTo(new[] { currentGit }),
                "An empty POSIX PATH consists of one current-directory component.");
        });
    }

    [Test]
    public void Path_lookup_keeps_windows_empty_component_behavior()
    {
        using var currentDirectory = new TemporaryDirectory();
        using var pathDirectory = new TemporaryDirectory();
        _ = CreateFile(currentDirectory.Path, "git");
        string pathGit = CreateFile(pathDirectory.Path, "git");

        IReadOnlyList<string> result = ProcessGitInstallationProbe.FindOnPath(
            "git",
            $";{pathDirectory.Path};;",
            pathExtensionsValue: null,
            GitHostPlatform.Windows,
            currentDirectory.Path);

        Assert.That(result, Is.EqualTo(new[] { pathGit }));
    }

    [Test]
    public void Path_lookup_resolves_relative_components_against_the_captured_current_directory()
    {
        using var currentDirectory = new TemporaryDirectory();
        string relativeDirectory = Path.Combine(currentDirectory.Path, "tools");
        Directory.CreateDirectory(relativeDirectory);
        string relativeGit = CreateFile(relativeDirectory, "git");
        ...
        Assert.That(result, Is.EqualTo(new[] { relativeGit }));
    }

    [Test]
    public async Task Windows_discovery_deduplicates_case_variant_candidates()
    {
        const string First = "C:/Program Files/Git/cmd/git.exe";
        const string Duplicate = "c:/program files/git/cmd/GIT.EXE";
        var config = new VersionControlConfig();
        var probe = new FakeProbe { Paths = [First, Duplicate] };
        probe.Results[(First, "--version")] = new GitProbeResult(1, "", "not executable");
        probe.Results[(Duplicate, "--version")] = new GitProbeResult(
            0,
            "git version 2.50.0.windows.1",
            "");
        var locator = new GitInstallationLocator(config, probe, GitHostPlatform.Windows);
        ...
        Assert.Multiple(() =>
        {
            Assert.That(result, Is.EqualTo(GitAvailability.NotInstalled));
            Assert.That(
                probe.RunCalls.Count(call => call.Arguments == "--version"),
                Is.EqualTo(1));
        });
    }
