...
public class RepositoryWatcherTests
{
    ...
    public void Reserved_directories_follow_the_actual_volume_case_semantics(
        string relativePath)
    {
        string reservedName = relativePath.StartsWith(".BEUTL", StringComparison.Ordinal)
            ? ".beutl"
            : ".git";
        Directory.CreateDirectory(Path.Combine(_tempDirectory, reservedName));
        string path = Path.Combine(
            _tempDirectory,
            relativePath.Replace('/', Path.DirectorySeparatorChar));

        string eventName = relativePath.Split('/')[0];
        Assert.That(
    ...
    [Test]
    public async Task Only_serialized_temporary_paths_schedule_status_changes()
    {
        string requiredDirectory = Path.Combine(_tempDirectory, "state");
        Directory.CreateDirectory(requiredDirectory);
        string requiredPath = Path.Combine(requiredDirectory, "session.tmp");
        string scratchPath = Path.Combine(requiredDirectory, "scratch.tmp");
        await File.WriteAllTextAsync(requiredPath, "required");
        ...
        using var watcher = new RepositoryWatcher(
            ...
            timeProvider,
            startWatching: false);
        watcher.UpdateRequiredPaths(new HashSet<string>(StringComparer.Ordinal)
        {
            "state/session.tmp",
            "state/not-temporary.bin",
            "../outside.tmp",
        });
        int raised = 0;
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(watcher.ShouldExcludeWatchedPath(requiredPath), Is.False);
            Assert.That(watcher.ShouldExcludeWatchedPath(scratchPath), Is.True);
        });
    }

    [Test]
    public void Updating_serialized_paths_replaces_the_temporary_path_allowlist()
    {
        string firstPath = Path.Combine(_tempDirectory, "first.tmp");
        string secondPath = Path.Combine(_tempDirectory, "second.tmp");
        ...
        using var watcher = new RepositoryWatcher(
            ...
            TimeProvider.System,
            startWatching: false);
        watcher.UpdateRequiredPaths(new HashSet<string>(StringComparer.Ordinal) { "first.tmp" });

        watcher.UpdateRequiredPaths(new HashSet<string>(StringComparer.Ordinal) { "second.tmp" });

        Assert.Multiple(() =>
        {
            Assert.That(watcher.ShouldExcludeWatchedPath(firstPath), Is.True);
            Assert.That(watcher.ShouldExcludeWatchedPath(secondPath), Is.False);
        });
    }
