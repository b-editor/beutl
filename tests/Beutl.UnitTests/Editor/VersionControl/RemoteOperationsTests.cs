...
public sealed class RemoteOperationsTests : RealGitTestRepository
{
    ...
    public async Task PullFastForward_reports_and_preserves_a_stale_HEAD_lock_before_mutation()
    {
        ...
        RepositoryLockInfo offeredLock = service.RecoverableLock!;
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(File.Exists(headLockPath), Is.EqualTo(!OperatingSystem.IsWindows()));
            if (OperatingSystem.IsWindows())
            {
                Assert.That(service.RecoverableLock, Is.Null);
            }
            else
            {
                Assert.That(service.RecoverableLock, Is.SameAs(offeredLock));
            }
        });
    }
