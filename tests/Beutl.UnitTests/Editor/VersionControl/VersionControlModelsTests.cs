...
public class VersionControlModelsTests
{
    ...
    [Test]
    public void RepositoryInfo_accepts_a_contained_project_that_differs_only_in_unicode_normalization()
    {
        string temporaryRoot = Path.Combine(
            Path.GetTempPath(),
            $"beutl-repository-normalization-{Guid.NewGuid():N}");
        string repositoryRoot = Path.Combine(temporaryRoot, "caf\u00e9");
        string projectRoot = Path.Combine(repositoryRoot, "project");
        Directory.CreateDirectory(projectRoot);
        try
        {
            string aliasRepositoryRoot = Path.Combine(temporaryRoot, "cafe\u0301");
            if (!Directory.Exists(aliasRepositoryRoot))
            {
                Assert.Ignore("The temporary volume is Unicode-normalization-sensitive.");
            }
    ...
    [Test]
    public void Entry_identity_follows_the_actual_volume_casing_rules()
    {
        string temporaryRoot = Path.Combine(
            Path.GetTempPath(),
            $"beutl-entry-casing-{Guid.NewGuid():N}");
        const string storedName = "ReservedMetadata";
        const string alternateName = "rESERVEDmETADATA";
        ...
        try
        {
            ...
            {
                Directory.CreateDirectory(Path.Combine(temporaryRoot, alternateName));
                Assert.That(
                    VersionControlPathComparison.TryAreSameChildPath(
                        temporaryRoot,
                        storedName,
