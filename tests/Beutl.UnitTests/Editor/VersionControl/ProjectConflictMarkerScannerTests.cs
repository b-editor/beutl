...
public class ProjectConflictMarkerScannerTests
{
    ...
    public async Task FindFirstAsync_applies_the_per_file_budget_at_the_marker_boundary()
    {
        ...
        string contents = prefix + LfConflict;
        await File.WriteAllTextAsync(sidecar, contents);
        int endMarkerOffset = contents.IndexOf(">>>>>>> theirs", StringComparison.Ordinal);
        long completeMarkerLength = System.Text.Encoding.UTF8.GetByteCount(
    ...
    [Test]
    public async Task FindFirstAsync_uses_stable_known_file_order_for_the_total_budget()
    {
        string projectFile = Path.Combine(_root, "project.bep");
        string firstFile = Path.Combine(_root, "a.scene");
        string conflictFile = Path.Combine(_root, "b.scene");
        const string firstContents = "abcdefghijklmnopqrstuvwxyz0123456789\n";
    ...
    public async Task FindFirstAsync_prioritizes_the_main_project_over_unknown_references()
    {
        ...
        Assert.That(result, Is.EqualTo(projectFile));
    }
    ...
    [Test]
    public void Metadata_directory_pruning_follows_the_actual_volume_casing_rules()
    {
        string gitDirectory = Path.Combine(_root, ".git");
        string alternateDirectory = Path.Combine(_root, ".Git");
        Directory.CreateDirectory(gitDirectory);
        bool volumeFoldsCase = Directory.Exists(alternateDirectory);
        if (!volumeFoldsCase)
        {
        ...
        Assert.That(
            ProjectConflictMarkerScanner.ShouldDescendInto(alternateDirectory),
            Is.EqualTo(!volumeFoldsCase));
    }
