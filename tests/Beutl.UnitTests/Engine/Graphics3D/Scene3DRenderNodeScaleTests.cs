...
{
    [Test]
    public void AllocationFailure_IsPropagatedOnlyForDelivery()
    {
        var failure = new InvalidOperationException("3D allocation failed");

        Assert.That(
            () => Scene3DRenderNode.ThrowIfDeliveryAllocationFailure(RenderIntent.Preview, failure),
            Throws.Nothing);
        InvalidOperationException? thrown = Assert.Throws<InvalidOperationException>(
            () => Scene3DRenderNode.ThrowIfDeliveryAllocationFailure(RenderIntent.Delivery, failure));
