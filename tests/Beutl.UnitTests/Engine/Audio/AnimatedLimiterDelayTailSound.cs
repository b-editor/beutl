...
public sealed partial class AnimatedLimiterDelayTailSound : Sound
{
    public AnimatedLimiterDelayTailSound() => ScanProperties<AnimatedLimiterDelayTailSound>();
    ...
    public override void Compose(AudioContext context, Sound.Resource resource)
    {
        ...
        limiter.Lookahead.Animation = new KeyFrameAnimation<float>
        {
            KeyFrames =
            {
                ...
                new KeyFrame<float>
                {
                    KeyTime = TimeRange.Duration - oneSample,
                    Value = 0f,
                    Easing = new LinearEasing(),
                },
                new KeyFrame<float>
