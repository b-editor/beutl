...
// A capture records the canvas's SurfaceDensity (the immutable whole-surface density) and the replay un-scales
// by THAT, independent of the replay canvas's own density — so a backdrop captured at one density and replayed
// inside a buffer-flushing FilterEffect (a different density) is not double-scaled. Under the base-CTM model the
// canvas bakes CreateScale(density), so both capture and replay draw in LOGICAL coords with no manual prescale.
[NonParallelizable]
[TestFixture]
public class BackdropScaleTests
{
    ...
    public void SnapshotBackdrop_ReplayedInFlush_NotDoubleScaled()
    {
        ...
        VulkanTestEnvironment.InvokeOnRenderThread(() =>
        {
            ...
            // feature 003: the canvas bakes the base CTM CreateScale(s_out), so the rect is drawn in LOGICAL coords
            // (no manual prescale). Snapshot records SurfaceDensity = s_out as the capture scale.
            ...
            // 2. Replay it on a separate density-s_out canvas (the new flush-canvas model: SurfaceDensity = w via
            // the baked base CTM, no manual prescale). The capture must un-scale by the density it was CAPTURED at.
    ...
    // the captured surface is w-dense; recording canvas.SurfaceDensity = w lets the replay un-scale by w (keying
    // off the current Density, which a PushDeviceSpace block lowers to 1, would mis-tag and double-scale it).
    [Test]
    public void SnapshotBackdropRenderNode_CapturedOnNestedFlushCanvas_NotDoubleScaled()
    {
        ...
        VulkanTestEnvironment.InvokeOnRenderThread(() =>
        {
            ...
            // CreateScale(w) (the new flush-canvas model — no manual prescale; the rect is drawn LOGICAL). The
            // snapshot op records canvas.SurfaceDensity = w as its capture scale.
