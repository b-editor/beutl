...
public sealed class GpuPassFusionFrozenBaselineLiveTests
{
    ...
    [Test]
    public void AuthoredPrimaryGammaStage_IsMeaningfullyNonVacuous()
    {
        VulkanTestEnvironment.EnsureAvailable();
        (FeatureVisualCapture reference, FeatureVisualCapture control) =
            VulkanTestEnvironment.InvokeOnRenderThread(() =>
            {
                FeatureVisualCapture reference = FeatureVisualEvidenceExporter.CaptureVisualSceneForTest(
                    "primary-cross-node",
                    outputScale: 1,
                    maxWorkingScale: 2,
                    requestedRegion: null);
                FeatureVisualCapture control = FeatureVisualEvidenceExporter.CaptureVisualSceneForTest(
                    "primary-control-gamma-disabled",
                    outputScale: 1,
                    maxWorkingScale: 2,
                    requestedRegion: null);
                return (reference, control);
            });
        Rgba16fParityMetrics delta = Rgba16fEvidenceWriter.CalculateParity(
            reference.Bytes,
            control.Bytes,
            reference.Width,
            reference.Height,
        ...
        double required = GpuPassFusionBaselineEvidence.NonVacuityParityTolerance + 0.005;
        TestContext.WriteLine(
            $"Authored gamma={FeatureVisualEvidenceExporter.PrimaryEvidenceGammaPercent:R}, "
            + $"discriminator={discriminator:F9}, required>{required:F9}");

        Assert.That(discriminator, Is.GreaterThan(required));
    }
