...
public sealed class LosslessCompositeCoverageTests
{
    ...
    public void DrawableRotation_RotatesDropShadowOffsetInLocalSpace()
    {
        ...
        VulkanTestEnvironment.InvokeOnRenderThread(() =>
        {
            var rotation = new RotationTransform(45);
            var shadow = new DropShadow();
            shadow.Position.CurrentValue = new Point(60, 0);
            shadow.Sigma.CurrentValue = new Size(4, 4);
            shadow.Color.CurrentValue = Colors.White;
            shadow.ShadowOnly.CurrentValue = true;
            var frame = new PixelSize(600, 600);
            using Drawable.Resource control = CreateTransformedRectangle(
                width: 200,
                height: 100,
                rotation,
                effect: null);
            using Drawable.Resource filtered = CreateTransformedRectangle(
                width: 200,
                height: 100,
                new RotationTransform(45),
                shadow);
            using Bitmap controlBitmap = RenderThroughPipeline(control, density: 1, frame);
            using Bitmap shadowBitmap = RenderThroughPipeline(filtered, density: 1, frame);

            Point controlCentroid = MeasureAlphaCentroid(controlBitmap);
            Point shadowCentroid = MeasureAlphaCentroid(shadowBitmap);
            var displacement = new Vector(
                shadowCentroid.X - controlCentroid.X,
                shadowCentroid.Y - controlCentroid.Y);
            TestContext.WriteLine(
                $"rotated shadow centroid displacement = ({displacement.X:F3}, {displacement.Y:F3})");

            Assert.Multiple(() =>
            {
                Assert.That(displacement.X, Is.InRange(38, 47),
                    "The local +x shadow offset must rotate down-right by 45 degrees.");
                Assert.That(displacement.Y, Is.InRange(38, 47),
                    "The local +x shadow offset must rotate down-right by 45 degrees.");
                Assert.That(Math.Abs(displacement.X - displacement.Y), Is.LessThan(2),
                    "The rotated shadow displacement must remain on the 45-degree local axis.");
            });
        });
    }

    [Test]
    public void DrawableScale_ScalesLocalBlurFootprint()
    {
        VulkanTestEnvironment.EnsureAvailable();
        VulkanTestEnvironment.InvokeOnRenderThread(() =>
        {
    ...
    private static Point MeasureAlphaCentroid(Bitmap bitmap)
    {
        ...
        for (int y = 0; y < bitmap.Height; y++)
        {
            for (int x = 0; x < bitmap.Width; x++)
            {
                ...
                double alpha = (double)BitConverter.UInt16BitsToHalf(pixels[pixelOffset + 3]);
        ...
        Assert.That(totalAlpha, Is.GreaterThan(0), "The centroid fixture must render non-transparent pixels.");
        return new Point((float)(weightedX / totalAlpha), (float)(weightedY / totalAlpha));
    }
    ...
    private static Drawable.Resource CreateTransformedRectangle(
        float width,
        float height,
        Transform transform,
        FilterEffect? effect)
