...
/// </summary>
/// <remarks>
/// <c>RenderCacheResolver.AddResources</c> writes the list's count and then each <c>CacheIdentity</c> in
/// declaration order into the output-cache key, so both the contents and the order of a <c>resources:</c>
...
{
    [Test]
    public void GeometryRenderNode_DeclaresGeometryThenHitTestThenPaint()
    {
        ...
        {
            IReadOnlyList<RenderResource> declared = payload.Description.Resources;
            Assert.Multiple(() =>
            {
    ...
    public void ImageSourceRenderNode_DeclaresTheSourceFirst()
    {
        ...
        WithOpaqueRoot(node, payload =>
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(declared[0].CacheIdentity.Version, Is.EqualTo(imageResource.Version));
            });
        });
    }
    ...
    private static object?[] Keys(IReadOnlyList<RenderResource> resources)
        => [.. resources.Select(static resource => resource.CacheIdentity.Key as Guid?)
    ...
    private sealed class TwoResourceNode(bool swapped) : RenderNode
    {
        ...
        public override void Process(RenderNodeContext context)
        {
            ...
            OpaqueRenderDescription description = OpaqueRenderDescription.Create(
                ...
                resources: swapped ? [second, first] : [first, second]);
