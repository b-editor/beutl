...
// feature 003 (FR-029): particles have no concrete bitmap input — each per-particle drawable re-rasterizes —
// so the supply-driven working density is just the output scale s_out. ParticleRenderNode rasterizes the
// per-particle drawable into a ceil(bounds × w) buffer and tags its composite op At(w). These tests assert the
// emitted op carries a CONCRETE At(w) density (never re-rasterizable Unbounded), at output scales 1.0 and 2.0,
// so a parent boundary treats the particle composite as the bitmap it is (FR-019b) rather than re-rasterizing
// it above the pixels it actually has.
[NonParallelizable]
[TestFixture]
public class ParticleRenderNodeScaleTests
{
    // EmissionRate=60/s, Lifetime=2s, MaxParticles=5000 are the ParticleEmitter constructor defaults, and
    // TimeRange.Start defaults to TimeSpan.Zero, so context.Time is the elapsed simulation time. At t=1s the
    // simulation deterministically holds alive particles (integral emit-per-step), so Process emits an op.
    ...
    // The particle composite is bitmap content at the density the cached per-particle drawable was rasterized at
    // (FR-019b): w == s_out, reported concretely (At(w).IsUnbounded == false), even at w == 1 where it is still
    // observably non-vector.
    [TestCase(1.0f)]
    [TestCase(2.0f)]
