...
public sealed class PairedVisualEvidenceArchiveTests
{
    private const string ExpectedResultSha256 =
        "86fd3e92f1bb578f2b404d41c35fea4a61a93b759f5061051a935e59acd91260";

    private const string ExpectedFeatureCodeSha =
    ...
    [Test]
    public void CurrentResultAndInputsMatchThePinnedRunAndEveryBlobHash()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(result.RootElement.GetProperty("schemaVersion").GetInt32(), Is.EqualTo(2));
            Assert.That(result.RootElement.GetProperty("status").GetString(), Is.EqualTo("passed"));
            Assert.That(
                result.RootElement.GetProperty("targetCodeSha").GetString(),
                Is.EqualTo(GpuPassFusionBaselineEvidence.ExpectedBaselineCodeSha));
            Assert.That(
                result.RootElement.GetProperty("featureCodeSha").GetString(),
                Is.EqualTo(ExpectedFeatureCodeSha));
        });
