...
- `tests/Beutl.HeadlessUITests/` references `src/Beutl` (the one suite that does — see `.claude/rules/csharp.md`) and drives the real shell: `ProjectService` / `EditorService` / `MainViewModel` / `EditViewModel`. Its `TestApp` reproduces `App.RegisterServices` (handlers, registrars, primitive-extension load) minus `App.Initialize`'s `RunStartupTask` (auth / update / restore I/O). Three `InternalsVisibleTo` grants (Beutl, Beutl.Core, Beutl.Extensibility) let it set the internal service handlers.
