...
internal static class FeatureVisualEvidenceExporter
{
    ...
    internal const float PrimaryEvidenceGammaPercent = 275;
    ...
    public static int Run(string[] args, TextWriter output, TextWriter error)
    {
        ...
        try
        {
            ...
            FeatureBaselineManifestSnapshot baselineSnapshot = LoadManifestSnapshot(baselineManifestPath);
            FeatureVisualGeneration generation = RenderThread.Dispatcher.Invoke(
                () => Generate(baselineSnapshot.Manifest, baselineSnapshot.Sha256));
    ...
    private static FeatureVisualGeneration Generate(JsonObject baseline, string baselineManifestSha256)
    {
    ...
    private static FeatureSceneFixture CreateManualFixture(string id)
    {
        return id switch
        {
            "primary-cross-node" => BuildPrimaryChain(PrimaryEvidenceGammaPercent, 0.62f, 72),
            "primary-control-all-identity" => BuildPrimaryChain(100, 1, 0),
            "primary-control-gamma-disabled" => BuildPrimaryChain(100, 0.62f, 72),
            "primary-control-opacity-disabled" => BuildPrimaryChain(PrimaryEvidenceGammaPercent, 1, 72),
            "primary-control-invert-disabled" => BuildPrimaryChain(PrimaryEvidenceGammaPercent, 0.62f, 0),
    ...
    private static FeatureBaselineManifestSnapshot LoadManifestSnapshot(string path)
    {
        if (!File.Exists(path))
        ...
        return new FeatureBaselineManifestSnapshot(manifest, Sha256Hex(bytes));
    }

    internal static FeatureBaselineManifestSnapshot LoadManifestSnapshotForTest(string path)
        => LoadManifestSnapshot(path);
    ...
    private static void WriteGeneration(string outputDirectory, FeatureVisualGeneration generation)
    {
        ...
        {
            string path = ResolveArtifactPath(outputDirectory, name);
            using var stream = new FileStream(path, FileMode.CreateNew, FileAccess.Write, FileShare.None);
            stream.Write(bytes);
        }

        string manifestPath = ResolveArtifactPath(outputDirectory, "manifest.json", allowManifest: true);
        ...
        writer.Write(generation.Manifest.ToJsonString(s_jsonOptions));
        writer.Write('\n');
    }
        ...
        bool allowManifest = false)
    {
        if (string.IsNullOrWhiteSpace(name)
            ...
            || (!allowManifest && string.Equals(name, "manifest.json", StringComparison.OrdinalIgnoreCase)))
        {
            throw new InvalidDataException($"Feature evidence artifact name is unsafe: '{name}'.");
        }
        ...
        if (!string.Equals(Path.GetDirectoryName(path), root, StringComparison.Ordinal))
            throw new InvalidDataException($"Feature evidence artifact escapes its output directory: '{name}'.");
        return path;
    }

    internal static string ResolveArtifactPathForTest(string outputDirectory, string name)
        => ResolveArtifactPath(outputDirectory, name);
    ...
    private static string CurrentCodeSha()
        => BenchmarkHarnessProvenance.ExtractSourceRevision(
            AssemblyVersion(typeof(FeatureVisualEvidenceExporter).Assembly));

    internal static string CurrentCodeShaForTest() => CurrentCodeSha();
...
internal static class Rgba16fEvidenceWriter
{
        ...
        PixelRect? region)
    {
        ...
        ValidateFiniteComponents(reference);
        ValidateFiniteComponents(actual);
    ...
    internal static void ValidateFiniteComponents(ReadOnlySpan<byte> value)
    {
        if ((value.Length & 1) != 0)
            throw new InvalidDataException("RGBA16F input must contain complete half-float components.");
        for (int offset = 0; offset < value.Length; offset += 2)
            _ = ReadFiniteHalf(value, offset);
    }
...
internal sealed record FeatureBaselineManifestSnapshot(JsonObject Manifest, string Sha256);
